"""
Moteur de detection : raccordement des cables aux noeuds du reseau.
...
Regle 2 — cables de terre (priorite majeure) :
...
Versions :
  ...
  agnostique de version, comme les autres controles de cable.
...
from recostar.controle.fonctions_communes.geometrie import extraire_extremites, extraire_point_xy
...
def executer_analyse(
    ...
    features_electrique, crs, fichier_cable_electrique_absent = charger_features(
        repertoire_resolu, FICHIER_CABLE_ELECTRIQUE
    )
    features_terre, crs_terre, fichier_cable_terre_absent = charger_features(repertoire_resolu, FICHIER_CABLE_TERRE)
    ...
    geojson_ecarts = construire_geojson_ecarts(anomalies, profil, crs if crs is not None else crs_terre)
    ...
    return {
        "succes": True,
        # Priorite de chaque type retenu, derivee du niveau de son code. Le
        # moteur ne la connait pas : elle depend du controle qui l'appelle.
        "priorites": {t: niveau_anomalie(profil.code_controle, t) for t in sorted(types_retenus)},
