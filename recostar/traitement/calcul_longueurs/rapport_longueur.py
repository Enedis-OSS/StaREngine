...
def _construire_statistiques(
    ...
    elements.append(
        Paragraph(
            f"Nombre de cables : <b>{nb_cables}</b> (HTA : <b>{nb_hta}</b>, BT : <b>{nb_bt}</b>)",
            styles["normal"],
        )
    )
    elements.append(
...
def _charger_resultats_longueurs(chemin_projet: str) -> list[dict[str, Any]]:
    ...
    if not donnees.get("succes"):
        return []
    return valider_resultats_longueurs(donnees)


def main() -> None:
    """Point d'entree du script de generation du rapport PDF des longueurs."""
    parseur = argparse.ArgumentParser(description="Generation du rapport PDF des longueurs de cables")
    ...
    resultats = _charger_resultats_longueurs(chemin_projet)
    ...
    chemin_pdf = generer_rapport_longueur(chemin_projet, resultats)
