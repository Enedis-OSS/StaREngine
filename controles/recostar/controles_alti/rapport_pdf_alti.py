"""
Generation du rapport PDF de synthese des controles altimetriques.

Produit un document PDF recapitulatif des resultats de l'ensemble des
controles altimetriques (3D, Z nul, sommets, IGN). Le rapport presente
une synthese globale, un tableau recapitulatif par controle, puis le
detail des entites en ecart avec leurs identifiants.

Usage CLI :
    python rapport_pdf_alti.py --repertoire <chemin> [--sortie <chemin>]

Sortie : rapport_controles_alti.pdf
"""
...
FICHIER_RAPPORT: str = "rapport_controles_alti.pdf"

# Dimensions de la page
LARGEUR_PAGE, HAUTEUR_PAGE = A4
MARGE = 2.0 * cm

# Label commun pour l'en-tete de colonne identifiant d'entite
_LABEL_ID_ENTITE: str = "ID entite"

# Configuration des colonnes de detail par controle
# Chaque entree : (cle_propriete, en_tete_colonne, largeur_cm)
_COLONNES_3D: tuple[tuple[str, str, float], ...] = (
    ("id_entite", _LABEL_ID_ENTITE, 4.5),
    ("fichier_source", "Fichier source", 6.0),
    ("type_geometrie", "Type geometrie", 3.5),
    ("priorite", "Priorite", 2.5),
)

_COLONNES_Z_NULL: tuple[tuple[str, str, float], ...] = (
    ("id_entite", _LABEL_ID_ENTITE, 3.5),
    ("fichier_source", "Fichier source", 5.0),
    ("indice_sommet", "Indice sommet", 2.0),
    ("type_geometrie", "Type geometrie", 3.0),
    ("priorite", "Priorite", 2.5),
)

_COLONNES_SOMMETS: tuple[tuple[str, str, float], ...] = (
    ("id_cable", "ID cable", 5.0),
    ("indice_sommet", "Indice sommet", 2.5),
    ("ecart_residuel_m", "Ecart (m)", 2.5),
    ("seuil_m", "Seuil (m)", 2.0),
    ("priorite", "Priorite", 2.5),
)

_COLONNES_IGN: tuple[tuple[str, str, float], ...] = (
    ("id_entite", _LABEL_ID_ENTITE, 3.0),
    ("indice_sommet", "Indice", 1.5),
    ("altitude_geojson_m", "Z GeoJSON (m)", 2.5),
    ("altitude_ign_m", "Z IGN (m)", 2.5),
    ("ecart_m", "Ecart (m)", 2.0),
    ("source_ign", "Source IGN", 3.0),
)
...
CONTROLES: tuple[tuple[str, str, str, tuple[tuple[str, str, float], ...]], ...] = (
    (
        "ecarts_3d.geojson",
        "Conformite 3D",
        "Entites ne possedant pas de coordonnees Z",
        _COLONNES_3D,
    ),
    (
        "ecarts_z_null.geojson",
        "Coordonnees Z nulles",
        "Sommets dont l'altitude est exactement 0.0",
        _COLONNES_Z_NULL,
    ),
    (
        "ecarts_controle_alti_sommets.geojson",
        "Altimetrie des sommets",
        "Incoherences altimetriques locales le long des cables",
        _COLONNES_SOMMETS,
    ),
    (
        "ecarts_z_ign.geojson",
        "Altimetrie IGN",
        "Ecarts entre les altitudes Z et les references IGN",
        _COLONNES_IGN,
    ),
)
...
def generer_rapport_pdf(
    ...
    """Genere le rapport PDF de synthese des controles altimetriques.
    ...
    # Titre
    elements.append(Paragraph("Rapport des controles altimetriques", styles["titre"]))
    elements.append(Spacer(1, 6 * mm))
...
def main() -> None:
    ...
    parseur = argparse.ArgumentParser(
        description="Generation du rapport PDF des controles altimetriques"
    )
