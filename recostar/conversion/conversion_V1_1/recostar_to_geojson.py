...
- Extraction des relations (cable-noeud, cheminement-cable, ouvrage-materiel, aerien-support)
...
# Tag canonique de la 1.1 : RecoStar-v1.1. L'ancien tag RecoStar-v1.10, émis
# par les versions précédentes du convertisseur, reste accepté en lecture pour
# la compatibilité des fichiers déjà produits (legacy).
SCHEMA_URL_V1_1 = "https://gitlab.com/StaR-Elec/StaR-Elec/-/raw/RecoStar-v1.1/RecoStaR/SchemaStarElecRecoStar.xsd"
...
# URLs reconnues comme V1.1 : tag canonique + tag legacy v1.10.
SCHEMA_URLS_V1_10: frozenset[str] = frozenset({SCHEMA_URL_V1_1, SCHEMA_URL_V1_10})
...
GEOMETRY_SURFACE_3D = "Surface3D"

# Nature de la geometrie portee par une RPD_GeometrieSupplementaire_Reco.
# Le format distingue ligne et surface la ou le GeoJSON ne connait que des
# types geometriques : une ligne fermee y est indiscernable d'un contour.
CHAMP_NATURE_GEOMETRIE = "nature_geometrie"
...
RELATION_TYPES = frozenset(["CableElectrique_NoeudReseau", "Cheminement_Cables", "Ouvrage_Materiel"])

_NS_LOOKUP: dict[str, str] = {
    "gml": NAMESPACE_GML,
...
def _tag_qualifie(namespace: str, element: str) -> str:
    return f"{{{_NS_LOOKUP[namespace]}}}{element}"
...
class GMLNamespaceHelper:
    ...
    def tag(self, namespace: str, element: str) -> str:
        ...
        return _tag_qualifie(namespace, element)
...
class GeometryParser:
    ...
    def parse_point(self, point_elem: ET.Element) -> dict | None:
    ...
    def parse_linestring(self, linestring_elem: ET.Element) -> dict | None:
    ...
    def parse_polygon(self, polygon_elem: ET.Element) -> dict | None:
    ...
    def parse_geometry(self, geom_elem: ET.Element) -> dict | None:
...
class EntityExtractor:
    ...
    def __init__(self, ns_helper: GMLNamespaceHelper):
        ...
        self.relations = {
            ...
            "support_aerien": {},  # support_id → [aerien_ids] (mapping N-N)
        }
    ...
    def _get_text(self, elem: ET.Element, child_name: str) -> str | None:
    ...
    def _get_href(self, elem: ET.Element, child_name: str) -> str | None:
    ...
    def peupler_reseau_href(self, properties: dict, elem: ET.Element):
        """Ajoute reseau_href aux propriétés d'une entité de réseau.

        Le XSD déclare `reseau` en maxOccurs="unbounded" sur ElementReseauType :
        une entité peut relever de plusieurs tranches de travaux. Les
        identifiants sont concaténés comme les autres relations N-N du GeoJSON.
        Les entités hors ElementReseauType (Materiel, GeometrieSupplementaire)
        ne portent aucune balise `reseau` et ressortent donc sans la propriété.
        """
        href_attr = self.ns_helper.tag("xlink", "href")
        reseaux = [
            href for ref in elem.findall(self.ns_helper.tag("RecoStaR", "reseau")) if (href := ref.get(href_attr))
        ]
        if reseaux:
            properties["reseau_href"] = ",".join(reseaux)

    def _get_measure(self, elem: ET.Element, child_name: str) -> tuple[float | None, str | None]:
    ...
    def _references_liees(self, nom_relation: str, entite_id: str | None) -> list[str]:
        """Retourne les identifiants liés à une entité pour une relation N-N donnée.

        Point d'accès unique aux relations multivaluées : évite de répéter la
        double résolution de dictionnaire dans chaque extracteur.
        """
        if not entite_id:
            return []
        return self.relations.get(nom_relation, {}).get(entite_id, [])

    def _peupler_cables_href(self, properties: dict, noeud_id: str | None):
        ...
        """
        cables = self._references_liees("cable_noeud", noeud_id)
        if not cables:
            return
    ...
    def _peupler_aeriens_href(self, properties: dict, support_id: str | None):
        """Ajoute aeriens_href aux propriétés d'un support.

        Même forme de stockage que cables_href : identifiants séparés par des
        virgules, dans l'ordre de lecture du GML. La relation est N-N, un
        support portant plusieurs tronçons aériens dès qu'il est en angle ou
        en dérivation.
        """
        aeriens = self._references_liees("support_aerien", support_id)
        if aeriens:
            properties["aeriens_href"] = ",".join(aeriens)

    def _assembler_geometries_cheminements(self, cable_id: str | None) -> dict | None:
        ...
        all_coords = [geom_cache[cid]["coordinates"] for cid in cheminement_ids if cid in geom_cache]
    ...
    def extract_coffret(self, elem: ET.Element) -> dict:
        ...
        properties = {
            ...
            "Statut": statut,
            "Commentaire": self._get_text(elem, "Commentaire"),
            "geometriesupplementaire_href": self._get_href(elem, "geometriesupplementaire"),
        }
    ...
    def extract_galerie(self, elem: ET.Element) -> dict:
        """Extrait RPD_Galerie_Reco et stocke la géométrie pour héritage par les câbles"""
        gml_id = elem.get(self.ns_helper.tag("gml", "id"))

        properties = {
            "fid": self._get_fid("RPD_Galerie_Reco"),
            "ogr_pkid": f"RPD_Galerie_Reco_line_{self.counter['RPD_Galerie_Reco'] - 1}",
            "id": gml_id,
            "PrecisionXY": self._get_text(elem, "PrecisionXY"),
            "PrecisionZ": self._get_text(elem, "PrecisionZ"),
        }

        # Hauteur avec UOM (requis selon XSD)
        hauteur, hauteur_uom = self._get_measure(elem, "Hauteur")
        if hauteur is not None:
            properties["Hauteur"] = hauteur
            properties["Hauteur_uom"] = hauteur_uom

        # Largeur avec UOM (requis selon XSD)
        largeur, largeur_uom = self._get_measure(elem, "Largeur")
        if largeur is not None:
            properties["Largeur"] = largeur
            properties["Largeur_uom"] = largeur_uom

        # ProfondeurMinNonReg avec UOM (optionnel)
        profondeur, profondeur_uom = self._get_measure(elem, "ProfondeurMinNonReg")
        if profondeur is not None:
            properties["ProfondeurMinNonReg"] = profondeur
            properties["ProfondeurMinNonReg_uom"] = profondeur_uom

        # Référence câbles depuis les relations Cheminement_Cables
        if gml_id in self.relations["cheminement_cable"]:
            properties["cables_href"] = self.relations["cheminement_cable"][gml_id]
        ...
        # Stockage obligatoire pour héritage de géométrie par les câbles associés
        if geometry and gml_id:
            self.cheminement_geometries[gml_id] = geometry
    ...
    def _trouver_linestring(self, ligne_elem: ET.Element) -> ET.Element | None:
        """Retourne le gml:LineString porte par un Ligne3D ou Ligne2.5D.

        Les deux encapsulations du format sont acceptees : le LineString
        directement sous la balise, ou enveloppe dans un MultiCurve.
        """
        direct = ligne_elem.find(self.ns_helper.tag("gml", "LineString"))
        if direct is not None:
            return direct
        multicurve = ligne_elem.find(self.ns_helper.tag("gml", "MultiCurve"))
        if multicurve is None:
            return None
        curve_member = multicurve.find(self.ns_helper.tag("gml", "curveMember"))
        if curve_member is None:
            return None
        return curve_member.find(self.ns_helper.tag("gml", "LineString"))
    ...
    def extract_geometrie_supplementaire(self, elem: ET.Element) -> dict:
        ...
        if surface_elem is not None:
            ...
            if polygon:
                ...
                properties[CHAMP_NATURE_GEOMETRIE] = GEOMETRY_SURFACE_3D

        # Ligne3D (RPD) ou Ligne2.5D (legacy). Sans surface pour l'occuper, la
        # ligne devient la geometrie de la feature : une entite dont toute la
        # geometrie tiendrait dans une propriete texte ressortirait sans
        # geometrie, et les controles la liraient comme telle.
        ...
        if ligne_elem is not None:
            linestring = self._trouver_linestring(ligne_elem)
            if geometry is None and linestring is not None:
                geometry = self.geom_parser.parse_linestring(linestring)
                properties[CHAMP_NATURE_GEOMETRIE] = GEOMETRY_LIGNE_3D
            else:
                # Une surface occupe deja la geometrie, ou la ligne est
                # inexploitable : le canal texte reste son seul support.
    ...
    def extract_support(self, elem: ET.Element) -> dict:
        ...
        # Aériens portés par ce support, depuis les relations Aerien_Support
        self._peupler_aeriens_href(properties, gml_id)
    ...
    def extract_module_raccordement(self, elem: ET.Element) -> dict:
        ...
        properties = {
            ...
            "ogr_pkid": (f"RPD_ModuleRaccordement_Reco_{self.counter['RPD_ModuleRaccordement_Reco'] - 1}"),
            "id": gml_id,
    ...
    def extract_poste_electrique(self, elem: ET.Element) -> dict:
        ...
        properties = {
            ...
            "conteneur_href": self._get_href(elem, "conteneur"),
            "InformationSupplementaire": self._get_text(elem, "InformationSupplementaire"),
            "Statut": self._get_text(elem, "Statut"),
...
class GMLConverter:
    ...
    def _detecter_version_schema(self, root: ET.Element) -> str:
        ...
        # Vérification exacte : toutes les URLs connues comme V1.0 puis V1.1.
        if any(url in schema_location for url in SCHEMA_URLS_V1_0):
            return VERSION_V1_0
        if any(url in schema_location for url in SCHEMA_URLS_V1_10):
            return VERSION_V1_10

        # Fallback : recherche partielle. 'RecoStar-v1.1' couvre le tag canonique
        # comme le tag legacy 'RecoStar-v1.10' (sous-chaîne).
    ...
    def _extraire_metadonnees_completes(self, root: ET.Element) -> dict:
        ...
        # Extraction des éléments ReseauUtilite. Le modèle en autorise
        # plusieurs par fichier (tranches de travaux, cf. regles_entete :
        # cardinalité (1, -1)), d'où une liste et non un objet unique.
        reseaux = [
            self._extraire_champs_reseau(reseau_elem)
            for reseau_elem in root.findall(f".//{tag('RecoStaR', 'ReseauUtilite')}")
        ]
        if reseaux:
            info["ReseauUtilite"] = reseaux
    ...
    def _store_cable_noeud(self, child: ET.Element, get_href):
        ...
        if etat:
            self.extractor.relations["cable_noeud_etat"][f"{noeud_id}_{cable_id}"] = etat
    ...
    def _store_aerien_support(self, child: ET.Element, get_href):
        """Enregistre une relation Aerien_Support (N aériens ↔ N supports).

        Le sens conservé est support → aériens : c'est le support qui porte
        la propriété aeriens_href dans le GeoJSON.
        """
        aerien_id = get_href(child, "aerien")
        support_id = get_href(child, "support")
        if not (aerien_id and support_id):
            return
        self.extractor.relations["support_aerien"].setdefault(support_id, []).append(aerien_id)
    ...
    def _extract_relations(self, root: ET.Element):
        ...
        relation_handlers = {
            ...
            "Aerien_Support": self._store_aerien_support,
        }
    ...
    def _collecter_ids_cables_terre(self, features_by_type: dict[str, list[dict]]) -> set:
    ...
    def _construire_index_tension_cables(self, features_by_type: dict[str, list[dict]]) -> dict[str, str]:
    ...
    def _regrouper_noeuds_par_conteneur(
        ...
        for type_entite in noeud_types:
            for feat in features_by_type.get(type_entite, []):
                ...
                if conteneur:
                    conteneur_noeuds.setdefault(conteneur, []).append((type_entite, props))
        return conteneur_noeuds

    def _collecter_cables_electriques_du_groupe(self, noeuds: list[tuple[str, dict]], cable_terre_ids: set) -> set:
    ...
    def _nettoyer_cables_noeud_terre(self, props: dict, cable_terre_ids: set) -> bool:
        ...
        if not href:
            return False
        cables_restants = [cid.strip() for cid in href.split(",") if cid.strip() in cable_terre_ids]
        if cables_restants:
    ...
    def _filtrer_cables_par_tension_jonction(
        ...
        return {id_cable for id_cable in cables_electriques if index_tension_cables.get(id_cable) == tension_jonction}
    ...
    def _appliquer_propagation_conteneur(
        ...
        Retourne (nb_propagations, nb_nettoyages_terre).
        """
        cables_electriques = self._collecter_cables_electriques_du_groupe(noeuds, cable_terre_ids)
    ...
    def _propager_cables_dans_conteneurs(self, features_by_type: dict[str, list[dict]]):
        ...
        conteneur_noeuds = self._regrouper_noeuds_par_conteneur(features_by_type, noeud_types)

        propagation_count = 0
        nettoyage_terre_count = 0

        for noeuds in conteneur_noeuds.values():
            prop, nett = self._appliquer_propagation_conteneur(noeuds, cable_terre_ids, index_tension_cables)
            propagation_count += prop
            nettoyage_terre_count += nett

        if propagation_count > 0:
            print(f"  [OK] {propagation_count} noeud(s) enrichi(s) avec câbles du conteneur")
        if nettoyage_terre_count > 0:
            print(f"  [OK] {nettoyage_terre_count} noeud(s) Terre nettoyé(s) (câbles électriques retirés)")

    def _inject_materiel_properties_into_jonctions(self, features_by_type: dict[str, list[dict]]):
        ...
        for materiel_feat in materiels:
            ...
            if materiel_id:
                ...
                materiel_props = {field: props.get(field) for field in required_fields if props.get(field) is not None}
        ...
        if injection_count > 0:
            print(f"  [OK] {injection_count} jonction(s) enrichie(s) avec propriétés matériel")
    ...
    def _extract_features_by_pass(
        ...
        for member in root.findall(self.ns_helper.tag("gml", "featureMember")):
            for child in member:
                ...
                if tag in exclude_types or tag not in target_types or tag not in extractors:
                    continue
                try:
                    feature = extractors[tag](child)
                    # Rattachement au reseau : applique ici plutot que dans
                    # chaque extracteur, la balise etant heritee par toutes
                    # les entites d'ElementReseauType.
                    self.extractor.peupler_reseau_href(feature["properties"], child)
                    features_by_type[tag].append(feature)
    ...
    def _generer_geom_supp_v1_0(self, features_by_type: dict[str, list[dict]]):
        ...
        geom_supp_list = features_by_type.setdefault("RPD_GeometrieSupplementaire_Reco", [])
        ...
        if creation_count > 0:
            print(f"  [OK] {creation_count} geometrie(s) supplementaire(s) creee(s) pour les supports (V1.0)")
    ...
    def _write_geojson_files(self, features_by_type: dict, output_dir: Path):
        ...
        for entity_type, features in features_by_type.items():
            ...
            geojson_data = {
                ...
                "crs": {
                    "type": "name",
                    "properties": {"name": f"urn:ogc:def:crs:{self.srs.replace(':', '::')}"},
                },
                "features": features,
            }
    ...
    def convert_gml_to_geojson(self, gml_path: Path, output_dir: Path):
        ...
        extractors = {
            ...
            "RPD_Galerie_Reco": self.extractor.extract_galerie,
        ...
        self._extract_features_by_pass(root, extractors, features_by_type, conteneur_types)

        print("Extraction des cheminements...")
        self._extract_features_by_pass(root, extractors, features_by_type, cheminement_types)

        print("Extraction des autres entités...")
        already_processed = conteneur_types | cheminement_types
        self._extract_features_by_pass(root, extractors, features_by_type, all_types, already_processed)
...
Exemples:
  ...
  python gml_to_geojson.py recolement.gml geojson_output/
    ...
    parser.add_argument("output_dir", type=Path, help="Répertoire de sortie pour les fichiers GeoJSON")
