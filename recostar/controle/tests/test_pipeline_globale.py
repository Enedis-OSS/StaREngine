...
from recostar.controle.pipeline_globale import (
    DOSSIER_CONTROLE,
...
class TestRegistreFamilles:
    ...
    def test_familles_declarees(self) -> None:
        ...
        assert cles == {"structuration", "projection", "altimetrie", "cheminement", "cable", "conteneur"}
...
class TestCodeEtLibelle:
    ...
    def test_code_convention_geojson(self) -> None:
        assert code_controle("e5107") == "E5107"

    def test_code_convention_xsd(self) -> None:
        assert code_controle("E0110") == "E0110"

    def test_libelle_declare(self) -> None:
        assert libelle_controle("E-6110") == LIBELLES_CONTROLES["E-6110"]

    def test_libelle_absent_retombe_sur_le_code(self) -> None:
        assert libelle_controle("e0999") == "E0999"
...
class TestExecuterPipeline:
    ...
    def test_numero_affaire_transmis_a_projection(self, jeu_minimal: Path) -> None:
        """Sans numero d'affaire, E-5106 ne peut pas s'executer ; avec, il s'execute."""
        sans = executer_pipeline(str(jeu_minimal))
        codes_en_echec = set(sans["familles"]["projection"]["controles_en_echec"])
        assert "E-5106" in codes_en_echec
        ...
        assert controles["E-5106"]["erreur"] != "Parametre --numero_affaire requis"

    def test_numero_affaire_transmis_au_cable(self, jeu_minimal: Path) -> None:
        """Sans numero d'affaire, E-3304 ne peut pas s'executer ; avec, il s'execute."""
        sans = executer_pipeline(str(jeu_minimal))
        assert "E-3304" in set(sans["familles"]["cable"]["controles_en_echec"])

        avec = executer_pipeline(str(jeu_minimal), numero_affaire="RAC-ABC-12-345678")
        controles = {c["code"]: c for c in avec["familles"]["cable"]["controles"]}
        assert controles["E-3304"]["erreur"] != "Parametre --numero_affaire requis"
...
class TestGenererRapportPdf:
    ...
    def _familles(self) -> tuple[ResultatFamille, ...]:
        ...
        return (
            ResultatFamille(
                ...
                "Câble",
                (
                    ResultatControle("E-9500", "Cohérence du DomaineTension", True, 0, {}),
                    ResultatControle("E-6110", "Raccordement", True, 3, {PRIORITE_FORTE: 3}),
                ),
            ),
            ResultatFamille(
                "projection",
                "Projection",
                (ResultatControle("E-5106", "Emprise DR", False, 0, {}, "Parametre requis"),),
            ),
    ...
    def test_pdf_sans_aucune_anomalie(self, tmp_path: Path) -> None:
        familles = (ResultatFamille("cable", "Câble", (ResultatControle("E-9500", "Libelle", True, 0, {}),)),)
        chemin = tmp_path / "rapport.pdf"
        generer_rapport_pdf(familles, agreger(familles), chemin, tmp_path)
        assert chemin.is_file()
