"""
Tests du moteur de nomenclature de composition des coffrets.
...
from recostar.controle.conteneur.detection_composition_coffret import (
    AU_PLUS_UN,
    CHAMP_TYPE_COFFRET_HREF,
    COUCHES_NOEUDS_AUTORISES,
    COUPE_CIRCUIT,
    EXACTEMENT_UN,
    JEU_BARRES,
    MODULE_RACCORDEMENT,
    ...
    relever_coffrets_rattaches,
    resoudre_type_coffret,
)
from recostar.controle.conteneur.e6107 import PROFIL_ECARTS as _PROFIL_6107
from recostar.controle.conteneur.e6107 import TYPES_RETENUS as _TYPES_6107
from recostar.controle.conteneur.e6208 import PROFIL_ECARTS as _PROFIL_6208
from recostar.controle.conteneur.e6208 import TYPES_RETENUS as _TYPES_6208
...
_PROFIL_PAR_TYPE = {
    **dict.fromkeys(_TYPES_6107, _PROFIL_6107),
    **dict.fromkeys(_TYPES_6208, _PROFIL_6208),
}

_PROFIL_DEFAUT = _PROFIL_6107
...
def _noeud(
    identifiant: str,
    conteneur_href: Any = ID_COFFRET,
    statut: str = "UnderCommissionning",
) -> dict[str, Any]:
    """Feature GeoJSON representant un noeud rattache a un coffret.

    Le Statut est renseigne par defaut : un noeud hors service ne satisfait pas
    l'obligation de rattachement (E-6107), et le laisser vide ferait signaler
    tous les coffrets des jeux de test.
    """
    return {
        "type": "Feature",
        "properties": {
            "id": identifiant,
            "conteneur_href": conteneur_href,
            CHAMP_STATUT: statut,
        },
        "geometry": None,
    }
...
class TestClassifierComposition:
    ...
    def test_aucune_obligation_de_presence(self) -> None:
        """La nomenclature ne plafonne que les effectifs.

        L'obligation de rattachement porte desormais sur le coffret entier
        (E-6107), et non type par type : un coffret vide ne contrevient a aucune
        regle de composition.
        """
        assert classifier_composition(NOMENCLATURES["RMBT300"], {}) == []
    ...
    def test_cumul_compte_et_interdiction(self) -> None:
        ...
        assert codes == [TYPE_NOEUDS_EXCESSIFS, TYPE_NOEUD_NON_AUTORISE]
...
class TestDetecterAnomalies:
    ...
    def test_coffret_conforme(self) -> None:
        ...
        assert detecter_anomalies(_index(), comptes, {ID_COFFRET}) == []

    def test_coffret_rattache_sans_compte_est_conforme(self) -> None:
        """Un coffret absent des comptes mais rattache ne contrevient a rien."""
        assert detecter_anomalies(_index(), {}, {ID_COFFRET}) == []

    def test_coffret_non_rattache_signale(self) -> None:
        """Aucun noeud autorise ne reference le coffret : c'est l'anomalie E-6107."""
        anomalies = detecter_anomalies(_index(), {}, set())
        assert _types(anomalies) == [TYPE_COFFRET_SANS_NOEUD]
        assert anomalies[0]["nombre_trouve"] == 0
        assert anomalies[0]["id_coffret"] == ID_COFFRET

    def test_champs_de_l_anomalie(self) -> None:
        comptes = {ID_COFFRET: {MODULE_RACCORDEMENT: 1, TERRE: 4}}
        anomalie = detecter_anomalies(_index(), comptes, {ID_COFFRET})[0]
    ...
    def test_plusieurs_anomalies_par_coffret(self) -> None:
        ...
        anomalies = detecter_anomalies(_index(), comptes, {ID_COFFRET})
    ...
    def test_plusieurs_coffrets(self) -> None:
        ...
        anomalies = detecter_anomalies(coffrets, {"cof2": {JEU_BARRES: 1}}, {"cof1", "cof2"})
        assert _types(anomalies) == [TYPE_NOEUD_NON_AUTORISE]
        assert compter_coffrets_non_conformes(anomalies) == 1
...
class TestObligationDeRattachement:
    """Regle E-6107 : un coffret doit etre rattache a au moins un noeud autorise.

    La regle ne se prononce pas sur le TypeCoffret : n'importe lequel des sept
    types admis suffit, quel que soit le coffret. Elle est donc independante des
    nomenclatures, qui plafonnent les effectifs type par type.
    """
    ...
    def test_liste_des_couches_autorisees(self) -> None:
        assert COUCHES_NOEUDS_AUTORISES == self._COUCHES_ATTENDUES

    def test_jonction_hors_liste(self) -> None:
        """La jonction n'est pas un noeud de coffret : elle ne satisfait pas la regle."""
        assert "RPD_Jonction_Reco" not in COUCHES_NOEUDS_AUTORISES

    def _jeu(self, tmp_path: Any, couche: str | None, statut: str = "UnderCommissionning") -> str:
        noeuds = {couche: [_noeud("n1", ID_COFFRET, statut)]} if couche else {}
        _ecrire_jeu(tmp_path, [_coffret()], noeuds)
        return str(tmp_path)

    def test_support_modules_suffit(self, tmp_path: Any) -> None:
        """Cas du jeu PLAISANCE : un RMBT600 porte par un SupportModules seul.

        La nomenclature RMBT600 n'exige plus de ModuleRaccordement : ce coffret
        etait signale a tort, alors que le verificateur ne le releve pas.
        """
        rattaches = relever_coffrets_rattaches(self._jeu(tmp_path, "RPD_SupportModules_Reco"), _index("RMBT600"))
        assert rattaches == {ID_COFFRET}

    def test_statut_hors_service_ne_compte_pas(self, tmp_path: Any) -> None:
        repertoire = self._jeu(tmp_path, "RPD_SupportModules_Reco", statut="Abandoned")
        assert relever_coffrets_rattaches(repertoire, _index()) == set()

    def test_statut_functional_compte(self, tmp_path: Any) -> None:
        repertoire = self._jeu(tmp_path, "RPD_Terre_Reco", statut="Functional")
        assert relever_coffrets_rattaches(repertoire, _index()) == {ID_COFFRET}

    def test_couche_non_autorisee_ne_compte_pas(self, tmp_path: Any) -> None:
        repertoire = self._jeu(tmp_path, COUCHE_INTERDITE)
        assert relever_coffrets_rattaches(repertoire, _index()) == set()

    def test_aucun_noeud(self, tmp_path: Any) -> None:
        assert relever_coffrets_rattaches(self._jeu(tmp_path, None), _index()) == set()

    def test_bout_en_bout_coffret_rattache_conforme(self, tmp_path: Any) -> None:
        repertoire = self._jeu(tmp_path, "RPD_SupportModules_Reco")
        assert _executer(repertoire)["nombre_anomalies"] == 0

    def test_bout_en_bout_coffret_non_rattache_signale(self, tmp_path: Any) -> None:
        repertoire = self._jeu(tmp_path, COUCHE_INTERDITE)
        resultat = _executer(repertoire)
        assert resultat["anomalies_par_type"][TYPE_COFFRET_SANS_NOEUD] == 1
...
class TestConstruireGeojsonEcarts:
    ...
    def test_collection_vide(self) -> None:
        assert construire_geojson_ecarts([], _PROFIL_DEFAUT) == {
            "type": "FeatureCollection",
            "features": [],
        }

    def test_socle_commun(self) -> None:
        anomalies = detecter_anomalies(_index(), {ID_COFFRET: {TERRE: 2}}, {ID_COFFRET})
    ...
    def test_proprietes_metier(self) -> None:
        ...
        proprietes = construire_geojson_ecarts(
            detecter_anomalies(_index(), comptes, {ID_COFFRET}),
            _profil(detecter_anomalies(_index(), comptes, {ID_COFFRET})),
    ...
    def test_geometrie_du_coffret(self) -> None:
        anomalies = detecter_anomalies(_index(), {ID_COFFRET: {TERRE: 2}}, {ID_COFFRET})
        assert construire_geojson_ecarts(anomalies, _profil(anomalies))["features"][0]["geometry"] == GEOM_COFFRET
...
class TestExecuterControleCli:
    ...
    def test_coffret_sans_noeud_rattache(self, tmp_path: Any) -> None:
        """Aucun noeud ne reference le coffret : obligation de rattachement (E-6107)."""
        _ecrire_jeu(tmp_path, [_coffret()], {})
        resultat = _executer(str(tmp_path))
        assert resultat["anomalies_par_type"] == {TYPE_COFFRET_SANS_NOEUD: 1}
...
class TestVersions:
    ...
    def test_champs_supplementaires_v1_1_sans_effet(self, tmp_path: Any) -> None:
        ...
        resultat = _executer(str(tmp_path))
        assert resultat["anomalies_par_type"] == {TYPE_COFFRET_SANS_NOEUD: 2}
