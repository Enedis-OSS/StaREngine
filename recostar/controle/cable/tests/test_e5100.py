"""
Tests du controle E-5100 : densite de sommets des cables electriques.
...
from recostar.controle.cable.e5100 import (
    FICHIER_SORTIE,
    SEUIL_DISTANCE,
    STATUT_CONTROLE,
    TYPE_ANOMALIE,
...
from recostar.controle.fonctions_communes.geometrie import extraire_parties_lineaires
...
class TestAnalyserGeometrie:
    ...
    def test_distance_3d(self) -> None:
        """Le denivele compte dans la distance entre sommets consecutifs.

        Les altitudes sont volontairement non nulles : un Z a 0.0 signale une
        altitude absente et serait corrige par propagation, ce qui annulerait le
        denivele que ce test verifie.
        """
        # Segment horizontal de 12 m + denivele de 9 m -> 15 m (3D), conforme
        _, nb = analyser_geometrie(_ligne([[0.0, 0.0, 100.0], [12.0, 0.0, 109.0]]))
        assert nb == 0
        # Meme XY mais denivele de 10 m -> sqrt(144+100) ~ 15.62 m -> non conforme
        distance_max, nb2 = analyser_geometrie(_ligne([[0.0, 0.0, 100.0], [12.0, 0.0, 110.0]]))
        assert nb2 == 1
        assert distance_max > SEUIL_DISTANCE

    def test_z_nul_ne_cree_pas_de_faux_positif(self) -> None:
        """Un sommet a Z=0 ne doit pas produire un denivele fictif.

        Cas rencontre en production : l'extremite d'un cheminement est un point
        calcule par le logiciel de saisie, dont l'altitude n'a jamais ete
        renseignee (Z=0), tandis que les sommets leves sont vers 310,9 m NGF.
        Avant correction, ce segment de 63 cm etait mesure a 310,92 m et rendait
        le cable non conforme. L'altitude manquante releve d'E-5107, pas d'E-5100.
        """
        geom = _ligne(
            [
        ...
        assert nb == 0
        assert distance_max < 1.0

    def test_z_nul_ne_masque_pas_un_vrai_depassement(self) -> None:
        """La correction du Z ne doit pas rendre le controle aveugle en planimetrie."""
        geom = _ligne([[0.0, 0.0, 0.0], [30.0, 0.0, 310.92]])
        distance_max, nb = analyser_geometrie(geom)
        assert nb == 1
        assert distance_max == pytest.approx(30.0)
