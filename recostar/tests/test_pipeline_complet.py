...
class TestRegistreEtapes:
    ...
    def test_seule_l_etape_de_controle_est_un_paquet(self) -> None:
        """Le controle est le seul domaine packagé ; les autres restent a imports plats."""
        assert {e.cle for e in ETAPES if e.module is not None} == {"controle"}

    def test_module_declare_correspond_au_script(self) -> None:
        """Le module et le script doivent designer le meme fichier, sans quoi le
        garde-fou d'existence surveillerait autre chose que ce qui est execute."""
        for etape in ETAPES:
            if etape.module is None:
                continue
            attendu = etape.module.removeprefix("recostar.").replace(".", "/") + ".py"
            assert etape.script == attendu, etape.cle


class TestInvocation:
    """Forme d'invocation d'une etape : par chemin ou par module."""

    def test_etape_a_imports_plats_invoquee_par_chemin(self) -> None:
        etape = next(e for e in ETAPES if e.module is None)
        invocation = _invocation(etape)
        assert len(invocation) == 1
        assert invocation[0] == str(RACINE / etape.script)

    def test_etape_packagee_invoquee_par_module(self) -> None:
        """`-m` est indispensable : par chemin, les imports absolus du paquet
        ne se resoudraient pas, sys.path[0] etant le dossier du script."""
        etape = next(e for e in ETAPES if e.module is not None)
        assert _invocation(etape) == ["-m", etape.module]

    def test_module_importable(self) -> None:
        """Le module declare doit exister : une faute de frappe passerait sinon
        inapercue jusqu'a l'execution du pipeline complet."""
        from importlib.util import find_spec

        for etape in ETAPES:
            if etape.module is not None:
                assert find_spec(etape.module) is not None, etape.module
