"""
Tests du controle E-6109 : localisation des points de comptage et ouvrages collectifs.
...
Couvre :
  ...
  - l'execution CLI
  - la coherence avec E-6105 et E-6204
...
from recostar.controle.conteneur.e6109 import (
    COUCHES_CIBLES,
...
def _ecrire_jeu(
    ...
    ecrire_collection(
        str(tmp_path / f"{couche}{EXTENSION_COUCHE}"),
        ouvrages if ouvrages is not None else [_ouvrage()],
    )
    ecrire_collection(
        str(tmp_path / f"{couche_conteneur}{EXTENSION_COUCHE}"),
        [
            {
                "type": "Feature",
    ...
    ecrire_collection(
        str(tmp_path / f"RPD_GeometrieSupplementaire_Reco{EXTENSION_COUCHE}"),
...
class TestEstAControler:
    ...
    def test_meme_perimetre_de_statut_qu_e0606(self) -> None:
        from recostar.controle.conteneur.e6204 import STATUTS_CONTROLES as STATUTS_E606
...
class TestParcourirOuvrages:
    ...
    def test_les_deux_couches_parcourues(self, tmp_path: Any) -> None:
        ...
        ecrire_collection(str(tmp_path / f"{AUTRE_COUCHE_CIBLE}{EXTENSION_COUCHE}"), [_ouvrage("o2")])
...
class TestConstruireGeojsonEcarts:
    ...
    def test_socle_commun(self) -> None:
        ...
        assert props["code_controle"] == "E-6109"
...
class TestCli:
    ...
    def test_crs_propage(self, tmp_path: Any) -> None:
        _ecrire_jeu(tmp_path, href_geomsupp=None)
        ecrire_collection_avec_crs(str(tmp_path / f"{COUCHE_CIBLE}{EXTENSION_COUCHE}"), [_ouvrage()], "EPSG:2154")
        executer_controle_cli(str(tmp_path))
...
# --------------------------------------------------------------------------- #
# Coherence avec E-6105 et E-6204
# --------------------------------------------------------------------------- #
...
class TestCoherenceAvecLesControlesVoisins:
    ...
    def test_discriminant_de_geometrie_propre_partage(self) -> None:
        from recostar.controle.conteneur.e6109 import possede_geometrie_propre as propre_e0607
        from recostar.controle.conteneur.e6204 import possede_geometrie_propre as propre_e0606

        assert propre_e0606 is propre_e0607

    def test_chaine_geomsupp_partagee_avec_e0605(self) -> None:
        """Les trois motifs de rupture de chaine sont ceux de la chaine commune."""
        from recostar.controle.conteneur.detection_localisation_noeud import classifier_chaine_conteneur as chaine_e0605
        from recostar.controle.conteneur.e6109 import classifier_chaine_conteneur as chaine_e0607

        assert chaine_e0605 is chaine_e0607

    def test_indexation_partagee_avec_e0606(self) -> None:
        from recostar.controle.conteneur.e6109 import indexer_conteneurs_autorises as index_e0607
        from recostar.controle.conteneur.e6204 import indexer_conteneurs_autorises as index_e0606

        assert index_e0606 is index_e0607

    def test_repli_de_geometrie_partage(self) -> None:
        """Les trois controles appliquent la meme regle de repli sur le conteneur."""
        from recostar.controle.conteneur.detection_localisation_noeud import geometrie_ecart as repli_e0605
        from recostar.controle.conteneur.e6109 import geometrie_ecart as repli_e0607

        assert repli_e0605 is repli_e0607

    def test_couches_cibles_disjointes_d_e0605(self) -> None:
        """E-6105 et E-6109 ne controlent jamais la meme entite."""
        from recostar.controle.conteneur.detection_localisation_noeud import COUCHES_CIBLES as CIBLES_E605
