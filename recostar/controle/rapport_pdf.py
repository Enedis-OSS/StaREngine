...
from metadonnees_jeu import champs_affichables, lire_metadonnees
...
# Table declarative plutot qu'une cascade de if sur PRIORITE_BLOQUANT : ajouter
# ou recolorer une priorite se fait ici seulement, les fonctions de style
# n'ayant pas a connaitre l'echelle.
...
def _commandes_priorites_ligne(
    ...
    bloquante : une famille ne presentant que des ecarts majeurs doit voir son
...
def _style_tableau_detail(famille: ResultatFamille) -> TableStyle:
    ...
    for indice, controle in enumerate(famille.controles, start=1):
        if not controle.succes:
            ...
            # un comptage, ce qui la distingue d'une priorite majeure.
...
def _page_synthese(
    ...
    elements.append(
        Paragraph(
            "Seules les anomalies de priorité « Bloquante » déclassent le statut. Les priorités "
            "« Majeure », « Mineure » et « Information » sont comptabilisées et détaillées : elles "
            "signalent un écart à corriger sans invalider la conformité. Le statut « Incomplet » "
            "signale qu'aucune anomalie bloquante n'a été détectée mais qu'au moins un contrôle n'a "
