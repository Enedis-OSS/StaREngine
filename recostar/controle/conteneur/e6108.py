"""
Controle E-6108 : types de noeuds autorises a se rattacher a un coffret.
...
noeuds pour qualifier les coffrets, comme E-7102 parcourt les jonctions pour
...
Meme parti qu'E-6211, qui confronte les points de leve a toutes les
autres couches.
...
donc pas signale — a la difference d'E-7102, dont la regle exige explicitement
la presence du lien.

Regle de gestion : une anomalie est emise **par lien fautif** (coffret, noeud),
convention des controles de relation du projet (E-9500, E-6103, E-9502). Un coffret
...
Usage CLI :
    python -m recostar.controle.conteneur.e6108 --repertoire <chemin> [--sortie <chemin>]

Sortie : ecarts_e6108_coffret_noeud_non_autorise.geojson
"""
...
FICHIER_SORTIE: str = "ecarts_e6108_coffret_noeud_non_autorise.geojson"

# Identite du controle, utilisee pour normaliser les proprietes des ecarts.
CODE_CONTROLE: str = "E-6108"
...
# Couches de noeuds autorisees a referencer un coffret. La liste releve du
# modele, non de ce controle : E-6114 la lit aussi, pour signaler ces memes
# couches lorsqu'elles visent un coffret de telecommunication.
COUCHES_NOEUDS_AUTORISEES: frozenset[str] = COUCHES_NOEUDS_COFFRET
...
def charger_coffrets_a_controler(
    ...
    geometrie de repli. Meme parti que l'index d'E-9502.
    """
...
def main() -> None:
    ...
    parseur = argparse.ArgumentParser(
        description=(
            "Controle E-6108 : les RPD_Coffret_Reco au statut UnderCommissionning "
