"""
Tests unitaires du controle de conformite 3D (controle_e200).
...
def repertoire_test(tmp_path: Any) -> str:
    ...
    features_3d = [
        construire_feature("pt-3d", "Point", [1.0, 2.0, 3.0]),
        construire_feature("ls-3d", "LineString", [[0, 0, 1], [1, 1, 2]]),
    ]
    ecrire_collection(str(tmp_path / "conforme.geojson"), features_3d)

    features_2d = [
        construire_feature("pt-2d", "Point", [1.0, 2.0]),
        construire_feature("ls-2d", "LineString", [[0, 0], [1, 1]]),
    ]
    ecrire_collection(str(tmp_path / "non_conforme.geojson"), features_2d)

    ecrire_collection(str(tmp_path / "ecarts_ancien.geojson"), [])
...
class TestCli:
    ...
    def test_aucune_anomalie_produit_collection_vide(self, tmp_path: Any) -> None:
        features_3d = [construire_feature("ok", "Point", [1.0, 2.0, 3.0])]
        ecrire_collection(str(tmp_path / "data.geojson"), features_3d)
        resultat = executer_controle_cli(str(tmp_path))
        assert resultat["succes"] is True
