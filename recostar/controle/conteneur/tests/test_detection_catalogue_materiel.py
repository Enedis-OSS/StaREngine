"""
Tests du moteur de conformite du materiel de jonction au catalogue.
...
from recostar.controle.conteneur.detection_catalogue_materiel import (
    STATUT_CONTROLE,
    TYPE_CASSE_INCORRECTE,
...
from recostar.controle.conteneur.e7104 import PROFIL_ECARTS as _PROFIL_7104
from recostar.controle.conteneur.e7104 import TYPES_RETENUS as _TYPES_7104
from recostar.controle.conteneur.e9600 import PROFIL_ECARTS as _PROFIL_9600
from recostar.controle.conteneur.e9600 import TYPES_RETENUS as _TYPES_9600
...
_PROFIL_PAR_TYPE = {
    **dict.fromkeys(_TYPES_7104, _PROFIL_7104),
    **dict.fromkeys(_TYPES_9600, _PROFIL_9600),
}

_PROFIL_DEFAUT = _PROFIL_7104
...
def _catalogue_mock() -> CatalogueMateriel:
    ...
    return CatalogueMateriel(
        ...
        # Ecriture de reference du catalogue : c'est a elle qu'E-7305 confronte
        # la casse du couple porte par le materiel.
        ecriture_par_entree={
            ("hta", "3m", "modele-a"): ("3M", "Modele-A"),
            ("hta", "3m", "modele-b"): ("3M", "Modele-B"),
            ("hta", "maec", "modele-a"): ("MAEC", "Modele-A"),
            ("bt", "3m", "modele-c"): ("3M", "Modele-C"),
        },
    )
...
class TestChargerCatalogue:
    ...
    def test_catalogue_reel_du_depot(self) -> None:
        ...
        from recostar.controle.conteneur.detection_catalogue_materiel import CHEMIN_CATALOGUE
...
class TestClassifierMateriel:
    ...
    def test_espaces_ignores(self) -> None:
        """Le repliement des espaces ne fait pas anomalie : E-7305 ne vise que la casse."""
        assert classifier_materiel("HTA", " 3M ", "Modele-A\n", _catalogue_mock()) == []

    def test_casse_du_domaine_ignoree(self) -> None:
        """Le DomaineTension n'entre pas dans le couple que qualifie E-7305."""
        assert classifier_materiel(" hta ", "3M", "Modele-A", _catalogue_mock()) == []

    def test_casse_du_couple_signalee(self) -> None:
        """Le couple est reconnu, mais pas ecrit comme au catalogue : E-7305."""
        assert classifier_materiel("HTA", "3m", "Modele-A", _catalogue_mock()) == [TYPE_CASSE_INCORRECTE]

    def test_casse_du_modele_signalee(self) -> None:
        assert classifier_materiel("HTA", "3M", "MODELE-A", _catalogue_mock()) == [TYPE_CASSE_INCORRECTE]

    def test_une_seule_anomalie_pour_les_deux_champs(self) -> None:
        """Une anomalie par couple, non par champ fautif."""
        assert classifier_materiel("HTA", "3m", "MODELE-A", _catalogue_mock()) == [TYPE_CASSE_INCORRECTE]

    def test_casse_incorrecte_exclusive_du_couple_non_reference(self) -> None:
        """Un couple inconnu releve d'E-7104 ; la casse ne se pose que s'il est reconnu."""
        codes = classifier_materiel("HTA", "MAEC", "Modele-B", _catalogue_mock())
        assert TYPE_CASSE_INCORRECTE not in codes
...
class TestConstruireGeojsonEcarts:
    ...
    def test_socle_commun(self) -> None:
        ...
        # Type rattache a E-7104 : la priorite est derivee du code, pas du repli.
        assert props["code_erreur"] == "E-7104"
    ...
    def test_geometrie_de_la_jonction_conservee(self) -> None:
        geom = construire_geojson_ecarts([self._anomalie()], _profil([self._anomalie()]))["features"][0]["geometry"]
...
class TestCli:
    ...
    @patch("recostar.controle.conteneur.detection_catalogue_materiel.charger_catalogue")
