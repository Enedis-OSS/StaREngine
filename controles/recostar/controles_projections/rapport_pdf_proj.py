"""
Generation du rapport PDF de synthese des controles de projection.

Produit un document PDF recapitulatif des resultats de l'ensemble des
controles de projection (conformite CRS, coherence ensemble, coordonnees).
...
Sortie : rapport_controles_proj.pdf
"""
...
FICHIER_RAPPORT: str = "rapport_controles_proj.pdf"
...
_LABEL_FICHIER_SOURCE: str = "Fichier source"
_LABEL_CRS_DETECTE: str = "CRS detecte"
_LABEL_TYPE_ANOMALIE: str = "Type anomalie"
_LABEL_PRIORITE: str = "Priorite"

# Configuration des colonnes de detail par controle
# Chaque entree : (cle_propriete, en_tete_colonne, largeur_cm)
_COLONNES_PROJ: tuple[tuple[str, str, float], ...] = (
    ("id_entite", _LABEL_ID_ENTITE, 4.0),
    ("fichier_source", _LABEL_FICHIER_SOURCE, 5.0),
    ("crs_detecte", _LABEL_CRS_DETECTE, 3.0),
    ("type_anomalie", _LABEL_TYPE_ANOMALIE, 3.0),
    ("priorite", _LABEL_PRIORITE, 2.0),
)

_COLONNES_ENSEMBLE: tuple[tuple[str, str, float], ...] = (
    ("id_entite", _LABEL_ID_ENTITE, 3.5),
    ("fichier_source", _LABEL_FICHIER_SOURCE, 5.0),
    ("crs_detecte", _LABEL_CRS_DETECTE, 3.0),
    ("crs_reference", "CRS reference", 3.0),
    ("priorite", _LABEL_PRIORITE, 2.0),
)

_COLONNES_COORDONNEES: tuple[tuple[str, str, float], ...] = (
    ("id_entite", _LABEL_ID_ENTITE, 3.5),
    ("fichier_source", _LABEL_FICHIER_SOURCE, 4.0),
    ("type_geometrie", "Type geometrie", 2.5),
...
CONTROLES: tuple[tuple[str, str, str, tuple[tuple[str, str, float], ...]], ...] = (
    (
        "ecarts_proj.geojson",
        "Conformite CRS",
        "Entites dont la projection n'appartient pas a la liste autorisee",
        _COLONNES_PROJ,
    ),
    (
        "ecart_proj_ensemble.geojson",
        "Coherence ensemble",
        "Entites dont le CRS differe du CRS de reference du lot",
        _COLONNES_ENSEMBLE,
    ),
    (
        "ecarts_proj_coordonnees.geojson",
        "Coherence coordonnees",
        "Sommets dont les coordonnees sont incoherentes avec le CRS declare",
        _COLONNES_COORDONNEES,
    ),
)
...
def construire_sections_detail(
    ...
    for resultat in resultats:
        ...
        elements.append(
            Paragraph(
                f"{label} ({nb} anomalie{'s' if nb > 1 else ''})",
                styles["sous_titre"],
            )
        )
...
def generer_rapport_pdf(
    ...
    """Genere le rapport PDF de synthese des controles de projection.
    ...
    # Titre
    elements.append(Paragraph("Rapport des controles de projection", styles["titre"]))
    elements.append(Spacer(1, 6 * mm))
...
def main() -> None:
    ...
    parseur = argparse.ArgumentParser(
        description="Generation du rapport PDF des controles de projection"
    )
