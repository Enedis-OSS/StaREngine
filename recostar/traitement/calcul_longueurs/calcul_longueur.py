...
def _resoudre_chemin_referentiel(chemin_explicite: str | None = None) -> str:
    ...
    if chemin_explicite is not None:
        return str(Path(chemin_explicite).resolve())
...
def construire_index_statuts(
    ...
    cable_types = referentiel.get("objets", {}).get("Câble", {}).get("types", {}).get(TYPE_STATUT_REFERENTIEL, {})
...
# controles E-5100, E-4200 et E-4201 doivent mesurer la longueur d'un cable
# exactement comme
...
def analyser_cable(
    ...
    if not est_aerien:
        ...
        corrections = calculer_corrections_cable(premier_sommet, dernier_sommet, entites_liees, domaine_tension)
    ...
    longueur_electrique = (
        longueur_geo + corrections.correction_depart + corrections.correction_arrivee + correction_aerien
    )
...
def executer_calcul(
    ...
    if chemin_recolement is not None:
        chemin_recolement = str(Path(chemin_recolement).resolve())
    if chemin_referentiel is not None:
        chemin_referentiel = str(Path(chemin_referentiel).resolve())

    recolement = chemin_recolement if chemin_recolement is not None else obtenir_chemin_recolement(chemin_projet)
...
def main() -> None:
    ...
    parseur = argparse.ArgumentParser(description="Calcul des longueurs geographiques et electriques des cables")
    parseur.add_argument("--chemin-projet", required=True, help="Chemin du projet RecoStaR")
    arguments = parseur.parse_args()
    chemin_projet = str(Path(arguments.chemin_projet).resolve())

    resultats = executer_calcul(chemin_projet)

    if resultats["succes"]:
        chemin_sortie = _ecrire_fichier_sortie(chemin_projet, resultats)
