"""
Tests du controle E-6205 : profondeur manquante aux points de charge generatrice.
...
from recostar.controle.cheminement.e6205 import (
...
class TestChargerCheminementsSouterrains:
    ...
    def test_cheminements_de_toutes_les_voies_souterraines_fusionnes(self, tmp_path):
        ...
        assert len(cheminements) == len(FICHIERS_CHEMINEMENT_SOUTERRAIN)
        assert absents == []
...
class TestConstruireGeojsonEcarts:
    ...
    def test_priorite_derivee_du_code_erreur(self):
        """Lot 2 : la priorite est le niveau du code E-6205, moyenne — non declassante."""
        anomalie = {
            "id_point": "id-p1",
            "coordonnees": [1.0, 2.0],
            "cheminements_touches": [{"id_cheminement": "id-c1", "fichier": "RPD_Fourreau_Reco.geojson"}],
        }
        props = construire_geojson_ecarts([anomalie], "1.1")["features"][0]["properties"]
        assert props["code_erreur"] == "E-6205"
        assert props["priorite"] == PRIORITE_MOYENNE
...
class TestCli:
    ...
    def test_fichier_source_absent_sans_objet(self, tmp_path):
        resultat = executer_controle_cli(str(tmp_path))
        assert resultat["succes"] is True
        assert FICHIER_SOURCE in resultat["motif"]
