"""
Controle E-3304 : cables electriques HTB situes dans l'emprise DR.
...
Regle de gestion :
  ...
  - Toute entite retenue situee DANS l'emprise DR genere une anomalie E-3304.
    Le sens est donc l'inverse d'E-5106, qui signale les entites HORS emprise.
...
Perimetre :
  - Certains numeros d'affaire excluent entierement le controle (12345678 et
    ...
    ignores sans verification ni anomalie. Meme regle metier que E-5106.
...
`fonctions_communes.emprise_dr` (module commun avec E-5106). Si le fichier
cable est dans un autre CRS projete, ses coordonnees sont reprojetees vers
EPSG:2154 via pyproj avant le test.

Usage CLI :
    python -m recostar.controle.cable.e3304 --repertoire <chemin> --numero_affaire <numero>
                                            [--sortie <chemin>]

Sortie : ecarts_e3304_cable_htb_emprise_dr.geojson
"""
...
FICHIER_SORTIE: str = "ecarts_e3304_cable_htb_emprise_dr.geojson"

# Identite du controle, utilisee pour normaliser les proprietes des ecarts.
CODE_CONTROLE: str = "E-3304"
...
def executer_controle_cli(
    ...
    # controle d'emprise (regle partagee avec E-5106, portee par emprise_dr).
    ...
    if affaire_exclue_du_controle(numero_affaire):
        return rapport_sans_objet(
            "numero d'affaire exclu du controle E-3304",
...
def main() -> None:
    ...
    parseur = argparse.ArgumentParser(
        description=(
            "Controle E-3304 : cables HTB dans l'emprise DR — tout "
