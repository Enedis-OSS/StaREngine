...
qui lui est fourni. Le code du contrôle suit la version contrôlée — **E0110** en
V1.1, **E0010** en V1.0 (point d'entrée dédié `e0010.py`).

Entrée  : Fichier GML RecoStaR à contrôler
Sortie  : Fichier JSON listant les erreurs d'ordre détectées

Usage :
    python -m recostar.controle.xsd_structuration.e0110 <fichier.gml> [--output-dir <repertoire>] \
...
from recostar.controle.xsd_structuration.codes_controle import RANG_ORDRE, identite_controle
from recostar.controle.xsd_structuration.priorites_structuration import statut_conformite, ventiler_par_priorite
from recostar.controle.xsd_structuration.sequenceur_xsd import ErreurOrdre, valider_sequence
...
class AnalyseurGML:
    ...
    def __init__(self, chemin_gml: Path, profil: ProfilVersion | None = None) -> None:
        ...
        # Profil par défaut : V1.1, quand l'appelant n'indique pas de version.
...
def _construire_rapport(
    ...
    # Ventilation par priorité : E0110 n'accorde aucune dérogation, toutes ses
    ...
    return {
        ...
        "niveau": "Forte",
        # Le code du contrôle dépend de la version : E0110 en V1.1, E0010 en V1.0.
        "type_controle": identite_controle(version, RANG_ORDRE).type_controle,
        "version_controlee": version,
        "conformite": conformite,
        "nb_erreurs": nb_erreurs,
        # E0110 ne produit que des erreurs : la ventilation par sévérité est
        # donc soit vide, soit réduite à la clé ERREUR. Format aligné sur E0114.
...
def _resoudre_chemin_sortie(
    ...
    Le suffixe porte le code du contrôle appliqué : `_controle_e0110.json` en
    V1.1, `_controle_e0010.json` en V1.0.
    """
...
DESCRIPTION_CLI: str = (
    "Contrôle de l'ordre de structure des objets RPD dans un fichier GML RecoStaR (E0110 en V1.1, E0010 en V1.0)."
)
