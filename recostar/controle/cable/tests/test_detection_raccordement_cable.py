"""
Tests du moteur de raccordement des cables aux noeuds du reseau.
...
from recostar.controle.cable.detection_raccordement_cable import (
    STATUT_CONTROLE,
...
from recostar.controle.cable.e6110 import PROFIL_ECARTS as _PROFIL_6110
from recostar.controle.cable.e6110 import TYPES_RETENUS as _TYPES_6110
from recostar.controle.cable.e6111 import PROFIL_ECARTS as _PROFIL_6111
from recostar.controle.cable.e6111 import TYPES_RETENUS as _TYPES_6111
from recostar.controle.cable.e9501 import PROFIL_ECARTS as _PROFIL_9501
from recostar.controle.cable.e9501 import TYPES_RETENUS as _TYPES_9501
from recostar.controle.cable.tests.utils_tests import construire_feature_noeud, ecrire_collection
from recostar.controle.fonctions_communes.geojson import PRIORITE_FORTE, PRIORITE_MOYENNE
from recostar.controle.fonctions_communes.geometrie import extraire_extremites
from recostar.controle.fonctions_communes.modele_recostar import (
    FICHIER_CABLE_ELECTRIQUE,
    FICHIER_CABLE_TERRE,
    FICHIER_TERRE,
)
from recostar.controle.fonctions_communes.references_cables import charger_types_noeuds_reseau
...
_PROFIL_PAR_TYPE = {
    **dict.fromkeys(_TYPES_6110, _PROFIL_6110),
    **dict.fromkeys(_TYPES_6111, _PROFIL_6111),
    **dict.fromkeys(_TYPES_9501, _PROFIL_9501),
}

_PROFIL_DEFAUT = _PROFIL_6110
...
class TestConstruireGeojsonEcarts:
    ...
    def test_priorite_par_regle(self) -> None:
        """Chaque regle porte son code, dans la sortie du controle qui la detient.

        Les deux anomalies cohabitaient dans un meme fichier tant qu'un controle
        unique les emettait ; depuis la scission, chacune a le sien.
        """
        proprietes = [
            construire_geojson_ecarts(
        ...
        # Les deux priorites sont derivees de leur code : E-6110 pour le cable
        # sans noeud, le code local E-9501 pour le cable de terre.
        assert [p["code_erreur"] for p in proprietes] == ["E-6110", "E-9501"]
        assert [p["priorite"] for p in proprietes] == [PRIORITE_FORTE, PRIORITE_MOYENNE]
    ...
    def test_champs_specifiques_omis_si_absents(self) -> None:
        ...
        props = construire_geojson_ecarts(anomalies, _profil(anomalies))["features"][0]["properties"]
    ...
    def test_geometrie_du_cable_conservee(self) -> None:
        ...
        assert construire_geojson_ecarts(anomalies, _profil(anomalies))["features"][0]["geometry"] == geometrie
...
class TestExecuterControleCli:
    ...
    def test_repertoire_introuvable(self, tmp_path: Any) -> None:
        resultat = _executer(os.path.join(tmp_path, "absent"))
    ...
    def test_fichier_sortie_ecrit(self, tmp_path: Any) -> None:
        ...
        assert resultat["sortie"].endswith(_FICHIER_TEST)
    ...
    def test_repertoire_sortie_distinct(self, tmp_path: Any) -> None:
        ...
        resultat = _executer(str(entree), str(sortie))
        assert os.path.isfile(os.path.join(sortie, _FICHIER_TEST))
        assert resultat["succes"] is True

    def test_priorites_exposees(self, tmp_path: Any) -> None:
        """Le rapport annonce la priorite effective de chaque type d'anomalie.

        Les quatre types la tiennent de leur code d'erreur, le cable de terre
        depuis qu'il porte le code local E-9501.
        """
        # Chaque controle n'expose que les priorites de ses propres types :
        # le moteur est partage, l'identite ne l'est pas.
        priorites_6110 = executer_analyse(str(tmp_path), _TYPES_6110, _PROFIL_6110, _FICHIER_TEST)["priorites"]
        assert priorites_6110[TYPE_SANS_NOEUD] == PRIORITE_FORTE

        priorites_6111 = executer_analyse(str(tmp_path), _TYPES_6111, _PROFIL_6111, _FICHIER_TEST)["priorites"]
        assert priorites_6111[TYPE_EXTREMITE_NON_RACCORDEE] == PRIORITE_MOYENNE

        priorites_9501 = executer_analyse(str(tmp_path), _TYPES_9501, _PROFIL_9501, _FICHIER_TEST)["priorites"]
        assert priorites_9501[TYPE_CABLE_TERRE_NON_RACCORDE] == PRIORITE_MOYENNE
...
class TestCompatibiliteVersions:
    ...
    def test_cables_href_separe_par_espaces(self, tmp_path: Any) -> None:
        """La convention d'espacement (E-5201) est acceptee comme la virgule."""
        ecrire_collection(
