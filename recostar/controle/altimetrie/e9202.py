"""
Controle E-9202 : coherence points de leve / geometries supplementaires de supports.
...
strictement comme E-9201 (et non uniquement sur les sommets, ce qui distingue
E-9202 d'E-6210).

Perimetre :
- E-9202 ne s'applique qu'en version RecoStaR 1.1. En version 1.0, le controle est
...
v1.1), par le mecanisme commun `fonctions_communes.version_recostar`. Elle peut
etre imposee via l'option --version.

La detection spatiale est portee par `fonctions_communes.points_leve` :
- charger_points_leve : chargement des points de leve en geometries 2D ;
...
- extraire_hrefs_geomsupp_liees : filtrage v1.1 par Statut, identique
...
Usage CLI :
    python -m recostar.controle.altimetrie.e9202 --repertoire <chemin> [--sortie <chemin>]
                                                 [--version {auto,1.0,1.1}]

Sortie : ecarts_e9202_point_leve_geom_supp_support.geojson
"""
...
# Moteur de detection spatiale commun aux controles de points de leve
...
FICHIER_SORTIE: str = "ecarts_e9202_point_leve_geom_supp_support.geojson"

# Identite du controle, utilisee pour normaliser les proprietes des ecarts.
# Le moteur est commun ; le code et la description sont propres a ce controle.
CODE_CONTROLE: str = "E-9202"
...
# Version RecoStaR pour laquelle le controle est actif (desactive en 1.0)
...
def extraire_hrefs_geomsupp_liees_supports(
    ...
    (memes champs geometriesupplementaire_href et Statut) : le moteur commun
    est reutilise directement. Retourne un frozenset pour le lookup O(1).
    """
    return extraire_hrefs_geomsupp_liees(features_supports, VERSION_APPLICABLE)
...
def _rapport_non_applicable(
    ...
    En version 1.0, E-9202 n'est pas applicable : aucune anomalie n'est possible,
    donc aucun fichier d'ecarts n'est produit. Le rapport suit la convention
    commune du controle sans element a controler (`sans_objet` et son motif),
    seule forme que la synthese et le rapport PDF savent restituer ; `applicable`
    reste porte en complement, le rapport nominal le renseignant aussi. Un
    eventuel fichier d'une execution precedente est supprime pour eviter de
    signaler des ecarts obsoletes.
    """
    ecrire_geojson_si_anomalies(construire_geojson_ecarts([], version_effective, PROFIL_ECARTS, crs), chemin_sortie)
    return rapport_sans_objet(
        f"Controle applicable a la seule version RecoStaR {VERSION_APPLICABLE} : jeu detecte en version {version_effective}",
...
def executer_controle_cli(
    ...
    """Execute le controle E-9202 en mode CLI.
    ...
    # Le controle est desactive en version 1.0 : sortie vide, aucune anomalie.
    ...
    if collection_support is None:
        return rapport_sans_objet(motif_couche_absente(FICHIER_SUPPORT, repertoire_resolu))
...
def main() -> None:
    """Point d'entree CLI du controle E-9202."""
    choix_version = (JETON_AUTO,) + VERSIONS_SUPPORTEES
    parseur = argparse.ArgumentParser(
        description=(
            "Controle E-9202 : detection des geometries supplementaires de "
    ...
    parseur.add_argument(
        ...
        help=(
            ...
            "'1.0' ou '1.1'. E-9202 est desactive en 1.0."
        ),
    )
