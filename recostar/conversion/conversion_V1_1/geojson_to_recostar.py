...
class MappeurEntites:
    ...
    def _qualifier_geometrie_supp(self, geometry: dict) -> str | None:
...
class GenerateurGML:
    ...
    def definir_metadonnees(
        ...
        Les valeurs CLI sont prioritaires. Si _metadata.json a été chargé,
        ses valeurs servent de fallback pour les champs non renseignés.
        """
        # Fallback depuis _metadata.json chargé
        meta_chargees = self._metadata_chargees.get("Metadata", {})
        reseau_charge = self._metadata_chargees.get("ReseauUtilite", {})

        self.metadata = {
            "logiciel": logiciel or meta_chargees.get("Logiciel", ""),
            "producteur": producteur or meta_chargees.get("Producteur", ""),
            "responsable": responsable or meta_chargees.get("Responsable", ""),
            "nom": nom or reseau_charge.get("Nom", ""),
            ...
            "reseau_id": reseau_charge.get("id", "Reseau"),
            "reseau_mention": reseau_charge.get("Mention", ""),
            "reseau_theme": reseau_charge.get("Theme", "ELECTRD"),
        }
        self.srs = srs
        self.mapper.srs = srs
    ...
    def _creer_membre_reseau(self) -> ET.Element:
        """Crée l'élément ReseauUtilite (conforme au XSD ReseauUtiliteType)"""
        member = ET.Element(f"{{{NAMESPACE_GML}}}featureMember")

        reseau_id = self.metadata.get("reseau_id", "Reseau")
        reseau = ET.SubElement(member, f"{{{NAMESPACE_RECOSTAR}}}ReseauUtilite")
        reseau.set(f"{{{NAMESPACE_GML}}}id", reseau_id)

        # Mention (conservée depuis le GML source si disponible)
        mention = ET.SubElement(reseau, f"{{{NAMESPACE_RECOSTAR}}}Mention")
        mention_text = self.metadata.get("reseau_mention", "")
        mention.text = mention_text or (
        ...
        theme_elem.text = self.metadata.get("reseau_theme", "ELECTRD")
    ...
    def _extraire_noeud_avec_etat(self, features: list) -> list:
        ...
        for feat in features:
            ...
            if not (feat_id and cable_href):
                continue
            cables = self._analyser_ids_cable(cable_href)
...
def main():
    ...
    parser.add_argument("--logiciel", default="LAZio", help="Logiciel utilisé pour la génération")

    parser.add_argument("--producteur", default="TEST", help="Producteur du récolement")

    parser.add_argument("--responsable", default="TEST", help="Responsable du récolement")
