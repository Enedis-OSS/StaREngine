"""
Tests unitaires du moteur de discretisation des courbes des cables.
...
from recostar.controle.cable.detection_discretisation import (
    SEUIL_ANGLE,
    SEUIL_CORDE_LONGUE,
    SEUIL_FLECHE_FORTE,
...
from recostar.controle.cable.e5101 import DESCRIPTIONS_ANOMALIES as DESCRIPTIONS_5101
from recostar.controle.cable.e5101 import PROFIL_ECARTS as _PROFIL_5101
from recostar.controle.cable.e5101 import TYPES_RETENUS as TYPES_RETENUS_5101
from recostar.controle.cable.e5101 import TYPES_RETENUS as _TYPES_5101
from recostar.controle.cable.e5200 import DESCRIPTIONS_ANOMALIES as DESCRIPTIONS_5200
from recostar.controle.cable.e5200 import PROFIL_ECARTS as _PROFIL_5200
from recostar.controle.cable.e5200 import TYPES_RETENUS as TYPES_RETENUS_5200
from recostar.controle.cable.e5200 import TYPES_RETENUS as _TYPES_5200
...
_PROFIL_PAR_TYPE = {
    **dict.fromkeys(_TYPES_5101, _PROFIL_5101),
    **dict.fromkeys(_TYPES_5200, _PROFIL_5200),
}

_PROFIL_DEFAUT = _PROFIL_5101
...
# decrite ; une mesure naive y verrait pourtant une fleche de 1,73 m, extrapolee
# depuis
...
class TestSeuilsDefaut:
    ...
    def test_valeurs_metier(self) -> None:
        ...
        assert (SEUIL_CORDE_LONGUE, SEUIL_ANGLE) == (1.00, 3.0)
    ...
    def test_seuils_defaut_alignes_sur_les_constantes(self) -> None:
        assert SEUILS_DEFAUT == SeuilsDiscretisation(
            ...
            corde_longue=SEUIL_CORDE_LONGUE,
        )


class TestClasserSommet:
    """Tests du classement d'un sommet par ses deux fleches et leurs cordes.
    ...
    La fleche croissant avec la corde a rayon fixe, `corde_fleche_max` est
    toujours la plus longue des deux.

    La regle transcrite est celle du verificateur RecoStaR
    (RECOSTAR_ANALYSE_GML_CTRL_GEO).
    """

    def test_les_deux_fleches_negligeables_conforme(self) -> None:
        assert classer_sommet(0.09, 0.02, 4.0) is None

    def test_les_deux_fleches_significatives_non_conforme(self) -> None:
        """Arc uniformement sous-decrit : les deux cordes ratent l'arc.

        Aucune tolerance de corde ne s'applique : elle ne vaut que pour un ecart
        isole.
        """
        assert classer_sommet(0.16, 0.11, 1.4) == TYPE_COURBE_MAL_DISCRETISEE

    def test_deux_fleches_significatives_au_seuil_exact(self) -> None:
        seuil = SEUIL_FLECHE_NEGLIGEABLE
        assert classer_sommet(seuil, seuil, 3.0) == TYPE_COURBE_MAL_DISCRETISEE

    def test_ecart_isole_sur_corde_courte_non_conforme(self) -> None:
        """Un ecart de plus de 10 cm porte par une corde de moins d'un metre.

        C'est le virage serre rendu par trop peu de points : la corde est courte,
        pourtant elle s'ecarte notablement de l'arc.
        """
        assert classer_sommet(0.85, 0.004, 0.9) == TYPE_COURBE_MAL_DISCRETISEE

    def test_ecart_isole_sur_corde_longue_conforme(self) -> None:
        """Meme ecart isole, porte par une corde de plus d'un metre : conforme.

        Cas reel du jeu Echantillon2 : une corde de 11,9 m sur un rayon de 34 m
        s'ecarte de 53 cm, sans que le cable soit mal place.
        """
        assert classer_sommet(0.531, 0.0004, 11.9) is None

    def test_corde_au_seuil_exact_reste_signalee(self) -> None:
        """Le seuil est exclusif : la corde doit **depasser** un metre pour tolerer."""
        assert classer_sommet(0.85, 0.004, SEUIL_CORDE_LONGUE) == TYPE_COURBE_MAL_DISCRETISEE

    def test_corde_juste_au_dessus_du_seuil_toleree(self) -> None:
        assert classer_sommet(0.85, 0.004, SEUIL_CORDE_LONGUE + 0.01) is None

    def test_les_deux_fleches_bloquantes(self) -> None:
        """Cas extreme : les deux cordes ratent l'arc de plus de 40 cm."""
        assert classer_sommet(0.90, 0.45, 6.0) == TYPE_COURBE_NON_DISCRETISEE

    def test_cas_extreme_prime_sur_la_tolerance_de_corde(self) -> None:
        """Une corde longue n'attenue pas le cas extreme : l'arc n'est decrit nulle part."""
        assert classer_sommet(0.90, 0.45, 500.0) == TYPE_COURBE_NON_DISCRETISEE

    def test_seuils_personnalises(self) -> None:
        seuils = SeuilsDiscretisation(fleche_negligeable=0.30)
        assert classer_sommet(0.20, 0.15, 4.0, seuils) is None


class TestConformiteAuVerificateur:
    """Ancrage sur les cas publies par le verificateur RecoStaR.

    Valeurs relevees dans `rapport_anomalies` du jeu PLAISANCEv3, ou le
    verificateur signale dix sommets en E-5200 et aucun en E-5101. Elles ont
    servi a diagnostiquer la divergence de regle : star-engine les tolerait
    toutes, la corde examinee n'etant pas la bonne.

    Chaque tuple vaut (fleche_max, fleche_min, corde_fleche_max, corde_fleche_min).
    Les cordes portant la fleche significative valent 0,61 a 0,93 m — toutes en
    deca du metre, d'ou le signalement.
    """

    # Les dix sommets signales, dans l'ordre du rapport d'anomalies.
    _SIGNALES = (
        (0.13488, 0.02071, 0.882, 0.359),
        (0.10997, 0.03703, 0.898, 0.531),
        (0.12227, 0.02738, 0.713, 0.353),
        (0.12867, 0.01856, 0.879, 0.346),
        (0.10052, 0.03747, 0.699, 0.437),
        (0.17812, 0.02706, 0.883, 0.367),
        (0.10348, 0.01639, 0.678, 0.280),
        (0.10462, 0.03071, 0.612, 0.345),
        (0.10467, 0.03201, 0.931, 0.524),
        (0.14776, 0.02226, 0.918, 0.372),
    )

    def test_les_dix_sommets_sont_signales(self) -> None:
        verdicts = [classer_sommet(*mesure[:3]) for mesure in self._SIGNALES]
        assert verdicts == [TYPE_COURBE_MAL_DISCRETISEE] * len(self._SIGNALES)

    def test_aucun_n_est_classe_en_cas_extreme(self) -> None:
        """Le verificateur ne signale aucun E-5101 sur ce jeu : les fleches
        restent tres en deca de 40 cm."""
        for mesure in self._SIGNALES:
            assert classer_sommet(*mesure[:3]) != TYPE_COURBE_NON_DISCRETISEE


class TestClasserSommetEchantillonnageDissymetrique:
    """Un sommet encadre par une micro-corde et une corde longue reste conforme.

    Ces cas produisaient des faux positifs avant l'introduction d'une tolerance
    de corde. La regle du verificateur les ecarte nativement : la fleche
    significative y est portee par la **longue** corde, et c'est cette corde-la
    que la tolerance examine. Les valeurs sont celles des cas reels du jeu
    Echantillon.
    """

    def test_micro_corde_entrante_ecartee(self) -> None:
        """Cable idb70c029c : cordes 0,08 m et 7,97 m, R = 5,46 m, fleche 1,73 m."""
        assert classer_sommet(1.726, 0.0, 7.97) is None

    def test_micro_corde_sortante_ecartee(self) -> None:
        """Cable idc2545906 : segment terminal de 1 cm contre une corde de 1,76 m."""
        assert classer_sommet(0.621, 0.0, 1.76) is None
    ...
    def test_corde_courte_juste_sous_le_seuil_ecartee(self) -> None:
        ...
        assert classer_sommet(0.590, 0.002, 8.28) is None

    def test_dissymetrie_sans_effet_si_les_deux_fleches_comptent(self) -> None:
        """La tolerance ne couvre que l'ecart isole, quelle que soit la dissymetrie."""
        assert classer_sommet(0.50, 0.15, 2.0) == TYPE_COURBE_MAL_DISCRETISEE
...
class TestCompterAnomaliesParType:
    ...
    def test_chaque_type_revient_a_un_controle(self) -> None:
        """Un type que nul controle ne retient serait detecte puis perdu.

        Le moteur ne decrit plus les anomalies lui-meme : chaque controle porte
        ses descriptions, et c'est leur reunion qui doit couvrir ce qu'il
        detecte.
        """
        assert TYPES_RETENUS_5101 | TYPES_RETENUS_5200 == {
            TYPE_COURBE_NON_DISCRETISEE,
            TYPE_COURBE_MAL_DISCRETISEE,
        }

    def test_types_disjoints(self) -> None:
        assert TYPES_RETENUS_5101 & TYPES_RETENUS_5200 == frozenset()

    def test_types_decrits_par_leur_controle(self) -> None:
        """Sans description, le fichier d'ecarts sortirait sans libelle exploitable."""
        assert set(DESCRIPTIONS_5101) == TYPES_RETENUS_5101
        assert set(DESCRIPTIONS_5200) == TYPES_RETENUS_5200
...
class TestConstruireGeojsonEcarts:
    ...
    def test_proprietes_anomalie(self) -> None:
        proprietes = construire_geojson_ecarts([self._anomalie()], _profil([self._anomalie()]))["features"][0][
            "properties"
        ]
        ...
        assert proprietes["code_erreur"] == "E-5200"
        assert proprietes["priorite"] == PRIORITE_MOYENNE

    def test_priorite_derivee_du_code_erreur(self) -> None:
        """Lot 2 : chaque type tient sa priorite du niveau de son code du verificateur.

        Les deux types se distinguent desormais par leur gravite : une courbe
        non discretisee est forte (E-5101), une courbe mal discretisee est
        moyenne (E-5200).
        """
        attendus = {
            TYPE_COURBE_NON_DISCRETISEE: (PRIORITE_FORTE, "E-5101"),
            TYPE_COURBE_MAL_DISCRETISEE: (PRIORITE_MOYENNE, "E-5200"),
        }
        for type_anomalie, (priorite, code_erreur) in attendus.items():
            anomalie = {**self._anomalie(), "type_anomalie": type_anomalie}
            proprietes = construire_geojson_ecarts([anomalie], _profil([anomalie]))["features"][0]["properties"]
            assert proprietes["code_erreur"] == code_erreur
            assert proprietes["priorite"] == priorite

    def test_geometrie_de_la_portion_conservee(self) -> None:
        resultat = construire_geojson_ecarts([self._anomalie()], _profil([self._anomalie()]))
    ...
    def test_crs_absent_non_ajoute(self) -> None:
        assert "crs" not in construire_geojson_ecarts([], _PROFIL_DEFAUT)
...
class TestExecuterControleCli:
    ...
    def test_repertoire_introuvable_retourne_erreur(self, tmp_path: Any) -> None:
        resultat = _executer(str(tmp_path / "absent"))
    ...
    def test_nominal_avec_anomalie(self, tmp_path: Any) -> None:
        ...
        assert resultat["anomalies_par_type"] == {TYPE_COURBE_NON_DISCRETISEE: 1}
    ...
    def test_fichier_ecarts_cree(self, tmp_path: Any) -> None:
        ...
        assert os.path.basename(resultat["sortie"]) == _FICHIER_TEST
    ...
    def test_sortie_dans_repertoire_dedie(self, tmp_path: Any) -> None:
        ...
        _executer(str(tmp_path), str(destination))
        assert os.path.isfile(str(destination / _FICHIER_TEST))
    ...
    def test_comportement_identique_v10_v11(self, tmp_path: Any) -> None:
        ...
        resultat_v10 = _executer(str(tmp_path))
        resultat_v11 = _executer(str(repertoire_v11))
