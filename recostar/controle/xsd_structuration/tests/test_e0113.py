"""
Tests unitaires du module e0113.
...
from recostar.controle.xsd_structuration.e0113 import (
    AnalyseurEntete,
    _construire_rapport,
...
from recostar.controle.xsd_structuration.sequenceur_xsd import ErreurOrdre
from recostar.controle.xsd_structuration.tests.utils_gml import (
    NS_RECOSTAR,
...
class TestUtilitairesNamespace:
    """Helpers communs aux trois contrôles E0110/E0111/E0113."""
...
class TestConvertirErreurOrdre:
    """Mapping des types d'erreur de séquence vers les codes E0113."""
...
class TestAnalyseurEnteteConforme:
    """Cas nominal : un fichier entièrement conforme à E0113."""
...
class TestAnalyseurSchemaLocation:
    ...
    def test_schema_location_main_priorite_majeure(
        ...
        """Seule dérogation de E0113 : la branche 'main' est majeure, pas bloquante.
        ...
        (erreur,) = erreurs
        assert erreur.priorite == PRIORITE_MOYENNE
        ...
        assert rapport["nb_par_priorite"] == {PRIORITE_MOYENNE: 1}
    ...
    def test_schema_location_autre_version_reste_bloquant(
        ...
        cibles = [e for e in erreurs if e.code == CODE_SCHEMA_LOCATION_VERSION_INCORRECTE]
        assert len(cibles) == 1
        assert cibles[0].priorite == PRIORITE_FORTE
...
class TestPrioriteAutresErreursEntete:
    ...
    def test_toutes_bloquantes_sur_entete_degrade(
        ...
        assert {e.priorite for e in erreurs} == {PRIORITE_FORTE}
        assert _construire_rapport(chemin, erreurs)["conformite"] == "NON_CONFORME"
...
class TestGenererRapport:
    ...
    def test_nom_suffixe_e0113(self, chemin_gml_vide):
        chemin = generer_rapport(chemin_gml_vide, [], chemin_gml_vide.parent)
        assert "_controle_e0113" in chemin.name
