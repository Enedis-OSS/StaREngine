#!/usr/bin/env python3
"""
Point d'entrée du contrôle E0014 : valeurs des champs (énumérations, CodeLists, formats) d'un fichier GML RecoStaR **V1.0**.

E0014 est l'équivalent V1.0 du contrôle E0114 (V1.1) : même moteur, même
...
`<nom_gml>_controle_e0014.json`.

Aucune logique de contrôle n'est dupliquée ici : ce module se contente de figer
la version appliquée par le moteur de `e0114.py`. L'option
`--version` n'est donc pas offerte, le code du contrôle désignant déjà la
version.

Entrée  : Fichier GML RecoStaR V1.0 à contrôler
Sortie  : Fichier JSON `<nom_gml>_controle_e0014.json`

Usage :
    python -m recostar.controle.xsd_structuration.e0014 <fichier.gml> [--output-dir <repertoire>]
"""

from recostar.controle.xsd_structuration.e0114 import main as _main
...
def main() -> None:
    """Exécute le contrôle E0014 en imposant la version RecoStaR 1.0."""
