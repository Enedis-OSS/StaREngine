...
§[3] (Metadata) et §9 (ReseauUtilite) que ni le contrôle structurel E0110
ni le contrôle métier E0111 ne prennent en charge.
...
d'analyse (e0113) consomme ces structures sans les recalculer.

Référence : "Structuration des informations attendue pour les fichiers
de récolement des ouvrages RécoStaR" V1.1.
"""

from recostar.controle.xsd_structuration.codes_controle import RANG_ENTETE
from recostar.controle.xsd_structuration.priorites_structuration import PRIORITE_MOYENNE, priorite_par_regle
...
PRIORITE_SCHEMA_LOCATION_BRANCHE_MAIN: str = PRIORITE_MOYENNE
...
# d'ordonnancement est ainsi mutualisé entre E0110 (types RPD) et E0113 (en-tête).
...
# ---------------------------------------------------------------------------
# Codes d'erreur produits par le contrôle E0113
# ---------------------------------------------------------------------------
...
class ErreurEntete:
    ...
    def __init__(
        ...
        # Slot plutôt qu'attribut de classe : la priorité est déduite du code
        # d'erreur que la règle déclenche, et l'en-tête est le seul contrôle
        # dont une règle y déroge (branche 'main' du XSD).
        self.priorite = priorite if priorite is not None else priorite_par_regle(RANG_ENTETE, code)
