...
from recostar.controle.xsd_structuration.codes_controle import RANG_ORDRE
...
class ErreurOrdre:
    ...
    def priorite(self) -> str:
        """Priorité déduite du code d'erreur du vérificateur (E-1101, E-1106).

        Le vérificateur n'a pas de code pour une séquence hors ordre : cette
        anomalie retombe sur la priorité par défaut de la structuration.
        """
        return priorite_par_regle(RANG_ORDRE, self.type_erreur)
...
def valider_sequence(
    ...
    le contrôle E0113). Par défaut, la table des séquences RPD est utilisée.
