...
class TestVentilerAnomalies:
    ...
    def test_ventilation_prete_convention_xsd(self) -> None:
        ...
        rapport = {
            "nb_erreurs": 7,
            "anomalies_par_priorite": {PRIORITE_BLOQUANT: 5, PRIORITE_MAJEUR: 1, PRIORITE_MINEUR: 1},
        }
    ...
    def test_ventilation_prete_prime_sur_la_priorite_par_defaut(self) -> None:
        ...
        rapport = {"nb_erreurs": 1, "anomalies_par_priorite": {PRIORITE_MAJEUR: 1}}
        assert ventiler_anomalies(rapport, PRIORITE_BLOQUANT) == {PRIORITE_MAJEUR: 1}
    ...
    def test_ventilation_prete_vide_retombe_sur_le_defaut(self) -> None:
        ...
        assert ventiler_anomalies(rapport, PRIORITE_BLOQUANT) == {PRIORITE_BLOQUANT: 4}
...
class TestStatutFamille:
    ...
    def test_conforme_malgre_anomalies_majeures(self) -> None:
        ...
        Convention appliquee par E202 et la regle 2 d'E506 : l'ecart doit
        etre corrige, mais il ne bloque pas la livraison du recolement.
        """
        famille = ResultatFamille("altimetrie", "Altimétrie", (_controle(nombre=19, priorites={PRIORITE_MAJEUR: 19}),))
        assert famille.statut == STATUT_CONFORME
    ...
    def test_conforme_malgre_anomalies_mineures(self) -> None:
        famille = ResultatFamille("cable", "Cable", (_controle(nombre=3, priorites={PRIORITE_MINEUR: 3}),))
        assert famille.statut == STATUT_CONFORME
    ...
    def test_bloquant_declasse_meme_avec_des_majeures(self) -> None:
        ...
        famille = ResultatFamille(
            ...
            "Altimétrie",
            (
                _controle("E202", nombre=19, priorites={PRIORITE_MAJEUR: 19}),
                _controle("E200", nombre=1, priorites={PRIORITE_BLOQUANT: 1}),
            ),
        )
        assert famille.statut == STATUT_NON_CONFORME
...
class TestPrioritesDeclassantes:
    ...
    def test_seul_bloquant_declasse(self) -> None:
        assert PRIORITES_DECLASSANTES == frozenset({PRIORITE_BLOQUANT})

    def test_majeur_et_mineur_ne_declassent_pas(self) -> None:
        assert PRIORITE_MAJEUR not in PRIORITES_DECLASSANTES
        assert PRIORITE_MINEUR not in PRIORITES_DECLASSANTES
