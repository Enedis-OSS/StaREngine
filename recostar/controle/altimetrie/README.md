...
| `priorite` | Niveau de priorité de l'anomalie, dérivé du code d'erreur (`forte`, `moyenne`, `basse`) |
...
|------|--------|-------|-------|----------|-------------------|
| E200 | `controle_e200.py` | Tous les GeoJSON | présence de Z | `basse` | `ecarts_e200_3d.geojson` |
| E201 | `controle_e201.py` | v1.0 : câbles élec. — v1.1 : tous les GeoJSON (statut `UnderCommissionning`) | Z = 0.0 | `basse` | `ecarts_e201_z_null.geojson` |
| E202 | `controle_e202.py` | Câbles (élec./terre/télécom) | écart résiduel > 0,40 m | `moyenne` | `ecarts_e202_controle_alti_sommets.geojson` |
| E203 | `controle_e203.py` | Géométries supplémentaires | écart / MNT IGN ≥ 0,40 m | `basse` | `ecarts_e203_z_ign.geojson` |
| E204 | `controle_e204.py` | Points levés ouvrage réseau | coordonnées identiques | `moyenne` | `ecarts_e204_doublons_spatiaux.geojson` |
| E205 | `controle_e205.py` | Géométries suppl. de coffrets | point de levé absent | `forte` | `ecarts_e205_point_leve_geom_supp.geojson` |
| E206 | `controle_e206.py` | Géométries suppl. de bâtiments techniques (rattachés à un poste) | point de levé absent **sur les sommets** | `moyenne` | `ecarts_e206_point_leve_sommets_geom_supp.geojson` |
| E207 | `controle_e207.py` | Géométries suppl. de supports (**v1.1 uniquement**) | point de levé absent (toute la géométrie) | `forte` | `ecarts_e207_point_leve_geom_supp_support.geojson` |
| E208 | `controle_e208.py` | Sommets des câbles (élec./terre/télécom) | sommet non superposé à un point de levé, ou X/Y/Z ≠ | `forte` | `ecarts_e208_point_leve_sommets_cables.geojson` |
| E209 | `controle_e209.py` | Points levés (`RPD_PointLeveOuvrageReseau_Reco`) | point levé orphelin (aucune superposition avec un autre GeoJSON) | `moyenne` | `ecarts_e209_points_leve_orphelins.geojson` |
...
**Sortie — `ecarts_e200_3d.geojson` :** un `Feature` par entité non conforme,
...
**Sortie — `ecarts_e201_z_null.geojson` :** un point `Point` par sommet fautif, avec
les propriétés :
...
**Sortie — `ecarts_e202_controle_alti_sommets.geojson` :** un point `Point` par
...
**Sortie — `ecarts_e203_z_ign.geojson` :** un point `Point` par sommet en écart, avec
les propriétés :
...
**Sortie — `ecarts_e204_doublons_spatiaux.geojson` :** un point `Point` par **groupe**
...
- Un point positionné sur le **bord** du polygone est accepté (`dwithin` à
  `TOLERANCE_SUPERPOSITION`, voir ci-dessous).
- Un `STRtree` Shapely est utilisé pour l'interrogation spatiale en O(log n).

**Tolérance de superposition — `TOLERANCE_SUPERPOSITION` (1 mm).** Le contact
d'un point avec le **contour** d'un polygone est un contact de **mesure nulle**,
exactement comme le contact avec une ligne : l'arrondi millimétrique de la
`posList` GML suffit à le rompre, et un prédicat `intersects` — de tolérance
nulle — rejetait alors un point pourtant correctement posé sur l'arête. Un point
**intérieur** au polygone n'a jamais été concerné, ce test étant numériquement
robuste. La constante est définie une seule fois dans
[`utils_geometrie_commun.py`](../utils_geometrie_commun.py) et partagée avec
E209 — même cause, donc même valeur. E207, qui délègue à
`detecter_geomsupp_sans_point_leve`, en hérite automatiquement.

**Sortie — `ecarts_e205_point_leve_geom_supp.geojson` :** un `Feature` par géométrie
...
**Sortie — `ecarts_e206_point_leve_sommets_geom_supp.geojson` :** un `Feature` par
...
Il **hérite donc automatiquement** de la tolérance `TOLERANCE_SUPERPOSITION`
décrite en section E205.

**Sortie — `ecarts_e207_point_leve_geom_supp_support.geojson` :** un `Feature` par
...
supplémentaires forcées en 2D (`force_2d`) — **même mécanisme géométrique et
même tolérance `TOLERANCE_SUPERPOSITION` que E205 et E209** : `en_contact`
interroge en `dwithin`.

> **La tolérance ne porte que sur l'exemption.** La règle principale d'E208 — la
> superposition exacte et l'égalité stricte X/Y/Z avec un point de levé — reste
> **sans aucune tolérance** (`_classifier_sommet`, index `dict[(x,y) → set(z)]`).
> C'est une règle métier délibérément stricte, à ne pas relâcher. Seul le test de
> contact avec la géométrie supplémentaire, qui **lève** l'obligation, est
> tolérancé.
>
> **C'est le contrôle où le défaut mordait le plus.** Un sommet d'extrémité posé
> sur le **contour** d'une géométrie supplémentaire manquait son exemption dès
> que l'arrondi millimétrique le décalait, et était signalé en anomalie
> **bloquante**. Sur `Echantillon2`, 33 des 37 anomalies étaient de faux
> positifs de ce type, toutes de cause `point_leve_absent` — la seule que
> l'exemption puisse lever — et toutes situées à moins de 1 mm du contour
> (de 0,002 mm à 0,951 mm).

L'absence de `RPD_GeometrieSupplementaire_Reco.geojson` n'est pas
bloquante : aucune exemption n'est alors appliquée, et le comportement historique
du contrôle est conservé à l'identique.

**Sortie — `ecarts_e208_point_leve_sommets_cables.geojson` :** un `Feature` **Point**
...
fichier est **orphelin** et signalé en anomalie de priorité moyenne.
...
- Toutes les géométries des autres fichiers (tous types) sont chargées en 2D
  ...
  interrogé avec le prédicat `dwithin`. C'est le **même moteur de
...
**Tolérance de superposition — `TOLERANCE_SUPERPOSITION = 0.001` (1 mm).**
Définie dans [`utils_geometrie_commun.py`](../utils_geometrie_commun.py) et
partagée avec E205 (et donc E207). Les
coordonnées RecoStaR sont arrondies au millimètre à la source (`posList` du GML).
Un point levé posé **entre deux sommets** d'un câble n'est donc jamais exactement
colinéaire au segment une fois arrondi, et un prédicat `intersects` — de tolérance
nulle — le déclare orphelin à tort. Le cas se produit même lorsque le point est
*mathématiquement* sur le segment : l'arithmétique flottante double précision
laisse alors un résidu de l'ordre de 10⁻¹⁰ m.

La valeur retenue couvre exactement l'arrondi de la donnée source et rien de plus :
un point réellement égaré, même au centimètre, reste détecté. Elle est
volontairement plus stricte que l'`EPSILON_SPATIAL` de E404 (1 cm), qui arbitre
une **adjacence métier** entre cheminements et non un artefact numérique.

> **Pourquoi seuls les points sur une ligne étaient touchés.** Un test
> d'appartenance **surfacique** (point à l'intérieur d'un `MultiPolygon`) est
> numériquement robuste — l'erreur d'arrondi devrait atteindre le décimètre pour
> faire basculer le résultat. Un test d'appartenance à une **ligne** est au
> contraire un problème de mesure nulle : la probabilité qu'un point non-sommet
> soit *exactement* colinéaire en flottant est essentiellement nulle.

**Sortie — `ecarts_e209_points_leve_orphelins.geojson` :** un `Feature` **Point** par
point levé orphelin (géométrie conservée), avec les propriétés `id_entite`,
`type_anomalie` = `point_leve_orphelin`, `priorite` = `moyenne`.
