...
class EtapePipeline:
    ...
    - `script` : chemin du script, relatif a RACINE ; sert de garde-fou
      d'existence, et de forme d'invocation pour les etapes a imports plats ;
    - `module` : chemin de paquet de l'etape, lorsqu'elle en est un. Renseigne,
      l'etape est lancee par `python -m` : un script de paquet importe par
      chemin ne resoudrait pas ses propres imports absolus ;
...
ETAPES: tuple[EtapePipeline, ...] = (
    ...
    EtapePipeline(
        ...
        module="recostar.controle.pipeline_globale",
        mode=MODE_CONTROLE,
        sortie_json=True,
    ),
    EtapePipeline(
...
def _invocation(etape: EtapePipeline) -> list[str]:
    """Arguments d'invocation de l'etape, hors interpreteur et arguments metier.

    Une etape declarant un `module` est lancee par `-m` : lancee par chemin, le
    repertoire du script occuperait sys.path[0] et ses imports absolus
    (`recostar.controle...`) resteraient introuvables. Les etapes a imports
    plats gardent l'invocation par chemin, qui est precisement ce dont elles
    ont besoin.
    """
    if etape.module is not None:
        return ["-m", etape.module]
    return [str(RACINE / etape.script)]
...
def executer_etape(etape: EtapePipeline, contexte: ContexteOrchestration) -> ResultatEtape:
    ...
    Le sous-processus est lance depuis la racine du depot : `python -m` y
    trouve le paquet `recostar`, et les etapes lancees par chemin ne dependent
    pas du repertoire courant, leurs arguments etant tous absolus.
    """
    ...
    commande = [sys.executable, *_invocation(etape), *CONSTRUCTEURS_ARGUMENTS[etape.mode](contexte)]
    debut = perf_counter()
    ...
    processus = subprocess.run(  # nosec B603
        commande, capture_output=True, text=True, check=False, cwd=RACINE.parent
    )
