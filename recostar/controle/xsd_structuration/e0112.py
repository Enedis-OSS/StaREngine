...
Le code du contrôle suit la version contrôlée — **E0112** en V1.1, **E0012** en
V1.0 (point d'entrée dédié `e0012.py`), le XSD appliqué étant celui du
...
Usage :
    python -m recostar.controle.xsd_structuration.e0112 <fichier.gml> [--xsd <chemin.xsd>] [--output-dir <repertoire>] \
...
from recostar.controle.xsd_structuration.cli_controle import (
    construire_parseur,
    resoudre_profil_argument,
    valider_arguments,
)
from recostar.controle.xsd_structuration.codes_controle import RANG_XSD_NATIF, identite_controle
...
# ---------------------------------------------------------------------------
# Codes d'erreur de la taxonomie française E0112
# ---------------------------------------------------------------------------
...
def _categoriser_type_erreur(type_lxml: str | None) -> str:
    """Mappe le type_name lxml vers un code français de la taxonomie E0112.
...
class ErreurXsd:
    """Erreur de validation XSD remontée par lxml, reclassée taxonomie E0112."""
    ...
    # Attribut de classe pour homogénéiser le rapport JSON avec E0114.
    severite = "ERREUR"

    # Priorité déduite du code d'erreur du vérificateur que ce contrôle
    # déclenche (E-1104, de niveau moyen) : elle n'est pas choisie ici.
    priorite = priorite_par_rang(RANG_XSD_NATIF)
...
def _convertir_entree_lxml(entree) -> ErreurXsd:
    """Convertit une _LogEntry lxml en ErreurXsd taxonomie E0112.
...
def _construire_rapport(
    ...
    """Construit le dictionnaire de rapport E0112 à sérialiser en JSON."""
    nb_erreurs = len(erreurs)
    # Ventilation par priorité : E0112 n'accorde aucune dérogation, un rejet par
    ...
    return {
        ...
        # E0112 ne produit que des erreurs : ventilation alignée sur E0114.
...
def _resoudre_chemin_sortie(
    ...
    Le suffixe porte le code du contrôle appliqué : `_controle_e0112.json` en
    V1.1, `_controle_e0012.json` en V1.0.
    """
...
DESCRIPTION_CLI: str = (
    ...
    "de la version contrôlée (E0112 en V1.1, E0012 en V1.0)."
)
