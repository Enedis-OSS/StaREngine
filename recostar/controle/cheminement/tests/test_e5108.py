"""
Tests unitaires du controle des superpositions de cheminements (e5108).
...
from recostar.controle.cheminement.e5108 import (
    FICHIER_SORTIE,
...
from recostar.controle.cheminement.tests.utils_tests import (
...
# Deux voies nommees, et non prises par leur rang dans FICHIERS_CHEMINEMENT :
# l'ordre du tuple a change quand la galerie y est entree, et un index y designe
# desormais une autre couche.
_NOM_FOURREAU = FICHIER_FOURREAU
_NOM_PLEINE_TERRE = FICHIER_PLEINE_TERRE

# Cable de reference des cas geometriques : deux cheminements qui le portent
# tous les deux remplissent la condition attributaire du controle.
_CABLE_COMMUN = "idcable-1"

# Second cable, sans recouvrement avec le premier : sert aux cas ou les deux
# cheminements sont attributairement etrangers l'un a l'autre.
_CABLE_AUTRE = "idcable-2"

# Proprietes GeoJSON portant la reference de cable, telles que produites par la
# conversion (cf. recostar_to_geojson : un cheminement porte un cable unique).
_PROPS_CABLE_COMMUN = {"cables_href": _CABLE_COMMUN}
_PROPS_CABLE_AUTRE = {"cables_href": _CABLE_AUTRE}
...
def _entite(
    ...
    identifiant: str = "e1",
    cables: tuple[str, ...] = (_CABLE_COMMUN,),
) -> EntiteCheminement:
    """Construit une EntiteCheminement directement depuis des coordonnees.

    `cables` vaut par defaut l'unique cable partage par toutes les entites de
    test : les cas purement geometriques restent ainsi lisibles sans repreciser
    la reference a chaque appel. Le filtre attributaire a ses propres tests, qui
    passent ce parametre explicitement.
    """
...
# ---------------------------------------------------------------------------
# Tests du filtre attributaire : lien par cables_href
# ---------------------------------------------------------------------------


class TestCablesPartages:
    """Intersection des references cables de deux cheminements."""

    def test_meme_cable_unique(self) -> None:
        a = _entite(_COORDS_HORIZ, cables=(_CABLE_COMMUN,))
        b = _entite(_COORDS_HORIZ_IDENT, cables=(_CABLE_COMMUN,))
        assert cables_partages(a, b) == {_CABLE_COMMUN}

    def test_cables_distincts_aucun_partage(self) -> None:
        a = _entite(_COORDS_HORIZ, cables=(_CABLE_COMMUN,))
        b = _entite(_COORDS_HORIZ_IDENT, cables=(_CABLE_AUTRE,))
        assert cables_partages(a, b) == frozenset()

    def test_recoupement_partiel_retourne_l_intersection(self) -> None:
        """Un seul cable commun suffit : c'est lui qui est trace deux fois."""
        a = _entite(_COORDS_HORIZ, cables=(_CABLE_COMMUN, _CABLE_AUTRE))
        b = _entite(_COORDS_HORIZ_IDENT, cables=(_CABLE_AUTRE, "idcable-3"))
        assert cables_partages(a, b) == {_CABLE_AUTRE}

    def test_reference_absente_d_un_cote(self) -> None:
        a = _entite(_COORDS_HORIZ, cables=())
        b = _entite(_COORDS_HORIZ_IDENT, cables=(_CABLE_COMMUN,))
        assert cables_partages(a, b) == frozenset()

    def test_reference_absente_des_deux_cotes(self) -> None:
        """Deux entites sans reference ne partagent rien : l'ensemble vide.

        Le piege serait de conclure a l'egalite de deux ensembles vides.
        """
        a = _entite(_COORDS_HORIZ, cables=())
        b = _entite(_COORDS_HORIZ_IDENT, cables=())
        assert cables_partages(a, b) == frozenset()


class TestFiltreAttributairePaire:
    """_analyser_paire ne retient que les superpositions d'un meme cable."""

    def test_meme_cable_superpose_est_une_anomalie(self) -> None:
        a = _entite(_COORDS_HORIZ, cables=(_CABLE_COMMUN,))
        b = _entite(_COORDS_HORIZ_IDENT, identifiant="e2", cables=(_CABLE_COMMUN,))
        anomalie = _analyser_paire(a, b)
        assert anomalie is not None
        assert anomalie["cables_communs"] == [_CABLE_COMMUN]

    def test_cables_differents_pas_d_anomalie(self) -> None:
        """Deux cheminements superposes portant des cables distincts sont licites."""
        a = _entite(_COORDS_HORIZ, cables=(_CABLE_COMMUN,))
        b = _entite(_COORDS_HORIZ_IDENT, identifiant="e2", cables=(_CABLE_AUTRE,))
        assert _analyser_paire(a, b) is None

    def test_reference_absente_pas_d_anomalie(self) -> None:
        """Sans reference, le doublon de saisie ne peut pas etre etabli."""
        a = _entite(_COORDS_HORIZ, cables=())
        b = _entite(_COORDS_HORIZ_IDENT, identifiant="e2", cables=())
        assert _analyser_paire(a, b) is None

    def test_meme_cable_sans_superposition_pas_d_anomalie(self) -> None:
        """Le lien attributaire ne suffit pas : la geometrie doit se recouvrir."""
        a = _entite([[0.0, 0.0], [10.0, 0.0]], cables=(_CABLE_COMMUN,))
        b = _entite([[100.0, 0.0], [200.0, 0.0]], identifiant="e2", cables=(_CABLE_COMMUN,))
        assert _analyser_paire(a, b) is None

    def test_recoupement_partiel_signale_le_cable_commun(self) -> None:
        a = _entite(_COORDS_HORIZ, cables=(_CABLE_COMMUN, _CABLE_AUTRE))
        b = _entite(_COORDS_HORIZ_IDENT, identifiant="e2", cables=(_CABLE_AUTRE,))
        anomalie = _analyser_paire(a, b)
        assert anomalie is not None
        assert anomalie["cables_communs"] == [_CABLE_AUTRE]

    def test_cables_communs_tries(self) -> None:
        """L'ordre de sortie est stable : un frozenset n'en garantit aucun."""
        a = _entite(_COORDS_HORIZ, cables=("idcable-9", "idcable-1"))
        b = _entite(_COORDS_HORIZ_IDENT, identifiant="e2", cables=("idcable-1", "idcable-9"))
        anomalie = _analyser_paire(a, b)
        assert anomalie is not None
        assert anomalie["cables_communs"] == ["idcable-1", "idcable-9"]


class TestExtractionCablesDepuisFeature:
    """Les formats de cables_href admis par le projet sont tous reconnus."""

    def test_chaine_simple(self) -> None:
        """Forme produite par la conversion : un cheminement, un cable."""
        feature = construire_feature_linestring("f1", _COORDS_HORIZ, {"cables_href": _CABLE_COMMUN})
        entite = _creer_entite_depuis_feature(feature, _NOM_FOURREAU)
        assert entite is not None
        assert entite.ids_cables == {_CABLE_COMMUN}

    def test_chaine_separee_par_virgules(self) -> None:
        feature = construire_feature_linestring("f1", _COORDS_HORIZ, {"cables_href": f"{_CABLE_COMMUN},{_CABLE_AUTRE}"})
        entite = _creer_entite_depuis_feature(feature, _NOM_FOURREAU)
        assert entite is not None
        assert entite.ids_cables == {_CABLE_COMMUN, _CABLE_AUTRE}

    def test_chaine_separee_par_espaces(self) -> None:
        feature = construire_feature_linestring("f1", _COORDS_HORIZ, {"cables_href": f"{_CABLE_COMMUN} {_CABLE_AUTRE}"})
        entite = _creer_entite_depuis_feature(feature, _NOM_FOURREAU)
        assert entite is not None
        assert entite.ids_cables == {_CABLE_COMMUN, _CABLE_AUTRE}
    ...
    def test_liste(self) -> None:
        feature = construire_feature_linestring("f1", _COORDS_HORIZ, {"cables_href": [_CABLE_COMMUN, _CABLE_AUTRE]})
        entite = _creer_entite_depuis_feature(feature, _NOM_FOURREAU)
        assert entite is not None
        assert entite.ids_cables == {_CABLE_COMMUN, _CABLE_AUTRE}

    def test_champ_absent_donne_un_ensemble_vide(self) -> None:
        feature = construire_feature_linestring("f1", _COORDS_HORIZ)
        entite = _creer_entite_depuis_feature(feature, _NOM_FOURREAU)
        assert entite is not None
        assert entite.ids_cables == frozenset()

    def test_champ_vide_donne_un_ensemble_vide(self) -> None:
        feature = construire_feature_linestring("f1", _COORDS_HORIZ, {"cables_href": ""})
        entite = _creer_entite_depuis_feature(feature, _NOM_FOURREAU)
        assert entite is not None
        assert entite.ids_cables == frozenset()
...
class TestConstruireGeojsonEcarts:
    ...
    def test_structure_feature_anomalie(self) -> None:
        ...
        assert props["cables_communs"] == _CABLE_COMMUN
...
def repertoire_intra_superposition(tmp_path: Any) -> str:
    """Fourreau : deux entites identiques portant le meme cable -> anomalie intra."""
    _ecrire_fourreau(
        tmp_path,
        [
            construire_feature_linestring("f1", _COORDS_HORIZ, _PROPS_CABLE_COMMUN),
            construire_feature_linestring("f2", _COORDS_HORIZ_IDENT, _PROPS_CABLE_COMMUN),
        ],
    )
    return str(tmp_path)
...
def repertoire_inter_superposition(tmp_path: Any) -> str:
    """Fourreau et PleineTerre identiques, meme cable -> anomalie inter-couches."""
    _ecrire_fourreau(tmp_path, [construire_feature_linestring("f1", _COORDS_HORIZ, _PROPS_CABLE_COMMUN)])
    _ecrire_pleine_terre(tmp_path, [construire_feature_linestring("p1", _COORDS_HORIZ_IDENT, _PROPS_CABLE_COMMUN)])
    return str(tmp_path)
...
def repertoire_sans_superposition(tmp_path: Any) -> str:
    ...
    _ecrire_fourreau(tmp_path, [construire_feature_linestring("f1", [[0.0, 0.0], [10.0, 0.0]], _PROPS_CABLE_COMMUN)])
    _ecrire_pleine_terre(
        tmp_path, [construire_feature_linestring("p1", [[100.0, 0.0], [200.0, 0.0]], _PROPS_CABLE_COMMUN)]
    )
    return str(tmp_path)


@pytest.fixture
def repertoire_superposition_cables_differents(tmp_path: Any) -> str:
    """Segments identiques mais cables distincts -> superposition justifiee."""
    _ecrire_fourreau(tmp_path, [construire_feature_linestring("f1", _COORDS_HORIZ, _PROPS_CABLE_COMMUN)])
    _ecrire_pleine_terre(tmp_path, [construire_feature_linestring("p1", _COORDS_HORIZ_IDENT, _PROPS_CABLE_AUTRE)])
    return str(tmp_path)


@pytest.fixture
def repertoire_superposition_sans_reference(tmp_path: Any) -> str:
    """Segments identiques sans cables_href -> doublon non etablissable."""
...
class TestCli:
    ...
    def test_superposition_cables_differents_zero_anomalie(
        self, repertoire_superposition_cables_differents: str
    ) -> None:
        """Deux cheminements confondus mais portant des cables distincts sont licites."""
        resultat = executer_controle_cli(repertoire_superposition_cables_differents)
        assert resultat["succes"] is True
        assert resultat["nombre_anomalies"] == 0

    def test_superposition_sans_reference_zero_anomalie(self, repertoire_superposition_sans_reference: str) -> None:
        """Sans cables_href, le doublon recherche ne peut pas etre etabli."""
        resultat = executer_controle_cli(repertoire_superposition_sans_reference)
        assert resultat["succes"] is True
        assert resultat["nombre_anomalies"] == 0

    def test_cables_communs_presents_dans_la_sortie(self, repertoire_intra_superposition: str) -> None:
        """L'ecart porte le cable en cause : sans lui, il n'est pas exploitable."""
        executer_controle_cli(repertoire_intra_superposition)
        chemin = os.path.join(repertoire_intra_superposition, FICHIER_SORTIE)
        with open(chemin, encoding="utf-8") as fichier:
            collection = json.load(fichier)
        assert collection["features"][0]["properties"]["cables_communs"] == _CABLE_COMMUN
    ...
    def test_fichiers_absents_listes_dans_resultat(self, repertoire_intra_superposition: str) -> None:
        # Seul le Fourreau est present : les quatre autres voies sont signalees.
        resultat = executer_controle_cli(repertoire_intra_superposition)
        assert len(resultat["fichiers_absents"]) == len(FICHIERS_CHEMINEMENT) - 1
