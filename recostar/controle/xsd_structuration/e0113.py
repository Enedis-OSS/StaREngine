...
contrôle suit la version contrôlée — **E0113** en V1.1, **E0013** en V1.0 (point
d'entrée dédié `e0013.py`).

Entrée  : Fichier GML RecoStaR à contrôler
Sortie  : Fichier JSON listant les erreurs d'en-tête détectées

Usage :
    python -m recostar.controle.xsd_structuration.e0113 <fichier.gml> [--output-dir <repertoire>] \
...
from recostar.controle.xsd_structuration.codes_controle import RANG_ENTETE, identite_controle
from recostar.controle.xsd_structuration.priorites_structuration import statut_conformite, ventiler_par_priorite
from recostar.controle.xsd_structuration.regles_entete import (
    CARDINALITES_ENTETE,
...
from recostar.controle.xsd_structuration.sequenceur_xsd import ErreurOrdre, SlotSequence, valider_sequence
...
# ---------------------------------------------------------------------------
# Utilitaires namespace (cohérents avec e0110 / e0111)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Mapping des types d'erreur ORDRE -> codes E0113
# ---------------------------------------------------------------------------

# Table immuable : ordre ⟶ code E0113. Pré-calculée pour éviter une cascade
...
def _convertir_erreur_ordre(erreur: ErreurOrdre) -> ErreurEntete:
    """Convertit une ErreurOrdre (E0110) en ErreurEntete (E0113).

    Le moteur valider_sequence renvoie des ErreurOrdre génériques ; on les
    requalifie ici en codes propres au contrôle E0113 pour conserver une
...
def _verifier_sequence_objet(
    ...
    Réutilise valider_sequence du moteur E0110 en lui passant la table
...
class AnalyseurEntete:
    """Analyse l'en-tête d'un fichier GML RecoStaR et applique le contrôle E0113."""
    ...
    def __init__(self, chemin_gml: Path, profil: ProfilVersion | None = None) -> None:
        ...
        # Profil par défaut : V1.1, quand l'appelant n'impose pas de version.
...
def _construire_rapport(
    ...
    """Construit le dictionnaire de rapport E0113 à sérialiser en JSON."""
    ...
    return {
        ...
        # E0113 ne produit que des erreurs : ventilation alignée sur E0114.
...
def _resoudre_chemin_sortie(
    ...
    Le suffixe porte le code du contrôle appliqué : `_controle_e0113.json` en
    V1.1, `_controle_e0013.json` en V1.0.
    """
