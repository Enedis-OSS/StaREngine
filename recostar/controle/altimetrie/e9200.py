...
- autres versions : toutes les entites sont controlees.
La version est detectee par le mecanisme commun `version_recostar` (presence du champ
TypeLeve dans RPD_PointLeveOuvrageReseau_Reco) et peut etre imposee en CLI.

Usage CLI :
    python -m recostar.controle.altimetrie.e9200 --repertoire <chemin> [--sortie <chemin>]
                                                 [--version {auto,1.0,1.1}]

Sortie : ecarts_e9200_z_ign.geojson
"""
...
# Detection de version, commune a tous les controles GeoJSON. Le wrapper par
# repertoire est reexpose ici sous le nom determiner_version_effective, que
# l'API de ce module presente a ses appelants.
...
FICHIER_SOURCE: str = FICHIER_GEOM_SUPP

# Nom du fichier GeoJSON de sortie
FICHIER_SORTIE: str = "ecarts_e9200_z_ign.geojson"

# Identite du controle, utilisee pour normaliser les proprietes des ecarts.
CODE_CONTROLE: str = "E-9200"
...
# wrapper determiner_version_effective viennent de `version_recostar`.

# Filtrage specifique a la version 1.1. Memes valeurs que dans E-9201, definies
# localement pour ne pas dependre de son module, qui charge shapely.
...
def filtrer_features_selon_statut(
    ...
    d'origine est retournee telle quelle, sans copie afin d'eviter une
    duplication memoire inutile.
    """
    if version != "1.1":
        return features
    return [
...
def executer_controle_cli(
    ...
    if collection is None:
        return rapport_sans_objet(motif_couche_absente(FICHIER_SOURCE, repertoire_resolu))

    features = collection.get("features", [])
    crs = collection.get("crs")
    if not features:
        return rapport_sans_objet(f"Couche {FICHIER_SOURCE} vide : aucun element a controler")
    ...
    # Hors v1.1, l'absence de sommet 3D est une anomalie de donnees : toutes les
    # entites sont controlees, aucune ne devrait etre privee de Z. En v1.1,
    # l'absence d'entite UnderCommissionning est au contraire nominale.
