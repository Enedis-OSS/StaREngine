"""
Controle E-6109 : localisation des points de comptage et ouvrages collectifs.
...
Le discriminant, mutualise avec E-6204 (`possede_geometrie_propre`), est
...
Usage CLI :
    python -m recostar.controle.conteneur.e6109 --repertoire <chemin> [--sortie <chemin>]

Sortie : ecarts_e6109_ouvrage_localisation_absente.geojson
"""
...
# relation mutualises avec la chaine de localisation ; discriminant de geometrie
# propre et indexation des conteneurs mutualises avec E-6204. Les trois controles
# evaluent
...
COUCHES_CONTENEUR_AUTORISEES: frozenset[str] = frozenset({COUCHE_COFFRET, COUCHE_BATIMENT})
...
FICHIER_SORTIE: str = "ecarts_e6109_ouvrage_localisation_absente.geojson"

# Identite du controle, utilisee pour normaliser les proprietes des ecarts.
CODE_CONTROLE: str = "E-6109"
...
# `classifier_chaine_conteneur`, mutualise avec la chaine de localisation.
...
def est_a_controler(proprietes: Mapping[str, Any]) -> bool:
    ...
    terrain. Meme perimetre de statut qu'E-6108 et E-6204.
    """
...
def motif_echec_conteneur(
    ...
    proviennent de `classifier_chaine_conteneur`, la chaine de geometrie
    supplementaire etant identique a celle de la chaine de localisation et
    d'E-6204.
    """
    ...
    return classifier_chaine_conteneur(conteneurs[reference], geometries_supplementaires)
    ...
    features: list[dict[str, Any]] = [
        {
            ...
            "properties": {
                ...
                "fichier_source": f"{a['couche_ouvrage']}{EXTENSION_COUCHE}",
...
def main() -> None:
    ...
    parseur = argparse.ArgumentParser(
        description=(
            "Controle E-6109 : les RPD_PointDeComptage_Reco et "
