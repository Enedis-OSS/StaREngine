...
par les contrôles E0110 à E0114.
"""
...
from recostar.controle.xsd_structuration.rapport_commun import (
    NIVEAU_CONTROLE,
...
class TestResoudreCheminRapport:
    ...
    def test_sans_repertoire_sortie(self, chemin_gml: Path):
        chemin = resoudre_chemin_rapport(chemin_gml, None, "_controle_e0110.json")
    ...
    def test_avec_repertoire_sortie(self, chemin_gml: Path, tmp_path: Path):
        ...
        dossier.mkdir()
        chemin = resoudre_chemin_rapport(chemin_gml, dossier, "_controle_e0110.json")
        assert chemin.parent == dossier

    def test_nom_derive_du_gml(self, chemin_gml: Path):
        chemin = resoudre_chemin_rapport(chemin_gml, None, "_controle_e0113.json")
        assert chemin.name == "fichier_controle_e0113.json"
...
class TestConstruireRapport:
    ...
    def test_champs_attendus(self, chemin_gml: Path):
        rapport = construire_rapport(chemin_gml, "E0110_ORDRE", [], "1.1")
    ...
    def test_non_conforme_avec_erreurs(self, chemin_gml: Path):
        rapport = construire_rapport(chemin_gml, "E0110_ORDRE", [_ErreurFactice()], "1.1")
    ...
    def test_metadonnees_propagees(self, chemin_gml: Path):
        rapport = construire_rapport(chemin_gml, "E0113_ENTETE", [], "1.0")
        assert rapport["type_controle"] == "E0113_ENTETE"
    ...
    def test_erreurs_serialisees(self, chemin_gml: Path):
        rapport = construire_rapport(chemin_gml, "E0110_ORDRE", [_ErreurFactice(code="X")], "1.1")
        assert rapport["erreurs"] == [{"code": "X", "severite": "ERREUR"}]

    def test_champs_specifiques_inseres(self, chemin_gml: Path):
        """E0112 documente son XSD : le champ suit immédiatement 'fichier'."""
    ...
    def test_sans_champs_specifiques(self, chemin_gml: Path):
        rapport = construire_rapport(chemin_gml, "E0110_ORDRE", [], "1.1", champs_specifiques=None)
...
class TestEcritureRapport:
    ...
    def test_generer_enchaine_construction_et_ecriture(self, chemin_gml: Path):
        chemin = generer_rapport(
            chemin_gml,
            "E0111_METIER",
            "_controle_e0111.json",
            [_ErreurFactice()],
            None,
            "1.1",
        )
        assert chemin.name == "fichier_controle_e0111.json"
        rapport = json.loads(chemin.read_text(encoding="utf-8"))
        assert rapport["type_controle"] == "E0111_METIER"
        assert rapport["conformite"] == "NON_CONFORME"

    def test_generer_respecte_le_repertoire_sortie(self, chemin_gml: Path, tmp_path: Path):
        dossier = tmp_path / "out"
        dossier.mkdir()
        chemin = generer_rapport(chemin_gml, "E0110_ORDRE", "_controle_e0110.json", [], dossier, "1.1")
        assert chemin.parent == dossier
