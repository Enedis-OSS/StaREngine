...
**E0114** en V1.1, **E0014** en V1.0 (point d'entrée dédié `e0014.py`).

Entrée  : Fichier GML RecoStaR à contrôler
Sortie  : Fichier JSON listant les erreurs détectées (sévérité ERREUR unique)

Usage :
    python -m recostar.controle.xsd_structuration.e0114 <fichier.gml> [--output-dir <repertoire>] \
...
from recostar.controle.xsd_structuration.codes_controle import RANG_VALEURS, identite_controle
from recostar.controle.xsd_structuration.priorites_structuration import statut_conformite, ventiler_par_priorite
from recostar.controle.xsd_structuration.regles_valeurs import (
    SEVERITE_ERREUR,
    ErreurValeur,
    evaluer_valeur,
)
...
# ---------------------------------------------------------------------------
# Utilitaires XML — repris à l'identique de e0111 pour cohérence
# inter-contrôles. La duplication est volontaire (3 fonctions, 6 lignes)
# afin de garder E0114 importable de manière autonome sans coupler les
# contrôles entre eux.
# ---------------------------------------------------------------------------
...
def _extraire_valeur(element: Element) -> str | None:
    ...
    Ordre de résolution identique à e0111._extraire_valeur :
...
def _compter_par_severite(erreurs: list[ErreurValeur]) -> dict[str, int]:
    """Compte le nombre d'entrées par sévérité (ERREUR uniquement en E0114)."""
...
def _construire_rapport(
    ...
    """Construit le dictionnaire de rapport E0114 à sérialiser en JSON."""
    par_severite = _compter_par_severite(erreurs)
    # nb_erreurs = nombre d'entrées de sévérité ERREUR (unique sévérité E0114).
    # Clé harmonisée avec E0110-E0113.
    nb_erreurs = par_severite.get(SEVERITE_ERREUR, 0)
    # E0114 est mono-sévérité mais multi-priorités : la règle E_THEME_RPD est
...
def _resoudre_chemin_sortie(
    ...
    Le suffixe porte le code du contrôle appliqué : `_controle_e0114.json` en
    V1.1, `_controle_e0014.json` en V1.0.
    """
...
DESCRIPTION_CLI: str = (
    ...
    "RecoStaR : énumérations, CodeLists et formats (E0114 en V1.1, E0014 en V1.0)."
)
