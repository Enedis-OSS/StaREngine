"""
Tests unitaires du controle d'appartenance a l'emprise DR (E-5106).
...
from recostar.controle.fonctions_communes.emprise_dr import point_dans_emprises, resoudre_repertoires
...
from recostar.controle.projection.e5106 import (
    affaire_exclue_du_controle,
    construire_geojson_ecarts,
    detecter_entites_hors_emprise,
    executer_controle_cli,
)
from recostar.controle.projection.tests.utils_tests import construire_feature, ecrire_collection
...
class TestExtrairePrefixe:
    ...
    def test_format_rac_sans_trigramme_sans_objet(self) -> None:
    ...
    def test_format_da_vide_avant_slash_sans_objet(self) -> None:
        _, _, erreur = _extraire_prefixe("/256553")
        assert erreur is not None

    def test_format_inconnu_sans_objet(self) -> None:
        _, _, erreur = _extraire_prefixe("INCONNU123")
        assert erreur is not None

    def test_format_vide_sans_objet(self) -> None:
...
class TestAffaireExclueDuControle:
    """Tests du predicat d'exclusion totale de E-5106 (affaire_exclue_du_controle)."""
...
class TestCli:
    ...
    def test_numero_affaire_absent_sans_objet(self, tmp_path: Any) -> None:
        resultat = executer_controle_cli(str(tmp_path), None)
        assert resultat["succes"] is False
        assert "numero_affaire" in resultat["erreur"]

    def test_numero_affaire_vide_sans_objet(self, tmp_path: Any) -> None:
        resultat = executer_controle_cli(str(tmp_path), "")
        assert resultat["succes"] is False

    def test_repertoire_inexistant_sans_objet(self) -> None:
    ...
    def test_affaire_exclue_court_circuite_sans_traitement(self, tmp_path: Any) -> None:
        # Numero exclu -> succes immediat, 0 anomalie, rapport sans objet.
    ...
    def test_affaire_exclue_motif_restituable(self, tmp_path: Any) -> None:
        """Le motif accompagne le marqueur : le rapport PDF le restitue."""
        resultat = executer_controle_cli(str(tmp_path), "OSR-CVL-25-007998")
        assert "exclu du controle E-5106" in resultat["motif"]
    ...
    def test_affaire_exclue_ne_cree_pas_de_fichier_ecarts(self, tmp_path: Any) -> None:
        ...
        assert not os.path.isfile(str(tmp_path / "ecarts_e5106_emprise_dr.geojson"))

    @patch("recostar.controle.projection.e5106._charger_references")
    ...
    @patch("recostar.controle.projection.e5106._charger_references")
    def test_format_affaire_invalide_sans_objet(self, mock_refs: Any, tmp_path: Any) -> None:
        mock_refs.return_value = (_REFS_MOCK, None)
        resultat = executer_controle_cli(str(tmp_path), "FORMAT_INVALIDE")
        assert resultat["succes"] is False

    @patch("recostar.controle.projection.e5106._charger_references")
    def test_trigramme_inconnu_sans_objet(self, mock_refs: Any, tmp_path: Any) -> None:
        mock_refs.return_value = (_REFS_MOCK, None)
        resultat = executer_controle_cli(str(tmp_path), "RAC-XYZ-25-001234")
        assert resultat["succes"] is False
        assert "XYZ" in resultat["erreur"]

    @patch("recostar.controle.projection.e5106._charger_emprises_dr")
    @patch("recostar.controle.projection.e5106._charger_references")
    def test_sans_geojson_sans_objet(self, mock_refs: Any, mock_emprises: Any, tmp_path: Any) -> None:
        mock_refs.return_value = (_REFS_MOCK, None)
        mock_emprises.return_value = (_EMPRISE_8A, None)
        resultat = executer_controle_cli(str(tmp_path), "RAC-CVL-25-007998")
        assert resultat["succes"] is True
        assert "GeoJSON" in resultat["motif"]
    ...
    def test_fichier_ecarts_cree(self, mock_refs: Any, mock_emprises: Any, tmp_path: Any) -> None:
        ...
        assert os.path.isfile(str(tmp_path / "ecarts_e5106_emprise_dr.geojson"))
    ...
    def test_dossier_sortie_distinct(self, mock_refs: Any, mock_emprises: Any, tmp_path: Any) -> None:
        ...
        assert os.path.isfile(os.path.join(dossier_sortie, "ecarts_e5106_emprise_dr.geojson"))
