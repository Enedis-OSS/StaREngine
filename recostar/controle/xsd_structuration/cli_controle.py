...
Les six contrôles partagent la même enveloppe en ligne de commande : un
...
Ce module factorise cette enveloppe afin que les points d'entrée V1.1 (E0110 à
E0114) et V1.0 (E0010 à E0014) n'aient à déclarer que ce qui leur est propre.

Deux modes de sélection de version :
- `version_imposee=None` : l'option `--version {auto,1.0,1.1}` est offerte, la
  version est détectée ou imposée par l'utilisateur (points d'entrée E011x) ;
- `version_imposee="1.0"` : la version est figée, l'option n'est pas offerte —
  le point d'entrée porte déjà la version dans son code (points d'entrée E001x).
"""
...
from recostar.controle.xsd_structuration.cli_version import ajouter_argument_version, resoudre_profil_cli
from recostar.controle.xsd_structuration.codes_controle import identite_controle
...
def construire_parseur(description: str, version_imposee: str | None = None) -> argparse.ArgumentParser:
    ...
    # (E001x) désignant déjà sans ambiguïté la version appliquée.
...
def executer_controle(
    ...
    Le code affiché (E0110/E0010…) est dérivé du rang du contrôle et de la version
