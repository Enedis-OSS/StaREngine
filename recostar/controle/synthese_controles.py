...
et certains controles multi-regles (E506) exposent un dictionnaire `priorites`
...
# Priorites d'anomalie, de la plus grave a la moins grave. Les cinq niveaux sont
# en service : bloquant (defaut avere), majeur puis mineur (defauts signales, non
# bloquants, de gravite decroissante), information (constat sans exigence de
# correction) et non_precisee (repli quand un rapport n'annonce pas de priorite).
...
# conforme ». Les priorites majeur, mineur et information sont signalees et
# comptees dans le rapport, mais n'invalident pas la conformite : elles
# remontent un ecart a corriger sans bloquer la livraison du recolement.
# Un frozenset : le test d'appartenance de nombre_anomalies_declassantes est en
# O(1) et la valeur reste immuable.
...
def ventiler_anomalies(
    ...
    de la plus explicite a la plus implicite :
      ...
      - `priorites` (type -> priorite) + `anomalies_par_type` : controle
        multi-priorites derivant la priorite du type d'anomalie (E506) ;
