...
class TestListerFichiersGeojson:
    ...
    def test_exclut_fichiers_ecarts(self, tmp_path: Any) -> None:
        (tmp_path / "donnees.geojson").write_text("{}", encoding="utf-8")
        (tmp_path / "ecarts_e0200_3d.geojson").write_text("{}", encoding="utf-8")
        fichiers = lister_fichiers_geojson(str(tmp_path))
        assert "donnees.geojson" in fichiers
        assert "ecarts_e0200_3d.geojson" not in fichiers
...
class TestNormaliserGeojsonEcarts:
    ...
    def test_socle_present_et_en_tete(self) -> None:
        collection = self._collection(
            {"type_anomalie": "anomalie_test", "priorite": PRIORITE_FORTE, "id_principal": "A1", "detail": "x"}
        )
        ...
        assert proprietes["code_controle"] == "E0999"
        # E0999 n'est pas un controle du referentiel : le code d'erreur reste nul
        # sans empecher l'emission de l'ecart.
        assert proprietes["code_erreur"] is None
    ...
    def test_aucune_duplication_des_champs_du_socle(self) -> None:
        collection = self._collection(
            {"type_anomalie": "anomalie_test", "id_entite": "Z9", "priorite": PRIORITE_MOYENNE}
        )
        proprietes = normaliser_geojson_ecarts(collection, ProfilEcarts("E0999", {}))["features"][0]["properties"]
