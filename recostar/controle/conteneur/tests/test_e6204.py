"""
Tests du controle E-6204 : localisation des remontees aero-souterraines.
...
from recostar.controle.conteneur.e6204 import (
    FICHIER_SORTIE,
...
from recostar.controle.fonctions_communes.geojson import EXTENSION_GEOJSON, PRIORITE_MOYENNE
...
# Racine du paquet conteneur : sert au garde d'unicite de geometrie_valide.
_RACINE_CONTENEUR = Path(__file__).resolve().parent.parent
...
class TestIndexerConteneursEtSupports:
    ...
    def test_supports_distingues_des_autres_conteneurs(self, tmp_path: Any) -> None:
        ecrire_collection(
            str(tmp_path / f"{COUCHE_SUPPORT}{EXTENSION_COUCHE}"),
            [{"type": "Feature", "properties": {"id": "s1", CHAMP_GEOMSUPP_HREF: "g1"}, "geometry": GEOM_SUPPORT}],
        )
        ecrire_collection(
            str(tmp_path / f"RPD_Coffret_Reco{EXTENSION_COUCHE}"),
    ...
    def test_extension_alignee_sur_le_socle(self) -> None:
        ...
        assert EXTENSION_COUCHE == EXTENSION_GEOJSON
...
class TestConstruireGeojsonEcarts:
    ...
    def test_socle_commun(self) -> None:
        ...
        assert props["code_controle"] == "E-6204"
...
# --------------------------------------------------------------------------- #
# Coherence avec la chaine de localisation
# --------------------------------------------------------------------------- #


class TestCoherenceAvecE605:
    """E-6105 et E-6204 evaluent la meme chaine, sur des entites disjointes."""

    def test_definition_de_geometrie_valide_unique(self) -> None:
        """La definition vit dans fonctions_communes, aucun controle ne la redit.

        La definition est commune aux deux controles. Le garde
        surveillait alors leur egalite ; il surveille desormais l'unicite de la
        source, seule facon d'empecher qu'une copie reapparaisse dans un
        controle et diverge en silence.
        """
        from recostar.controle.fonctions_communes import localisation_conteneur

        redefinitions = [
            chemin.name
            for chemin in (_RACINE_CONTENEUR / "..").resolve().glob("*/controle_e*.py")
            if "def geometrie_valide(" in chemin.read_text(encoding="utf-8")
        ]
        assert redefinitions == [], f"geometrie_valide redefini dans : {redefinitions}"
        assert callable(localisation_conteneur.geometrie_valide)

    def test_jonction_hors_perimetre_e0605(self) -> None:
        """La chaine commune ne controle pas les jonctions : E-6204 les prend en charge."""
        from recostar.controle.conteneur.detection_localisation_noeud import COUCHES_CIBLES
