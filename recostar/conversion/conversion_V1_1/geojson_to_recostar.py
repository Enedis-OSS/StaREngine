#!/usr/bin/env python3
"""
Convertit des fichiers GeoJSON RPD_* vers un fichier GML RecoStaR unique.
Conforme au schéma XSD StaR-Elec RecoStaR v1.10.

Entrée : Dossier contenant des fichiers GeoJSON (RPD_CableElectrique_Reco.geojson, etc.)
Sortie : Fichier GML unique avec métadonnées et relations préservées

"""

import argparse
import json
import sys
import uuid
from datetime import datetime
from pathlib import Path

# nosemgrep: python.lang.security.use-defused-xml.use-defused-xml
from xml.etree import ElementTree as ET  # nosec B405

# Namespaces XML/GML requis par le schéma RecoStaR
NAMESPACE_GML = "http://www.opengis.net/gml/3.2"
NAMESPACE_RECOSTAR = "http://StaR-Elec.com"
NAMESPACE_XLINK = "http://www.w3.org/1999/xlink"
NAMESPACE_XSI = "http://www.w3.org/2001/XMLSchema-instance"

# Constante pour le système de référence spatial par défaut
DEFAULT_SRS = "EPSG:2154"

NS_MAP = {
    "gml": NAMESPACE_GML,
    "RecoStaR": NAMESPACE_RECOSTAR,
    "xlink": NAMESPACE_XLINK,
    "xsi": NAMESPACE_XSI,
}

# Frozenset pour vérifications rapides (O(1) vs O(n) pour list)
REQUIRED_RPD_FILES = frozenset(
    [
        "RPD_CableElectrique_Reco",
        "RPD_CableTelecommunication_Reco",
        "RPD_CableTerre_Reco",
        "RPD_Coffret_Reco",
        "RPD_EnceinteCloturee_Reco",
        "RPD_Fourreau_Reco",
        "RPD_Galerie_Reco",
        "RPD_GeometrieSupplementaire_Reco",
        "RPD_JeuBarres_Reco",
        "RPD_Jonction_Reco",
        "RPD_ModuleRaccordement_Reco",
        "RPD_OuvrageCollectifBranchement_Reco",
        "RPD_PointDeComptage_Reco",
        "RPD_PointLeveOuvrageReseau_Reco",
        "RPD_BatimentTechnique_Reco",
        "RPD_PosteElectrique_Reco",
        "RPD_ProtectionMecanique_Reco",
        "RPD_SupportModules_Reco",
        "RPD_Terre_Reco",
    ]
)

# Types d'entites constituant les noeuds du reseau : ce sont les seules entites
# porteuses du champ cables_href materialisant la relation noeud <-> cable
# (relation CableElectrique_NoeudReseau du modele RecoStaR).
# Identifiant de repli du ReseauUtilite, employe quand le GeoJSON ne porte
# aucun rattachement : convention historique des fichiers produits par l'outil.
# Nature de la geometrie d'une RPD_GeometrieSupplementaire_Reco, reportee par
# la lecture du GML. Elle prime sur la deduction par la forme, qui ne sait pas
# distinguer une ligne fermee d'un contour.
CHAMP_NATURE_GEOMETRIE: str = "nature_geometrie"
NATURE_LIGNE: str = "Ligne3D"
NATURE_SURFACE: str = "Surface3D"

RESEAU_DEFAUT: str = "Reseau"

# Theme et mention de repli, employes quand le GML source n'en fournit pas.
THEME_DEFAUT: str = "ELECTRD"
LOGICIEL_DEFAUT: str = "LAZio"
VALEUR_DEFAUT: str = "TEST"
MENTION_DEFAUT: str = (
    "Récolement informatisé des ouvrages de distribution publique "
    "d'électricité, diffusion limitée au Maître d'Ouvrage, "
    "à l'Exploitant et leurs Prestataires."
)


def analyser_ids_references(href: str) -> list[str]:
    """Analyse une liste d'identifiants séparés par des virgules.

    Forme de stockage commune à toutes les relations N-N du GeoJSON
    (cables_href, aeriens_href, reseau_href).
    """
    return [ref.strip() for ref in href.split(",") if ref.strip()]


TYPES_NOEUDS_RESEAU: tuple[str, ...] = (
    "RPD_CoupeCircuitAFusibles_Reco",
    "RPD_JeuBarres_Reco",
    "RPD_Jonction_Reco",
    "RPD_ModuleRaccordement_Reco",
    "RPD_OuvrageCollectifBranchement_Reco",
    "RPD_PointDeComptage_Reco",
    "RPD_PosteElectrique_Reco",
    "RPD_SupportModules_Reco",
    "RPD_Terre_Reco",
)


class ElementGML:
    """Classe avec __slots__ pour économiser 20-30% de mémoire"""

    __slots__ = ("tag", "attrib", "text", "children")

    def __init__(self, tag: str, attrib: dict | None = None, text: str | None = None):
        self.tag = tag
        self.attrib = attrib or {}
        self.text = text
        self.children = []


class ConvertisseurGeometrie:
    """Convertit les géométries GeoJSON en éléments GML conformes au schéma"""

    __slots__ = ("srs",)

    def __init__(self, srs: str = DEFAULT_SRS):
        self.srs = srs

    def _formater_coord(self, value: float) -> str:
        return str(value)

    def _coords_vers_chaine(self, coords: list) -> str:
        """Convertit une liste de coordonnées en chaîne pour posList. join est plus rapide que +="""
        return " ".join(self._formater_coord(c) for coord in coords for c in coord)

    def point_vers_gml(self, geometry: dict, gml_id: str) -> ET.Element:
        """GeoJSON Point → gml:Point avec srsName et id"""
        point = ET.Element(f"{{{NAMESPACE_GML}}}Point")
        point.set("srsName", self.srs)

        coords = geometry["coordinates"]
        # Ajouter srsDimension="3" si coordonnées 3D
        if len(coords) > 2:
            point.set("srsDimension", "3")

        point.set(f"{{{NAMESPACE_GML}}}id", gml_id)

        pos = ET.SubElement(point, f"{{{NAMESPACE_GML}}}pos")
        pos.text = f"{coords[0]} {coords[1]}"
        if len(coords) > 2:
            pos.text += f" {coords[2]}"

        return point

    def ligne_vers_gml(self, geometry: dict, gml_id: str) -> ET.Element:
        """GeoJSON LineString → gml:LineString avec posList"""
        linestring = ET.Element(f"{{{NAMESPACE_GML}}}LineString")
        linestring.set("srsName", self.srs)
        linestring.set(f"{{{NAMESPACE_GML}}}id", gml_id)

        coords = geometry["coordinates"]
        pos_list = ET.SubElement(linestring, f"{{{NAMESPACE_GML}}}posList")
        pos_list.set("srsDimension", "3" if len(coords[0]) > 2 else "2")

        pos_list.text = self._coords_vers_chaine(coords)

        return linestring

    def polygone_vers_gml(self, geometry: dict, gml_id: str) -> ET.Element:
        """GeoJSON Polygon → gml:Polygon avec exterior LinearRing"""
        polygon = ET.Element(f"{{{NAMESPACE_GML}}}Polygon")
        polygon.set("srsName", self.srs)
        polygon.set(f"{{{NAMESPACE_GML}}}id", gml_id)

        coords = geometry["coordinates"]
        exterior = ET.SubElement(polygon, f"{{{NAMESPACE_GML}}}exterior")
        linear_ring = ET.SubElement(exterior, f"{{{NAMESPACE_GML}}}LinearRing")
        pos_list = ET.SubElement(linear_ring, f"{{{NAMESPACE_GML}}}posList")

        pos_list.set("srsDimension", "3" if len(coords[0][0]) > 2 else "2")
        pos_list.text = self._coords_vers_chaine(coords[0])

        return polygon

    def multipolygone_vers_gml(self, geometry: dict, gml_id: str) -> ET.Element | None:
        """GeoJSON MultiPolygon → gml:Polygon (prend le premier polygone uniquement)"""
        if geometry["coordinates"]:
            single_polygon = {
                "type": "Polygon",
                "coordinates": geometry["coordinates"][0],
            }
            return self.polygone_vers_gml(single_polygon, gml_id)
        return None


class MappeurEntites:
    """Transforme les entités GeoJSON en éléments XML GML RecoStaR conformes au XSD"""

    __slots__ = (
        "geo_converter",
        "srs",
        "seen_ids",
        "geom_counter",
        "commentaire_vide",
        "reseaux_defaut",
    )

    def __init__(self, srs: str = DEFAULT_SRS, commentaire_vide: bool = False):
        self.geo_converter = ConvertisseurGeometrie(srs)
        self.srs = srs
        self.seen_ids = set()  # Vérification unicité des IDs en O(1)
        self.geom_counter = {}  # Compteur pour générer des IDs de géométrie uniques
        self.commentaire_vide = commentaire_vide  # Émission d'un Commentaire vide si absent
        # Rattachement de repli des entités sans reseau_href, aligné par le
        # générateur sur les ReseauUtilite réellement émis dans le fichier.
        self.reseaux_defaut = [RESEAU_DEFAUT]

    def _ajouter_propriete(self, parent: ET.Element, name: str, value, uom: str | None = None):
        """Ajoute une balise enfant avec valeur. Ignore si la valeur est None ou vide"""
        if value is None or value == "":
            return

        elem = ET.SubElement(parent, f"{{{NAMESPACE_RECOSTAR}}}{name}")

        if uom:
            elem.set("uom", uom)

        if isinstance(value, bool):
            elem.text = "true" if value else "false"
        elif isinstance(value, float):
            # Convertir en int si la valeur est un entier (évite 35.0 → "35.0")
            elem.text = str(int(value)) if value.is_integer() else str(value)
        elif isinstance(value, int):
            elem.text = str(value)
        else:
            elem.text = str(value)

    def _ajouter_commentaire(self, parent: ET.Element, props: dict):
        """Ajoute la balise Commentaire (évolution V1.1 du standard).

        Le champ est optionnel dans le XSD (minOccurs="0") pour toutes les
        entités héritant d'ElementReseauType, ainsi que pour
        RPD_GeometrieSupplementaire_Reco qui le déclare en propre.

        Trois cas :
        - valeur renseignée dans le GeoJSON : balise portant le texte ;
        - valeur absente et mode commentaire_vide actif : balise vide, prête
          à être renseignée manuellement dans le GML produit ;
        - valeur absente sans le mode : aucune balise (comportement historique).
        """
        valeur = props.get("Commentaire")
        if valeur is not None and valeur != "":
            self._ajouter_propriete(parent, "Commentaire", valeur)
        elif self.commentaire_vide:
            ET.SubElement(parent, f"{{{NAMESPACE_RECOSTAR}}}Commentaire")

    def _ajouter_reference(self, parent: ET.Element, name: str, href: str, multiline_reseau: bool = False):
        """Ajoute une référence xlink:href. Ignore si la référence href est vide"""
        if not href:
            return

        elem = ET.SubElement(parent, f"{{{NAMESPACE_RECOSTAR}}}{name}")
        elem.set(f"{{{NAMESPACE_XLINK}}}href", href)

        # Formatage spécial pour balise reseau (retour ligne après)
        if multiline_reseau and name == "reseau":
            elem.text = "\n"

    def _ajouter_references_reseau(self, element: ET.Element, props: dict):
        """Ajoute la ou les balises `reseau` d'une entité d'ElementReseauType.

        Le XSD impose au moins une référence (minOccurs="1") et en autorise
        plusieurs. Le rattachement lu dans le GeoJSON prime ; à défaut —
        GeoJSON produit avant le transport du rattachement, ou entité forgée
        par le convertisseur — l'entité rejoint les réseaux de repli.
        """
        href = props.get("reseau_href")
        reseaux = analyser_ids_references(href) if href else self.reseaux_defaut
        for reseau_id in reseaux or self.reseaux_defaut:
            self._ajouter_reference(element, "reseau", reseau_id, multiline_reseau=True)

    def _obtenir_id_geom_unique(self, feature_type: str, ogr_pkid: str) -> str:
        """Génère un ID unique pour chaque géométrie : {ogr_pkid}.geom{counter}"""
        if feature_type not in self.geom_counter:
            self.geom_counter[feature_type] = 0

        self.geom_counter[feature_type] += 1
        counter = self.geom_counter[feature_type]

        return f"{ogr_pkid}.geom{counter - 1}"

    def mapper_cable_electrique(self, feature: dict, feature_id: str) -> ET.Element:
        """Entité GeoJSON → RPD_CableElectrique_Reco (sans géométrie, héritée du cheminement)"""
        props = feature["properties"]

        element = ET.Element(f"{{{NAMESPACE_RECOSTAR}}}RPD_CableElectrique_Reco")
        element.set(f"{{{NAMESPACE_GML}}}id", feature_id)

        # Référence au réseau (formatage avec retour ligne)
        self._ajouter_references_reseau(element, props)

        # Commentaire (hérité d'ElementReseau, optionnel)
        self._ajouter_commentaire(element, props)

        # Propriétés selon XSD (ordre alphabétique XSD)
        self._ajouter_propriete(element, "DomaineTension", props.get("DomaineTension"))
        self._ajouter_propriete(element, "Etiquette", props.get("Etiquette"))
        self._ajouter_reference(element, "FonctionCable", props.get("FonctionCable_href"))
        self._ajouter_propriete(element, "HierarchieBT", props.get("HierarchieBT"))
        self._ajouter_propriete(element, "Isolant", props.get("Isolant"))
        self._ajouter_propriete(element, "Materiau", props.get("Materiau"))
        self._ajouter_propriete(element, "NombreConducteurs", props.get("NombreConducteurs"))

        # Section avec UOM
        section_val = props.get("Section")
        if section_val is not None:
            self._ajouter_propriete(element, "Section", section_val, props.get("Section_uom", "mm-2"))

        section_neutre = props.get("SectionNeutre")
        if section_neutre is not None:
            self._ajouter_propriete(
                element,
                "SectionNeutre",
                section_neutre,
                props.get("SectionNeutre_uom", "mm-2"),
            )

        self._ajouter_propriete(element, "Statut", props.get("Statut"))

        return element

    def mapper_cable_terre(self, feature: dict, feature_id: str) -> ET.Element:
        """Mappe RPD_CableTerre_Reco"""
        props = feature["properties"]

        element = ET.Element(f"{{{NAMESPACE_RECOSTAR}}}RPD_CableTerre_Reco")
        element.set(f"{{{NAMESPACE_GML}}}id", feature_id)

        # Référence reseau
        self._ajouter_references_reseau(element, props)

        # Commentaire (optionnel)
        self._ajouter_commentaire(element, props)

        # FonctionCable (requis)
        self._ajouter_reference(element, "FonctionCable", props.get("FonctionCable_href"))

        # Materiau (requis)
        self._ajouter_propriete(element, "Materiau", props.get("Materiau"))

        # NatureCableTerre (optionnel)
        self._ajouter_reference(element, "NatureCableTerre", props.get("NatureCableTerre_href"))

        # noeudReseau (optionnel) - référence RPD_Terre_Reco, après NatureCableTerre
        noeud_href = props.get("noeudreseau_href")
        if noeud_href:
            self._ajouter_reference(element, "noeudReseau", noeud_href)

        # Section (requis, avec UOM)
        section_val = props.get("Section")
        if section_val is not None:
            self._ajouter_propriete(element, "Section", section_val, props.get("Section_uom", "mm-2"))

        # Statut (requis)
        self._ajouter_propriete(element, "Statut", props.get("Statut"))

        return element

    def mapper_cable_telecommunication(self, feature: dict, feature_id: str) -> ET.Element:
        """Mappe RPD_CableTelecommunication_Reco"""
        props = feature["properties"]

        element = ET.Element(f"{{{NAMESPACE_RECOSTAR}}}RPD_CableTelecommunication_Reco")
        element.set(f"{{{NAMESPACE_GML}}}id", feature_id)

        # Référence au réseau
        self._ajouter_references_reseau(element, props)

        # Commentaire (hérité d'ElementReseau, optionnel)
        self._ajouter_commentaire(element, props)

        # Capacite (optionnel, entier)
        self._ajouter_propriete(element, "Capacite", props.get("Capacite"))

        # Fonction (optionnel, string)
        self._ajouter_propriete(element, "Fonction", props.get("Fonction"))

        # Section avec UOM (optionnel)
        section_val = props.get("Section")
        if section_val is not None:
            self._ajouter_propriete(element, "Section", section_val, props.get("Section_uom", "mm-2"))

        # Statut (requis)
        self._ajouter_propriete(element, "Statut", props.get("Statut"))

        # TechnoCable (optionnel, CodeList href)
        self._ajouter_reference(element, "TechnoCable", props.get("TechnoCable_href"))

        return element

    def mapper_coffret(self, feature: dict, feature_id: str) -> ET.Element:
        """Mappe RPD_Coffret_Reco"""
        props = feature["properties"]

        element = ET.Element(f"{{{NAMESPACE_RECOSTAR}}}RPD_Coffret_Reco")
        element.set(f"{{{NAMESPACE_GML}}}id", feature_id)

        # Référence reseau avec style multiline
        self._ajouter_references_reseau(element, props)

        # Commentaire (hérité d'ElementReseau, optionnel)
        self._ajouter_commentaire(element, props)

        # Référence géométrie supplémentaire si présente
        geom_supp = props.get("geometriesupplementaire_href")
        if geom_supp:
            self._ajouter_reference(element, "geometriesupplementaire", geom_supp)

        # Propriétés
        self._ajouter_reference(element, "FonctionCoffret", props.get("FonctionCoffret_href"))

        # Géométrie Point avec ID unique
        if feature.get("geometry"):
            geom_id = self._obtenir_id_geom_unique("RPD_Coffret_Reco", props.get("ogr_pkid", feature_id))
            point = self.geo_converter.point_vers_gml(feature["geometry"], geom_id)
            geom_elem = ET.SubElement(element, f"{{{NAMESPACE_RECOSTAR}}}Geometrie")
            geom_elem.append(point)

        self._ajouter_reference(element, "ImplantationArmoire", props.get("ImplantationArmoire_href"))
        self._ajouter_propriete(element, "PrecisionXY", props.get("PrecisionXY"))
        self._ajouter_propriete(element, "PrecisionZ", props.get("PrecisionZ"))
        self._ajouter_propriete(element, "Statut", props.get("Statut"))
        self._ajouter_reference(element, "TypeCoffret", props.get("TypeCoffret_href"))

        return element

    def mapper_support(self, feature: dict, feature_id: str) -> ET.Element:
        """Mappe RPD_Support_Reco"""
        props = feature["properties"]

        element = ET.Element(f"{{{NAMESPACE_RECOSTAR}}}RPD_Support_Reco")
        element.set(f"{{{NAMESPACE_GML}}}id", feature_id)

        # Référence reseau avec style multiline
        self._ajouter_references_reseau(element, props)

        # Commentaire (hérité d'ElementReseau, optionnel)
        self._ajouter_commentaire(element, props)

        # Référence géométrie supplémentaire si présente
        geom_supp = props.get("geometriesupplementaire_href")
        if geom_supp:
            self._ajouter_reference(element, "geometriesupplementaire", geom_supp)

        # Classe (optionnel)
        self._ajouter_reference(element, "Classe", props.get("Classe_href"))

        # Effort (optionnel)
        effort = props.get("Effort")
        if effort is not None:
            effort_uom = props.get("Effort_uom", "kN")
            self._ajouter_propriete(element, "Effort", effort, effort_uom)

        # Géométrie Point avec ID unique
        if feature.get("geometry"):
            geom_id = self._obtenir_id_geom_unique("RPD_Support_Reco", props.get("ogr_pkid", feature_id))
            point = self.geo_converter.point_vers_gml(feature["geometry"], geom_id)
            geom_elem = ET.SubElement(element, f"{{{NAMESPACE_RECOSTAR}}}Geometrie")
            geom_elem.append(point)

        # HauteurPoteau (optionnel)
        hauteur = props.get("HauteurPoteau")
        if hauteur is not None:
            hauteur_uom = props.get("HauteurPoteau_uom", "m")
            self._ajouter_propriete(element, "HauteurPoteau", hauteur, hauteur_uom)

        # Matiere (optionnel)
        self._ajouter_reference(element, "Matiere", props.get("Matiere_href"))

        # NatureSupport (requis)
        self._ajouter_reference(element, "NatureSupport", props.get("NatureSupport_href"))

        # Précisions
        self._ajouter_propriete(element, "PrecisionXY", props.get("PrecisionXY"))
        self._ajouter_propriete(element, "PrecisionZ", props.get("PrecisionZ"))

        # Statut (requis V1.10)
        self._ajouter_propriete(element, "Statut", props.get("Statut"))

        return element

    def mapper_fourreau(self, feature: dict, feature_id: str) -> ET.Element:
        """Mappe RPD_Fourreau_Reco"""
        props = feature["properties"]

        element = ET.Element(f"{{{NAMESPACE_RECOSTAR}}}RPD_Fourreau_Reco")
        element.set(f"{{{NAMESPACE_GML}}}id", feature_id)

        # ORDRE STRICT SELON XSD :
        # 1. reseau+
        self._ajouter_references_reseau(element, props)

        # Commentaire (hérité d'ElementReseau, optionnel)
        self._ajouter_commentaire(element, props)

        # 2. CoupeType? (optionnel)
        coupe_type = props.get("CoupeType")
        if coupe_type:
            self._ajouter_propriete(element, "CoupeType", coupe_type)

        # 3. DiametreDuFourreau (requis)
        diametre = props.get("DiametreDuFourreau")
        if diametre is not None:
            self._ajouter_propriete(
                element,
                "DiametreDuFourreau",
                diametre,
                props.get("DiametreDuFourreau_uom", "mm"),
            )

        # 4. EtatCoupeType? (optionnel)
        etat_coupe = props.get("EtatCoupeType")
        if etat_coupe:
            self._ajouter_propriete(element, "EtatCoupeType", etat_coupe)

        # 5. Geometrie (requis)
        if feature.get("geometry"):
            geom_id = self._obtenir_id_geom_unique("RPD_Fourreau_Reco", props.get("ogr_pkid", feature_id))
            linestring = self.geo_converter.ligne_vers_gml(feature["geometry"], geom_id)
            geom_elem = ET.SubElement(element, f"{{{NAMESPACE_RECOSTAR}}}Geometrie")
            geom_elem.append(linestring)

        # 6. Materiau (requis)
        self._ajouter_propriete(element, "Materiau", props.get("Materiau"))

        # 7. PrecisionXY (requis)
        self._ajouter_propriete(element, "PrecisionXY", props.get("PrecisionXY"))

        # 8. PrecisionZ (requis)
        self._ajouter_propriete(element, "PrecisionZ", props.get("PrecisionZ"))

        # 9. ProfondeurMinNonReg? (optionnel)
        profondeur = props.get("ProfondeurMinNonReg")
        if profondeur is not None:
            self._ajouter_propriete(
                element,
                "ProfondeurMinNonReg",
                profondeur,
                props.get("ProfondeurMinNonReg_uom", "m"),
            )

        return element

    def mapper_galerie(self, feature: dict, feature_id: str) -> ET.Element:
        """Mappe RPD_Galerie_Reco"""
        props = feature["properties"]

        element = ET.Element(f"{{{NAMESPACE_RECOSTAR}}}RPD_Galerie_Reco")
        element.set(f"{{{NAMESPACE_GML}}}id", feature_id)

        # ORDRE STRICT SELON XSD :
        # 1. reseau+ (hérité ElementReseauType)
        self._ajouter_references_reseau(element, props)

        # Commentaire (hérité d'ElementReseau, optionnel)
        self._ajouter_commentaire(element, props)

        # 2. Geometrie (requis, gml:CurvePropertyType → LineString)
        if feature.get("geometry"):
            geom_id = self._obtenir_id_geom_unique("RPD_Galerie_Reco", props.get("ogr_pkid", feature_id))
            linestring = self.geo_converter.ligne_vers_gml(feature["geometry"], geom_id)
            geom_elem = ET.SubElement(element, f"{{{NAMESPACE_RECOSTAR}}}Geometrie")
            geom_elem.append(linestring)

        # 3. Hauteur (requis, uom="m")
        hauteur = props.get("Hauteur")
        if hauteur is not None:
            self._ajouter_propriete(element, "Hauteur", hauteur, props.get("Hauteur_uom", "m"))

        # 4. Largeur (requis, uom="m")
        largeur = props.get("Largeur")
        if largeur is not None:
            self._ajouter_propriete(element, "Largeur", largeur, props.get("Largeur_uom", "m"))

        # 5. PrecisionXY (requis)
        self._ajouter_propriete(element, "PrecisionXY", props.get("PrecisionXY"))

        # 6. PrecisionZ (requis)
        self._ajouter_propriete(element, "PrecisionZ", props.get("PrecisionZ"))

        # 7. ProfondeurMinNonReg? (optionnel)
        profondeur = props.get("ProfondeurMinNonReg")
        if profondeur is not None:
            self._ajouter_propriete(
                element,
                "ProfondeurMinNonReg",
                profondeur,
                props.get("ProfondeurMinNonReg_uom", "m"),
            )

        return element

    def _normaliser_wkt_ligne(self, ligne_text: str) -> str | None:
        """Normalise un texte WKT en chaîne de coordonnées posList.

        Supprime le préfixe LINESTRING, extrait les coordonnées entre parenthèses,
        et normalise les séparateurs. Retourne None si les coordonnées sont invalides.
        """
        text_clean = ligne_text.strip()
        if text_clean.upper().startswith("LINESTRING"):
            idx_open = text_clean.index("(")
            text_clean = text_clean[idx_open + 1 :].rstrip(")")

        # "x1 y1 z1,x2 y2 z2" → "x1 y1 z1 x2 y2 z2"
        normalized = text_clean.replace(",", " ")

        # Validation : au moins un triplet 3D complet
        coords_list = normalized.split()
        if len(coords_list) < 3:
            return None

        return normalized

    def _ajouter_ligne_3d(self, element: ET.Element, ligne_text: str, ogr_pkid: str):
        """Ajoute un élément Ligne3D (LineString GML) à partir d'un texte WKT."""
        normalized_text = self._normaliser_wkt_ligne(ligne_text)
        if not normalized_text:
            return

        linestring = ET.Element(f"{{{NAMESPACE_GML}}}LineString")
        linestring.set("srsName", self.srs)
        linestring.set(f"{{{NAMESPACE_GML}}}id", f"{ogr_pkid}.geom_ligne0")

        pos_list = ET.SubElement(linestring, f"{{{NAMESPACE_GML}}}posList")
        pos_list.set("srsDimension", "3")
        pos_list.text = normalized_text

        ligne_elem = ET.SubElement(element, f"{{{NAMESPACE_RECOSTAR}}}Ligne3D")
        ligne_elem.append(linestring)

    def _extraire_ligne_de_geometry(self, geometry: dict) -> dict | None:
        """Extrait une LineString GeoJSON depuis n'importe quelle géométrie.

        - LineString : retournée telle quelle.
        - MultiLineString : prend la première ligne.
        - Polygon : prend l'anneau extérieur (déjà fermé).
        - MultiPolygon : prend l'anneau extérieur du premier polygone.
        Retourne None pour les types non gérés ou les géométries vides.
        """
        geom_type = geometry.get("type")
        coords = geometry.get("coordinates")
        if not coords:
            return None

        # Tableau de correspondance type → coordonnées de la ligne extraite.
        if geom_type == "LineString":
            ligne_coords = coords
        elif geom_type == "MultiLineString" or geom_type == "Polygon":
            ligne_coords = coords[0]
        elif geom_type == "MultiPolygon":
            ligne_coords = coords[0][0]
        else:
            return None

        if len(ligne_coords) < 2:
            return None

        return {"type": "LineString", "coordinates": ligne_coords}

    def _ajouter_ligne_3d_depuis_geometry(self, element: ET.Element, geometry: dict, ogr_pkid: str):
        """Ajoute un élément Ligne3D dérivé d'une géométrie GeoJSON ouverte."""
        ligne_geom = self._extraire_ligne_de_geometry(geometry)
        if ligne_geom is None:
            return

        geom_id = f"{ogr_pkid}.geom_ligne_auto0"
        linestring = self.geo_converter.ligne_vers_gml(ligne_geom, geom_id)
        ligne_elem = ET.SubElement(element, f"{{{NAMESPACE_RECOSTAR}}}Ligne3D")
        ligne_elem.append(linestring)

    def _ajouter_surface_3d(self, element: ET.Element, geometry: dict, ogr_pkid: str):
        """Ajoute un élément Surface3D (Polygon GML) à l'élément parent."""
        geom_type = geometry["type"]
        geom_id = f"{ogr_pkid}.geom_surface0"

        if geom_type == "Polygon":
            polygon = self.geo_converter.polygone_vers_gml(geometry, geom_id)
        elif geom_type == "MultiPolygon":
            polygon = self.geo_converter.multipolygone_vers_gml(geometry, geom_id)
        else:
            return

        if polygon is not None:
            surface_elem = ET.SubElement(element, f"{{{NAMESPACE_RECOSTAR}}}Surface3D")
            surface_elem.append(polygon)

    def _qualifier_geometrie_supp(self, geometry: dict, props: dict | None = None) -> str | None:
        """Détermine la nature XSD d'une géométrie supplémentaire.

        La nature reportée par la lecture du GML prime : une Ligne3D fermée —
        un rectangle d'emprise tracé comme une ligne — est indiscernable d'un
        contour une fois dans le GeoJSON, et la déduction la reclasserait à
        tort en surface.

        À défaut, règle métier : une forme fermée (premier point == dernier
        point sur l'anneau/ligne extrait) est qualifiée en Surface3D, toute
        autre forme en Ligne3D. Retourne None si la géométrie ne peut pas être
        qualifiée (type non géré ou coordonnées insuffisantes).
        """
        nature_declaree = (props or {}).get(CHAMP_NATURE_GEOMETRIE)
        if nature_declaree in (NATURE_LIGNE, NATURE_SURFACE):
            return nature_declaree

        ligne_geom = self._extraire_ligne_de_geometry(geometry)
        if ligne_geom is None:
            return None

        coords = ligne_geom["coordinates"]
        # Comparaison X/Y uniquement : Z peut différer sans rompre la fermeture
        # planimétrique (cas typique d'une emprise au sol).
        premier = coords[0]
        dernier = coords[-1]
        if len(coords) >= 4 and premier[0] == dernier[0] and premier[1] == dernier[1]:
            return "Surface3D"
        return "Ligne3D"

    def _construire_geometry_surfacique(self, geometry: dict) -> dict | None:
        """Convertit une géométrie qualifiée fermée en Polygon exploitable.

        Les types Polygon et MultiPolygon sont retournés tels quels. Une
        LineString fermée est convertie en Polygon mono-anneau pour produire
        un gml:Polygon conforme à gml:SurfacePropertyType.
        """
        geom_type = geometry.get("type")
        if geom_type in ("Polygon", "MultiPolygon"):
            return geometry

        ligne_geom = self._extraire_ligne_de_geometry(geometry)
        if ligne_geom is None:
            return None

        return {"type": "Polygon", "coordinates": [ligne_geom["coordinates"]]}

    def mapper_geometrie_supplementaire(self, feature: dict, feature_id: str) -> ET.Element:
        """Entité GeoJSON → RPD_GeometrieSupplementaire_Reco

        Ordre XSD strict : Commentaire? → Ligne3D* → PrecisionXY → PrecisionZ → Surface3D*

        La nature de la géométrie (Ligne3D ou Surface3D) est déduite
        automatiquement de l'analyse des coordonnées : une forme fermée
        produit une Surface3D, sinon une Ligne3D.
        """
        props = feature["properties"]

        element = ET.Element(f"{{{NAMESPACE_RECOSTAR}}}RPD_GeometrieSupplementaire_Reco")
        element.set(f"{{{NAMESPACE_GML}}}id", feature_id)

        ogr_pkid = props.get("ogr_pkid", feature_id)
        geometry = feature.get("geometry")

        # 1. Commentaire (optionnel)
        self._ajouter_commentaire(element, props)

        # 2. Ligne3D (optionnel)
        # 2.a. Compatibilité descendante : propriété WKT explicite.
        ligne_text = props.get("Ligne3D")
        if ligne_text:
            self._ajouter_ligne_3d(element, ligne_text, ogr_pkid)

        # 2.b. Géométrie qualifiée comme ligne (forme ouverte).
        # La qualification est calculée une seule fois et utilisée plus bas.
        nature = self._qualifier_geometrie_supp(geometry, props) if geometry else None
        if geometry and nature == "Ligne3D":
            self._ajouter_ligne_3d_depuis_geometry(element, geometry, ogr_pkid)

        # 3 et 4. PrecisionXY et PrecisionZ (obligatoires)
        self._ajouter_propriete(element, "PrecisionXY", props.get("PrecisionXY"))
        self._ajouter_propriete(element, "PrecisionZ", props.get("PrecisionZ"))

        # 5. Surface3D (optionnel) - APRÈS les précisions, géométrie qualifiée
        # comme surface (forme fermée).
        if geometry and nature == "Surface3D":
            geom_surface = self._construire_geometry_surfacique(geometry)
            if geom_surface is not None:
                self._ajouter_surface_3d(element, geom_surface, ogr_pkid)

        return element

    def mapper_jonction(self, feature: dict, feature_id: str) -> ET.Element:
        """Mappe RPD_Jonction_Reco avec prise en charge des propriétés optionnelles Aerien et angle"""
        props = feature["properties"]

        element = ET.Element(f"{{{NAMESPACE_RECOSTAR}}}RPD_Jonction_Reco")
        element.set(f"{{{NAMESPACE_GML}}}id", feature_id)

        # Référence reseau avec style multiline
        self._ajouter_references_reseau(element, props)

        # Commentaire (hérité d'ElementReseau, optionnel)
        self._ajouter_commentaire(element, props)

        # ORDRE XSD STRICT : conteneur, DomaineTension, Geometrie?,
        # PrecisionXY?, PrecisionZ?, Statut, TypeJonction, angle?

        # 1. conteneur (hérité de NoeudReseauType - optionnel)
        conteneur_href = props.get("conteneur_href")
        if conteneur_href:
            self._ajouter_reference(element, "conteneur", conteneur_href)

        # 2. DomaineTension (requis)
        self._ajouter_propriete(element, "DomaineTension", props.get("DomaineTension"))

        # 3. Geometrie (optionnel) - SEULEMENT si pas de conteneur
        # Selon XSD: "Les nœuds ne possèdent pas de géométrie (positionnement déduit des Conteneur),
        # sauf pour ceux qui ne possèdent pas de relation avec un Conteneur"
        if feature.get("geometry") and not conteneur_href:
            geom_id = self._obtenir_id_geom_unique("RPD_Jonction_Reco", props.get("ogr_pkid", feature_id))
            point = self.geo_converter.point_vers_gml(feature["geometry"], geom_id)
            geom_elem = ET.SubElement(element, f"{{{NAMESPACE_RECOSTAR}}}Geometrie")
            geom_elem.append(point)

        # 4-5. PrecisionXY/Z (optionnels)
        self._ajouter_propriete(element, "PrecisionXY", props.get("PrecisionXY"))
        self._ajouter_propriete(element, "PrecisionZ", props.get("PrecisionZ"))

        # 6. Statut (requis)
        self._ajouter_propriete(element, "Statut", props.get("Statut"))

        # 7. TypeJonction (requis)
        self._ajouter_propriete(element, "TypeJonction", props.get("TypeJonction"))

        # 8. angle (optionnel)
        angle = props.get("angle")
        if angle is not None:
            self._ajouter_propriete(element, "angle", angle)

        return element

    def mapper_coupe_circuit_a_fusibles(self, feature: dict, feature_id: str) -> ET.Element:
        """Mappe RPD_CoupeCircuitAFusibles_Reco"""
        props = feature["properties"]

        element = ET.Element(f"{{{NAMESPACE_RECOSTAR}}}RPD_CoupeCircuitAFusibles_Reco")
        element.set(f"{{{NAMESPACE_GML}}}id", feature_id)

        # Référence reseau avec style multiline
        self._ajouter_references_reseau(element, props)

        # Commentaire (hérité d'ElementReseau, optionnel)
        self._ajouter_commentaire(element, props)

        # Référence conteneur (optionnel selon XSD - NoeudReseauType)
        conteneur_href = props.get("conteneur_href")
        if conteneur_href:
            self._ajouter_reference(element, "conteneur", conteneur_href)

        # Propriété obligatoire : Statut
        self._ajouter_propriete(element, "Statut", props.get("Statut"))

        return element

    def mapper_point_comptage(self, feature: dict, feature_id: str) -> ET.Element:
        """Mappe RPD_PointDeComptage_Reco"""
        props = feature["properties"]

        element = ET.Element(f"{{{NAMESPACE_RECOSTAR}}}RPD_PointDeComptage_Reco")
        element.set(f"{{{NAMESPACE_GML}}}id", feature_id)

        # Référence reseau avec style multiline
        self._ajouter_references_reseau(element, props)

        # Commentaire (hérité d'ElementReseau, optionnel)
        self._ajouter_commentaire(element, props)

        # Référence conteneur
        conteneur_href = props.get("conteneur_href")
        if conteneur_href:
            self._ajouter_reference(element, "conteneur", conteneur_href)

        # Géométrie avec ID unique
        if feature.get("geometry"):
            geom_id = self._obtenir_id_geom_unique("RPD_PointDeComptage_Reco", props.get("ogr_pkid", feature_id))
            point = self.geo_converter.point_vers_gml(feature["geometry"], geom_id)
            geom_elem = ET.SubElement(element, f"{{{NAMESPACE_RECOSTAR}}}Geometrie")
            geom_elem.append(point)

        # NumeroPRM (optionnel)
        self._ajouter_propriete(element, "NumeroPRM", props.get("NumeroPRM"))

        self._ajouter_propriete(element, "PrecisionXY", props.get("PrecisionXY"))
        self._ajouter_propriete(element, "PrecisionZ", props.get("PrecisionZ"))
        self._ajouter_propriete(element, "Statut", props.get("Statut"))

        return element

    def mapper_point_leve(self, feature: dict, feature_id: str) -> ET.Element:
        """Mappe RPD_PointLeveOuvrageReseau_Reco conforme au modèle V1.10.

        Ordre XSD V1.10 : ChargeGeneratrice?, Geometrie, Horodatage?,
        NumeroPoint, PrecisionXYnum, PrecisionZnum, Producteur.
        Les anciens attributs Leve et TypeLeve ne sont plus produits.
        """
        props = feature["properties"]

        element = ET.Element(f"{{{NAMESPACE_RECOSTAR}}}RPD_PointLeveOuvrageReseau_Reco")
        element.set(f"{{{NAMESPACE_GML}}}id", feature_id)

        # ChargeGeneratrice (optionnel, MeasureType avec uom)
        charge = props.get("ChargeGeneratrice")
        if charge is not None:
            self._ajouter_propriete(
                element,
                "ChargeGeneratrice",
                charge,
                props.get("ChargeGeneratrice_uom", "m"),
            )

        # Géométrie Point (obligatoire)
        if feature.get("geometry"):
            geom_id = self._obtenir_id_geom_unique("RPD_PointLeveOuvrageReseau_Reco", props.get("ogr_pkid", feature_id))
            point = self.geo_converter.point_vers_gml(feature["geometry"], geom_id)
            geom_elem = ET.SubElement(element, f"{{{NAMESPACE_RECOSTAR}}}Geometrie")
            geom_elem.append(point)

        # Horodatage (optionnel, xs:date)
        self._ajouter_propriete(element, "Horodatage", props.get("Horodatage"))

        # Propriétés obligatoires
        self._ajouter_propriete(element, "NumeroPoint", props.get("NumeroPoint"))
        self._ajouter_propriete(element, "PrecisionXYnum", props.get("PrecisionXYnum"))
        self._ajouter_propriete(element, "PrecisionZnum", props.get("PrecisionZnum"))
        self._ajouter_propriete(element, "Producteur", props.get("Producteur"))

        return element

    def mapper_materiel(self, feature: dict, feature_id: str) -> ET.Element:
        """Mappe RPD_Materiel_Reco"""
        props = feature["properties"]

        element = ET.Element(f"{{{NAMESPACE_RECOSTAR}}}RPD_Materiel_Reco")
        element.set(f"{{{NAMESPACE_GML}}}id", feature_id)

        self._ajouter_propriete(element, "Fabricant", props.get("Fabricant"))
        self._ajouter_propriete(element, "Modele", props.get("Modele"))
        self._ajouter_propriete(element, "NumeroLot", props.get("NumeroLot"))
        self._ajouter_propriete(element, "NumeroSerie", props.get("NumeroSerie"))

        return element

    def mapper_aerien(self, feature: dict, feature_id: str) -> ET.Element:
        """Mappe RPD_Aerien_Reco"""
        props = feature["properties"]

        element = ET.Element(f"{{{NAMESPACE_RECOSTAR}}}RPD_Aerien_Reco")
        element.set(f"{{{NAMESPACE_GML}}}id", feature_id)

        # Référence reseau avec style multiline
        self._ajouter_references_reseau(element, props)

        # Commentaire (hérité d'ElementReseau, optionnel)
        self._ajouter_commentaire(element, props)

        # Géométrie LineString avec ID unique
        if feature.get("geometry"):
            geom_id = self._obtenir_id_geom_unique("RPD_Aerien_Reco", props.get("ogr_pkid", feature_id))
            linestring = self.geo_converter.ligne_vers_gml(feature["geometry"], geom_id)
            geom_elem = ET.SubElement(element, f"{{{NAMESPACE_RECOSTAR}}}Geometrie")
            geom_elem.append(linestring)

        # ModePose (requis)
        self._ajouter_propriete(element, "ModePose", props.get("ModePose"))

        # PrecisionXY (requis)
        self._ajouter_propriete(element, "PrecisionXY", props.get("PrecisionXY"))

        # PrecisionZ (requis)
        self._ajouter_propriete(element, "PrecisionZ", props.get("PrecisionZ"))

        return element

    def mapper_pleine_terre(self, feature: dict, feature_id: str) -> ET.Element:
        """Mappe RPD_PleineTerre_Reco"""
        props = feature["properties"]

        element = ET.Element(f"{{{NAMESPACE_RECOSTAR}}}RPD_PleineTerre_Reco")
        element.set(f"{{{NAMESPACE_GML}}}id", feature_id)

        # Référence reseau avec style multiline
        self._ajouter_references_reseau(element, props)

        # Commentaire (hérité d'ElementReseau, optionnel)
        self._ajouter_commentaire(element, props)

        # CoupeType (optionnel)
        coupe_type = props.get("CoupeType")
        if coupe_type:
            self._ajouter_propriete(element, "CoupeType", coupe_type)

        # EtatCoupeType (optionnel)
        etat_coupe = props.get("EtatCoupeType")
        if etat_coupe:
            self._ajouter_propriete(element, "EtatCoupeType", etat_coupe)

        # Géométrie LineString avec ID unique
        if feature.get("geometry"):
            geom_id = self._obtenir_id_geom_unique("RPD_PleineTerre_Reco", props.get("ogr_pkid", feature_id))
            linestring = self.geo_converter.ligne_vers_gml(feature["geometry"], geom_id)
            geom_elem = ET.SubElement(element, f"{{{NAMESPACE_RECOSTAR}}}Geometrie")
            geom_elem.append(linestring)

        self._ajouter_propriete(element, "PrecisionXY", props.get("PrecisionXY"))
        self._ajouter_propriete(element, "PrecisionZ", props.get("PrecisionZ"))

        # ProfondeurMinNonReg (optionnel)
        profondeur = props.get("ProfondeurMinNonReg")
        if profondeur is not None:
            self._ajouter_propriete(
                element,
                "ProfondeurMinNonReg",
                profondeur,
                props.get("ProfondeurMinNonReg_uom", "m"),
            )

        return element

    def mapper_batiment_technique(self, feature: dict, feature_id: str) -> ET.Element:
        """Mappe RPD_BatimentTechnique_Reco"""
        props = feature["properties"]

        element = ET.Element(f"{{{NAMESPACE_RECOSTAR}}}RPD_BatimentTechnique_Reco")
        element.set(f"{{{NAMESPACE_GML}}}id", feature_id)

        # Référence reseau
        self._ajouter_references_reseau(element, props)

        # Commentaire (hérité d'ElementReseau, optionnel)
        self._ajouter_commentaire(element, props)

        # Référence géométrie supplémentaire si présente
        geom_supp = props.get("geometriesupplementaire_href")
        if geom_supp:
            self._ajouter_reference(element, "geometriesupplementaire", geom_supp)

        # Géométrie Point (requis)
        if feature.get("geometry"):
            geom_id = self._obtenir_id_geom_unique("RPD_BatimentTechnique_Reco", props.get("ogr_pkid", feature_id))
            point = self.geo_converter.point_vers_gml(feature["geometry"], geom_id)
            geom_elem = ET.SubElement(element, f"{{{NAMESPACE_RECOSTAR}}}Geometrie")
            geom_elem.append(point)

        # PrecisionXY (requis)
        self._ajouter_propriete(element, "PrecisionXY", props.get("PrecisionXY"))

        # PrecisionZ (requis)
        self._ajouter_propriete(element, "PrecisionZ", props.get("PrecisionZ"))

        # Statut (requis V1.10)
        self._ajouter_propriete(element, "Statut", props.get("Statut"))

        return element

    def mapper_enceinte_cloturee(self, feature: dict, feature_id: str) -> ET.Element:
        """Mappe RPD_EnceinteCloturee_Reco"""
        props = feature["properties"]

        element = ET.Element(f"{{{NAMESPACE_RECOSTAR}}}RPD_EnceinteCloturee_Reco")
        element.set(f"{{{NAMESPACE_GML}}}id", feature_id)

        # Référence reseau
        self._ajouter_references_reseau(element, props)

        # Commentaire (hérité d'ElementReseau, optionnel)
        self._ajouter_commentaire(element, props)

        # Référence géométrie supplémentaire si présente
        geom_supp = props.get("geometriesupplementaire_href")
        if geom_supp:
            self._ajouter_reference(element, "geometriesupplementaire", geom_supp)

        # Géométrie Point (requis)
        if feature.get("geometry"):
            geom_id = self._obtenir_id_geom_unique("RPD_EnceinteCloturee_Reco", props.get("ogr_pkid", feature_id))
            point = self.geo_converter.point_vers_gml(feature["geometry"], geom_id)
            geom_elem = ET.SubElement(element, f"{{{NAMESPACE_RECOSTAR}}}Geometrie")
            geom_elem.append(point)

        # PrecisionXY (requis)
        self._ajouter_propriete(element, "PrecisionXY", props.get("PrecisionXY"))

        # PrecisionZ (requis)
        self._ajouter_propriete(element, "PrecisionZ", props.get("PrecisionZ"))

        # Statut (requis V1.10)
        self._ajouter_propriete(element, "Statut", props.get("Statut"))

        return element

    def mapper_poste_electrique(self, feature: dict, feature_id: str) -> ET.Element:
        """Mappe RPD_PosteElectrique_Reco"""
        props = feature["properties"]

        element = ET.Element(f"{{{NAMESPACE_RECOSTAR}}}RPD_PosteElectrique_Reco")
        element.set(f"{{{NAMESPACE_GML}}}id", feature_id)

        # Référence reseau
        self._ajouter_references_reseau(element, props)

        # Commentaire (hérité d'ElementReseau, optionnel)
        self._ajouter_commentaire(element, props)

        # ORDRE IMPORTANT: conteneur DOIT être AVANT les propriétés spécifiques(NoeudReseauType avant RPD_PosteElectrique_RecoType)
        # conteneur_href (hérité de NoeudReseauType)
        conteneur_href = props.get("conteneur_href")
        if conteneur_href:
            self._ajouter_reference(element, "conteneur", conteneur_href)

        # Categorie (requis, xlink:href)
        self._ajouter_reference(element, "Categorie", props.get("Categorie_href"))

        # Code (requis, string)
        self._ajouter_propriete(element, "Code", props.get("Code"))

        # InformationSupplementaire (requis, string)
        self._ajouter_propriete(element, "InformationSupplementaire", props.get("InformationSupplementaire"))

        # Statut (requis)
        self._ajouter_propriete(element, "Statut", props.get("Statut"))

        # TypePoste (requis, xlink:href)
        self._ajouter_reference(element, "TypePoste", props.get("TypePoste_href"))

        # NOTE: PrecisionXY et PrecisionZ n'existent PAS dans RPD_PosteElectrique_RecoType XSD

        return element

    def mapper_protection_mecanique(self, feature: dict, feature_id: str) -> ET.Element:
        """Mappe RPD_ProtectionMecanique_Reco"""
        props = feature["properties"]

        element = ET.Element(f"{{{NAMESPACE_RECOSTAR}}}RPD_ProtectionMecanique_Reco")
        element.set(f"{{{NAMESPACE_GML}}}id", feature_id)

        # Référence reseau
        self._ajouter_references_reseau(element, props)

        # Commentaire (hérité d'ElementReseau, optionnel)
        self._ajouter_commentaire(element, props)

        # CoupeType (optionnel)
        coupe_type = props.get("CoupeType")
        if coupe_type:
            self._ajouter_propriete(element, "CoupeType", coupe_type)

        # EtatCoupeType (optionnel)
        etat_coupe = props.get("EtatCoupeType")
        if etat_coupe:
            self._ajouter_propriete(element, "EtatCoupeType", etat_coupe)

        # Geometrie (requis, LineString)
        if feature.get("geometry"):
            geom_id = self._obtenir_id_geom_unique("RPD_ProtectionMecanique_Reco", props.get("ogr_pkid", feature_id))
            linestring = self.geo_converter.ligne_vers_gml(feature["geometry"], geom_id)
            geom_elem = ET.SubElement(element, f"{{{NAMESPACE_RECOSTAR}}}Geometrie")
            geom_elem.append(linestring)

        # Materiau (requis)
        self._ajouter_propriete(element, "Materiau", props.get("Materiau"))

        # PrecisionXY (requis)
        self._ajouter_propriete(element, "PrecisionXY", props.get("PrecisionXY"))

        # PrecisionZ (requis)
        self._ajouter_propriete(element, "PrecisionZ", props.get("PrecisionZ"))

        # ProfondeurMinNonReg (optionnel)
        profondeur = props.get("ProfondeurMinNonReg")
        if profondeur is not None:
            self._ajouter_propriete(
                element,
                "ProfondeurMinNonReg",
                profondeur,
                props.get("ProfondeurMinNonReg_uom", "m"),
            )

        return element

    def mapper_jeu_barres(self, feature: dict, feature_id: str) -> ET.Element:
        """Mappe RPD_JeuBarres_Reco"""
        props = feature["properties"]

        element = ET.Element(f"{{{NAMESPACE_RECOSTAR}}}RPD_JeuBarres_Reco")
        element.set(f"{{{NAMESPACE_GML}}}id", feature_id)

        # Référence reseau
        self._ajouter_references_reseau(element, props)

        # Commentaire (hérité d'ElementReseau, optionnel)
        self._ajouter_commentaire(element, props)

        # conteneur_href (optionnel)
        conteneur_href = props.get("conteneur_href")
        if conteneur_href:
            self._ajouter_reference(element, "conteneur", conteneur_href)

        # Statut (requis)
        self._ajouter_propriete(element, "Statut", props.get("Statut"))

        return element

    def mapper_support_modules(self, feature: dict, feature_id: str) -> ET.Element:
        """Mappe RPD_SupportModules_Reco"""
        props = feature["properties"]

        element = ET.Element(f"{{{NAMESPACE_RECOSTAR}}}RPD_SupportModules_Reco")
        element.set(f"{{{NAMESPACE_GML}}}id", feature_id)

        # Référence reseau
        self._ajouter_references_reseau(element, props)

        # Commentaire (hérité d'ElementReseau, optionnel)
        self._ajouter_commentaire(element, props)

        # conteneur_href (optionnel)
        conteneur_href = props.get("conteneur_href")
        if conteneur_href:
            self._ajouter_reference(element, "conteneur", conteneur_href)

        # Statut (requis)
        self._ajouter_propriete(element, "Statut", props.get("Statut"))

        return element

    def mapper_terre(self, feature: dict, feature_id: str) -> ET.Element:
        """Mappe RPD_Terre_Reco"""
        props = feature["properties"]

        element = ET.Element(f"{{{NAMESPACE_RECOSTAR}}}RPD_Terre_Reco")
        element.set(f"{{{NAMESPACE_GML}}}id", feature_id)

        # Référence reseau
        self._ajouter_references_reseau(element, props)

        # Commentaire (hérité d'ElementReseau, optionnel)
        self._ajouter_commentaire(element, props)

        # conteneur_href (optionnel)
        conteneur_href = props.get("conteneur_href")
        if conteneur_href:
            self._ajouter_reference(element, "conteneur", conteneur_href)

        # NatureTerre (requis, xlink:href)
        self._ajouter_reference(element, "NatureTerre", props.get("NatureTerre_href"))

        # Resistance (optionnel avec unité)
        resistance = props.get("Resistance")
        if resistance is not None:
            self._ajouter_propriete(element, "Resistance", resistance, props.get("Resistance_uom", "Ohm"))

        # Statut (requis)
        self._ajouter_propriete(element, "Statut", props.get("Statut"))

        return element

    def mapper_module_raccordement(self, feature: dict, feature_id: str) -> ET.Element:
        """Mappe RPD_ModuleRaccordement_Reco

        Ordre XSD strict V1.1 (sequence ElementReseau → Ouvrage → NoeudReseau
        → Plage → ModuleRaccordement) :
        reseau, Commentaire?, conteneur?, Coupure, NbPlagesOccupees,
        noeudParent, Protection.
        Pas de géométrie propre (position déduite du conteneur).
        """
        props = feature["properties"]

        element = ET.Element(f"{{{NAMESPACE_RECOSTAR}}}RPD_ModuleRaccordement_Reco")
        element.set(f"{{{NAMESPACE_GML}}}id", feature_id)

        # 1. reseau (hérité ElementReseauType, requis)
        self._ajouter_references_reseau(element, props)

        # 2. Commentaire (hérité ElementReseauType, optionnel V1.1)
        self._ajouter_commentaire(element, props)

        # 3. conteneur (hérité NoeudReseauType, optionnel)
        conteneur_href = props.get("conteneur_href")
        if conteneur_href:
            self._ajouter_reference(element, "conteneur", conteneur_href)

        # 4. Coupure (bool, requis)
        self._ajouter_propriete(element, "Coupure", props.get("Coupure"))

        # 5. NbPlagesOccupees (int, requis)
        self._ajouter_propriete(element, "NbPlagesOccupees", props.get("NbPlagesOccupees"))

        # 6. noeudParent (référence RPD_SupportModules_Reco, requis)
        self._ajouter_reference(element, "noeudParent", props.get("noeudParent_href"))

        # 7. Protection (bool, requis)
        self._ajouter_propriete(element, "Protection", props.get("Protection"))

        return element

    def mapper_ouvrage_collectif_branchement(self, feature: dict, feature_id: str) -> ET.Element:
        """Mappe RPD_OuvrageCollectifBranchement_Reco"""
        props = feature["properties"]

        element = ET.Element(f"{{{NAMESPACE_RECOSTAR}}}RPD_OuvrageCollectifBranchement_Reco")
        element.set(f"{{{NAMESPACE_GML}}}id", feature_id)

        # Référence reseau
        self._ajouter_references_reseau(element, props)

        # Commentaire (hérité d'ElementReseau, optionnel)
        self._ajouter_commentaire(element, props)

        # conteneur_href (optionnel)
        conteneur_href = props.get("conteneur_href")
        if conteneur_href:
            self._ajouter_reference(element, "conteneur", conteneur_href)

        # Geometrie (optionnelle, Point)
        if feature.get("geometry") and not conteneur_href:
            geom_id = self._obtenir_id_geom_unique(
                "RPD_OuvrageCollectifBranchement_Reco",
                props.get("ogr_pkid", feature_id),
            )
            point = self.geo_converter.point_vers_gml(feature["geometry"], geom_id)
            geom_elem = ET.SubElement(element, f"{{{NAMESPACE_RECOSTAR}}}Geometrie")
            geom_elem.append(point)

        # PrecisionXY (optionnel)
        precision_xy = props.get("PrecisionXY")
        if precision_xy:
            self._ajouter_propriete(element, "PrecisionXY", precision_xy)

        # PrecisionZ (optionnel)
        precision_z = props.get("PrecisionZ")
        if precision_z:
            self._ajouter_propriete(element, "PrecisionZ", precision_z)

        # Statut (requis)
        self._ajouter_propriete(element, "Statut", props.get("Statut"))

        return element


class RemappeurIds:
    """Remplace tous les gml:id et les xlink:href associés par des UUIDs préfixés 'id'.

    Fonctionne en trois passes sur l'arbre XML complet :
    1. Construction de la table de correspondance ancien_id → nouvel_id
    2. Remplacement des attributs gml:id
    3. Mise à jour des xlink:href référençant un identifiant renommé
    """

    __slots__ = ("_table",)

    def __init__(self):
        self._table: dict[str, str] = {}

    @staticmethod
    def _nouvel_id() -> str:
        """Génère un identifiant UUID au format 'id{uuid4}'."""
        return f"id{uuid.uuid4()}"

    def _construire_table(self, root: ET.Element) -> None:
        """Parcourt l'arbre et génère un UUID pour chaque gml:id rencontré."""
        gml_id_attr = f"{{{NAMESPACE_GML}}}id"
        table = self._table
        # Cache de la référence de fonction pour éviter la résolution répétée en boucle
        gen_id = self._nouvel_id
        for elem in root.iter():
            old_id = elem.get(gml_id_attr)
            if old_id and old_id not in table:
                table[old_id] = gen_id()

    def _appliquer_gml_ids(self, root: ET.Element) -> None:
        """Remplace chaque gml:id par son équivalent UUID depuis la table."""
        gml_id_attr = f"{{{NAMESPACE_GML}}}id"
        table = self._table
        for elem in root.iter():
            old_id = elem.get(gml_id_attr)
            if old_id:
                elem.set(gml_id_attr, table[old_id])

    def _appliquer_hrefs(self, root: ET.Element) -> None:
        """Met à jour les xlink:href référençant un identifiant renommé.

        Gère les deux formes : href="old_id" (direct) et href="#old_id" (fragment local).
        Les hrefs vers des ressources externes ne figurant pas dans la table
        sont laissés intacts.
        """
        xlink_href_attr = f"{{{NAMESPACE_XLINK}}}href"
        table = self._table
        for elem in root.iter():
            href = elem.get(xlink_href_attr)
            if not href:
                continue
            if href.startswith("#"):
                ref_id = href[1:]
                if ref_id in table:
                    elem.set(xlink_href_attr, f"#{table[ref_id]}")
            elif href in table:
                elem.set(xlink_href_attr, table[href])

    def remanier(self, root: ET.Element) -> None:
        """Remanie l'ensemble des identifiants de l'arbre XML en UUIDs.

        Les trois passes sont exécutées dans l'ordre pour garantir la cohérence
        entre les gml:id renommés et les xlink:href mis à jour.
        """
        self._construire_table(root)
        self._appliquer_gml_ids(root)
        self._appliquer_hrefs(root)


class GenerateurGML:
    """Générateur GML principal avec optimisations de performances"""

    __slots__ = ("mapper", "srs", "metadata", "_metadata_chargees")

    def __init__(self, srs: str = DEFAULT_SRS, commentaire_vide: bool = False):
        self.mapper = MappeurEntites(srs, commentaire_vide=commentaire_vide)
        self.srs = srs
        self.metadata = {}
        self._metadata_chargees: dict = {}

    def definir_metadonnees(
        self,
        logiciel: str | None = None,
        producteur: str | None = None,
        responsable: str | None = None,
        nom: str | None = None,
        srs: str = DEFAULT_SRS,
    ):
        """Configure les métadonnées du GML.

        Trois niveaux, du plus prioritaire au moins prioritaire : la valeur
        passée en ligne de commande, celle conservée dans _metadata.json depuis
        le GML source, puis le défaut historique. Un argument à None signifie
        « non renseigné », ce qui laisse jouer les deux niveaux suivants.
        """
        # Fallback depuis _metadata.json chargé
        meta_chargees = self._metadata_chargees.get("Metadata", {})
        reseaux_charges = self._normaliser_reseaux(self._metadata_chargees.get("ReseauUtilite"))
        # Le premier réseau alimente les champs scalaires (Nom du réseau) et
        # sert de repli aux entités dépourvues de rattachement.
        reseau_charge = reseaux_charges[0] if reseaux_charges else {}

        self.metadata = {
            # Priorité : valeur explicite en ligne de commande, puis valeur
            # conservée du GML source, puis défaut historique.
            "logiciel": logiciel or meta_chargees.get("Logiciel") or LOGICIEL_DEFAUT,
            "producteur": producteur or meta_chargees.get("Producteur") or VALEUR_DEFAUT,
            "responsable": responsable or meta_chargees.get("Responsable") or VALEUR_DEFAUT,
            "nom": nom or reseau_charge.get("Nom") or VALEUR_DEFAUT,
            "srs": srs or meta_chargees.get("SRS", DEFAULT_SRS),
            "date": meta_chargees.get("Datecreation", datetime.now().strftime("%Y-%m-%d")),
            "version_specification": meta_chargees.get("VersionSpecification", ""),
            # Données réseau chargées (pour round-trip fidèle)
            "reseau_id": reseau_charge.get("id", RESEAU_DEFAUT),
            "reseau_mention": reseau_charge.get("Mention", ""),
            "reseau_theme": reseau_charge.get("Theme", THEME_DEFAUT),
            "reseaux": reseaux_charges,
        }
        self.srs = srs
        self.mapper.srs = srs
        self.mapper.geo_converter.srs = srs
        # Repli des entités sans rattachement : les réseaux réellement émis.
        self.mapper.reseaux_defaut = [reseau.get("id", RESEAU_DEFAUT) for reseau in reseaux_charges] or [RESEAU_DEFAUT]

    def _creer_membre_metadonnees(self) -> ET.Element:
        """Crée l'élément Metadata (conforme au XSD MetadataType)"""
        member = ET.Element(f"{{{NAMESPACE_GML}}}featureMember")

        metadata_id = f"id{uuid.uuid4()}"

        metadata = ET.SubElement(member, f"{{{NAMESPACE_RECOSTAR}}}Metadata")
        metadata.set(f"{{{NAMESPACE_GML}}}id", metadata_id)

        date_elem = ET.SubElement(metadata, f"{{{NAMESPACE_RECOSTAR}}}Datecreation")
        date_elem.text = self.metadata.get("date", datetime.now().strftime("%Y-%m-%d"))

        logiciel_elem = ET.SubElement(metadata, f"{{{NAMESPACE_RECOSTAR}}}Logiciel")
        logiciel_elem.text = self.metadata.get("logiciel", "")

        prod_elem = ET.SubElement(metadata, f"{{{NAMESPACE_RECOSTAR}}}Producteur")
        prod_elem.text = self.metadata.get("producteur", "")

        resp_elem = ET.SubElement(metadata, f"{{{NAMESPACE_RECOSTAR}}}Responsable")
        resp_elem.text = self.metadata.get("responsable", "")

        srs_elem = ET.SubElement(metadata, f"{{{NAMESPACE_RECOSTAR}}}SRS")
        srs_elem.text = self.metadata.get("srs", self.srs)

        # VersionSpecification (optionnel, conservé depuis le GML source)
        version_spec = self.metadata.get("version_specification", "")
        if version_spec:
            vs_elem = ET.SubElement(metadata, f"{{{NAMESPACE_RECOSTAR}}}VersionSpecification")
            vs_elem.text = version_spec

        return member

    def _creer_membres_reseau(self) -> list[ET.Element]:
        """Crée un featureMember par ReseauUtilite du fichier.

        Les réseaux conservés dans _metadata.json sont restitués tels quels,
        ce qui préserve les fichiers portant plusieurs tranches de travaux.
        Sans métadonnées chargées, un réseau unique est reconstruit depuis les
        valeurs fournies en ligne de commande.
        """
        reseaux = self.metadata.get("reseaux") or [
            {
                "id": self.metadata.get("reseau_id", RESEAU_DEFAUT),
                "Mention": self.metadata.get("reseau_mention", ""),
                "Nom": self.metadata.get("nom", ""),
                "Responsable": self.metadata.get("responsable", ""),
                "Theme": self.metadata.get("reseau_theme", THEME_DEFAUT),
            }
        ]
        return [self._creer_membre_reseau(reseau) for reseau in reseaux]

    def _creer_membre_reseau(self, reseau: dict) -> ET.Element:
        """Crée un élément ReseauUtilite (conforme au XSD ReseauUtiliteType).

        Ordre XSD strict : Mention, Nom, Responsable, Theme. Le Responsable
        est celui du réseau, distinct de celui du bloc Metadata.
        """
        member = ET.Element(f"{{{NAMESPACE_GML}}}featureMember")

        element = ET.SubElement(member, f"{{{NAMESPACE_RECOSTAR}}}ReseauUtilite")
        element.set(f"{{{NAMESPACE_GML}}}id", reseau.get("id") or RESEAU_DEFAUT)

        mention = ET.SubElement(element, f"{{{NAMESPACE_RECOSTAR}}}Mention")
        mention.text = reseau.get("Mention") or MENTION_DEFAUT

        nom_elem = ET.SubElement(element, f"{{{NAMESPACE_RECOSTAR}}}Nom")
        nom_elem.text = reseau.get("Nom") or self.metadata.get("nom", "")

        resp_elem = ET.SubElement(element, f"{{{NAMESPACE_RECOSTAR}}}Responsable")
        resp_elem.text = reseau.get("Responsable") or self.metadata.get("responsable", "")

        theme_elem = ET.SubElement(element, f"{{{NAMESPACE_RECOSTAR}}}Theme")
        theme_elem.text = reseau.get("Theme") or THEME_DEFAUT

        return member

    def _creer_relation_cable_noeud(self, cable_id: str, noeud_id: str, etat: str = "") -> ET.Element:
        """Crée une relation CableElectrique_NoeudReseau (ordre XSD respecté)."""
        member = ET.Element(f"{{{NAMESPACE_GML}}}featureMember")

        relation_id = f"id{uuid.uuid4()}"

        relation = ET.SubElement(member, f"{{{NAMESPACE_RECOSTAR}}}CableElectrique_NoeudReseau")
        relation.set(f"{{{NAMESPACE_GML}}}id", relation_id)

        # Ordre XSD : cableelectrique, EtatAvantRaccordement, noeudreseau
        cable_elem = ET.SubElement(relation, f"{{{NAMESPACE_RECOSTAR}}}cableelectrique")
        cable_elem.set(f"{{{NAMESPACE_XLINK}}}href", cable_id)

        if etat:
            etat_elem = ET.SubElement(relation, f"{{{NAMESPACE_RECOSTAR}}}EtatAvantRaccordement")
            etat_elem.text = etat

        noeud_elem = ET.SubElement(relation, f"{{{NAMESPACE_RECOSTAR}}}noeudreseau")
        noeud_elem.set(f"{{{NAMESPACE_XLINK}}}href", noeud_id)

        return member

    def _creer_relation_cheminement_cable(self, cable_id: str, cheminement_id: str) -> ET.Element:
        """Crée une relation Cheminement_Cables"""
        member = ET.Element(f"{{{NAMESPACE_GML}}}featureMember")

        relation_id = f"id{uuid.uuid4()}"

        relation = ET.SubElement(member, f"{{{NAMESPACE_RECOSTAR}}}Cheminement_Cables")
        relation.set(f"{{{NAMESPACE_GML}}}id", relation_id)

        cables_elem = ET.SubElement(relation, f"{{{NAMESPACE_RECOSTAR}}}cables")
        cables_elem.set(f"{{{NAMESPACE_XLINK}}}href", cable_id)

        chemin_elem = ET.SubElement(relation, f"{{{NAMESPACE_RECOSTAR}}}cheminement")
        chemin_elem.set(f"{{{NAMESPACE_XLINK}}}href", cheminement_id)

        return member

    def _creer_relation_ouvrage_materiel(self, ouvrage_id: str, materiel_id: str) -> ET.Element:
        """Crée une relation Ouvrage_Materiel"""
        member = ET.Element(f"{{{NAMESPACE_GML}}}featureMember")

        relation_id = f"id{uuid.uuid4()}"

        relation = ET.SubElement(member, f"{{{NAMESPACE_RECOSTAR}}}Ouvrage_Materiel")
        relation.set(f"{{{NAMESPACE_GML}}}id", relation_id)

        # Ordre XSD : materiel, ouvrage
        materiel_elem = ET.SubElement(relation, f"{{{NAMESPACE_RECOSTAR}}}materiel")
        materiel_elem.set(f"{{{NAMESPACE_XLINK}}}href", materiel_id)

        ouvrage_elem = ET.SubElement(relation, f"{{{NAMESPACE_RECOSTAR}}}ouvrage")
        ouvrage_elem.set(f"{{{NAMESPACE_XLINK}}}href", ouvrage_id)

        return member

    def _creer_relation_aerien_support(self, aerien_id: str, support_id: str) -> ET.Element:
        """Crée une relation Aerien_Support"""
        member = ET.Element(f"{{{NAMESPACE_GML}}}featureMember")

        relation_id = f"id{uuid.uuid4()}"

        relation = ET.SubElement(member, f"{{{NAMESPACE_RECOSTAR}}}Aerien_Support")
        relation.set(f"{{{NAMESPACE_GML}}}id", relation_id)

        # Ordre XSD : aerien, support
        aerien_elem = ET.SubElement(relation, f"{{{NAMESPACE_RECOSTAR}}}aerien")
        aerien_elem.set(f"{{{NAMESPACE_XLINK}}}href", aerien_id)

        support_elem = ET.SubElement(relation, f"{{{NAMESPACE_RECOSTAR}}}support")
        support_elem.set(f"{{{NAMESPACE_XLINK}}}href", support_id)

        return member

    def _extraire_relations_depuis_entites(self, features: list) -> list:
        """Extrait les paires (cable_id, id_entite) depuis une liste d'entités."""
        result = []
        for feat in features:
            props = feat.get("properties", {})
            feat_id = props.get("id")
            cable_href = props.get("cables_href")
            if not (feat_id and cable_href):
                continue
            for cid in analyser_ids_references(cable_href):
                result.append((cid, feat_id))
        return result

    def _extraire_relations_cable(self, features_by_type: dict[str, list], type_names: tuple) -> list:
        """Extrait les relations câble depuis les types donnés via cables_href."""
        result = []
        for type_name in type_names:
            features = features_by_type.get(type_name)
            if features:
                result.extend(self._extraire_relations_depuis_entites(features))
        return result

    def _extraire_noeud_avec_etat(self, features: list) -> list:
        """Extrait les triplets (cable_id, noeud_id, etat) depuis les noeuds."""
        result = []
        for feat in features:
            props = feat.get("properties", {})
            feat_id = props.get("id")
            cable_href = props.get("cables_href")
            if not (feat_id and cable_href):
                continue
            cables = analyser_ids_references(cable_href)
            etats_str = props.get("EtatAvantRaccordement", "")
            etats = etats_str.split(",") if etats_str else []
            for i, cid in enumerate(cables):
                etat = etats[i].strip() if i < len(etats) else ""
                result.append((cid, feat_id, etat))
        return result

    def _extraire_relations_cable_noeud(self, features_by_type: dict[str, list], type_names: tuple) -> list:
        """Extrait les relations câble-noeud avec EtatAvantRaccordement."""
        result = []
        for type_name in type_names:
            features = features_by_type.get(type_name)
            if features:
                result.extend(self._extraire_noeud_avec_etat(features))
        return result

    def _extraire_relations_ouvrage_materiel(self, features_by_type: dict[str, list]) -> list:
        """Extrait les relations ouvrage-matériel depuis les jonctions."""
        result = []
        jonction_features = features_by_type.get("RPD_Jonction_Reco")
        if not jonction_features:
            return result
        for ouvrage in jonction_features:
            props = ouvrage.get("properties", {})
            ouvrage_id = props.get("id")
            materiel_href = props.get("materiel_href")
            if ouvrage_id and materiel_href:
                result.append((ouvrage_id, materiel_href))
        return result

    def _extraire_relations_aerien_support(self, features_by_type: dict[str, list]) -> list:
        """Extrait les paires (aerien_id, support_id) depuis aeriens_href des supports.

        La relation est portée par le support : un poteau en angle ou en
        dérivation référence plusieurs aériens, d'où le découpage de la liste.
        """
        result = []
        supports = features_by_type.get("RPD_Support_Reco")
        if not supports:
            return result
        for support in supports:
            props = support.get("properties", {})
            support_id = props.get("id")
            aeriens_href = props.get("aeriens_href")
            if not (support_id and aeriens_href):
                continue
            for aerien_id in analyser_ids_references(aeriens_href):
                result.append((aerien_id, support_id))
        return result

    def _extraire_relations(self, features_by_type: dict[str, list]) -> dict[str, list]:
        """Extrait les relations à partir des références dans les propriétés."""
        chemin_types = (
            "RPD_Aerien_Reco",
            "RPD_Fourreau_Reco",
            "RPD_Galerie_Reco",
            "RPD_PleineTerre_Reco",
            "RPD_ProtectionMecanique_Reco",
        )
        return {
            "cheminement_cable": self._extraire_relations_cable(features_by_type, chemin_types),
            "cable_noeud": self._extraire_relations_cable_noeud(features_by_type, TYPES_NOEUDS_RESEAU),
            "ouvrage_materiel": self._extraire_relations_ouvrage_materiel(features_by_type),
            "aerien_support": self._extraire_relations_aerien_support(features_by_type),
        }

    def _extraire_crs_geojson(self, data: dict) -> str | None:
        """Extrait le CRS depuis la structure GeoJSON.

        Format attendu : {"crs": {"type": "name", "properties": {"name": "urn:ogc:def:crs:EPSG::2154"}}}
        Retourne le CRS normalisé (ex: "EPSG:2154") ou None si absent/invalide.
        """
        crs_block = data.get("crs")
        if not crs_block or crs_block.get("type") != "name":
            return None

        urn = crs_block.get("properties", {}).get("name", "")
        if not urn:
            return None

        # "urn:ogc:def:crs:EPSG::2154" → "EPSG:2154"
        parts = urn.split(":")
        if len(parts) >= 7 and parts[4]:
            return f"{parts[4]}:{parts[6]}"

        return None

    @staticmethod
    def _normaliser_reseaux(charge) -> list[dict]:
        """Ramène la clé ReseauUtilite de _metadata.json à une liste.

        Les fichiers produits avant le support multi-réseau portent un objet
        unique : ils restent lisibles sans conversion préalable.
        """
        if isinstance(charge, list):
            return [reseau for reseau in charge if isinstance(reseau, dict)]
        if isinstance(charge, dict):
            return [charge]
        return []

    def _charger_metadata_json(self, directory: Path) -> dict:
        """Charge le fichier _metadata.json depuis le répertoire d'entrée.

        Retourne un dictionnaire avec les clés Metadata et ReseauUtilite,
        ou un dictionnaire vide si le fichier n'existe pas.
        """
        metadata_path = directory / "_metadata.json"
        if not metadata_path.exists():
            return {}

        try:
            with open(metadata_path, encoding="utf-8") as f:
                data = json.load(f)
            print("  [OK] _metadata.json chargé")
            return data
        except Exception as e:
            print(
                f"Avertissement: Impossible de lire _metadata.json: {e}",
                file=sys.stderr,
            )
            return {}

    def charger_fichiers_geojson(self, directory: Path) -> tuple[dict[str, list], str | None]:
        """Charge tous les fichiers GeoJSON RPD_* et détecte le CRS source.

        Retourne un tuple (features_par_type, crs_detecte).
        Le CRS est extrait du premier fichier contenant une déclaration valide.
        Charge également _metadata.json si présent.
        """
        features_by_type = {}
        crs_detecte: str | None = None

        # Chargement des métadonnées sauvegardées (round-trip)
        self._metadata_chargees = self._charger_metadata_json(directory)

        # Recherche optimisée avec glob
        geojson_files = list(directory.glob("RPD_*.geojson"))

        for filepath in geojson_files:
            # Extraction du nom de fichier sans extension
            filename = filepath.stem

            try:
                with open(filepath, encoding="utf-8") as f:
                    data = json.load(f)

                if "features" in data:
                    features_by_type[filename] = data["features"]

                # Détection du CRS sur le premier fichier valide
                if crs_detecte is None:
                    crs_detecte = self._extraire_crs_geojson(data)

            except Exception as e:
                print(f"Erreur lors du chargement de {filepath}: {e}", file=sys.stderr)

        return features_by_type, crs_detecte

    def extraire_materiels_depuis_jonctions(self, jonction_features: list[dict]) -> tuple[list[dict], set[str]]:
        """Extrait les entités RPD_Materiel_Reco depuis les jonctions

        Optimisations appliquées :
        - Utilise set() pour éviter les doublons (recherche O(1))
        - Pré-allocation de la liste résultat
        - Fonctions locales pour réduire les recherches répétées

        Retourne :
            Tuple[List[Dict], Set[str]] : (liste_materiels, ensemble_ids_materiels_créés)
        """
        if not jonction_features:
            return [], set()

        # Utilisation de set pour unicité des IDs (O(1) vs list O(n))
        seen_materiel_ids = set()
        materiels = []

        # Fonction locale pour éviter les recherches répétées
        def get_props(feat):
            return feat.get("properties", {})

        # Champs requis pour créer un matériel (frozenset pour recherche O(1))
        required_fields = frozenset(["Fabricant", "Modele", "NumeroLot", "NumeroSerie"])

        for jonction in jonction_features:
            props = get_props(jonction)
            materiel_href = props.get("materiel_href")

            # Vérifier que materiel_href existe
            if not materiel_href:
                continue

            # Éviter les doublons (recherche set O(1))
            if materiel_href in seen_materiel_ids:
                continue

            # Vérifier présence des champs matériel
            has_all_fields = all(props.get(field) for field in required_fields)

            if has_all_fields:
                # Créer feature matériel
                materiel_feature = {
                    "type": "Feature",
                    "properties": {
                        "id": materiel_href,
                        "Fabricant": props["Fabricant"],
                        "Modele": props["Modele"],
                        "NumeroLot": props["NumeroLot"],
                        "NumeroSerie": props["NumeroSerie"],
                    },
                    "geometry": None,
                }

                materiels.append(materiel_feature)
                seen_materiel_ids.add(materiel_href)

        return materiels, seen_materiel_ids

    def _fusionner_materiels(self, features_by_type: dict[str, list]):
        """Fusionne les matériels extraits des jonctions avec les matériels existants."""
        jonction_features = features_by_type.get("RPD_Jonction_Reco", [])
        materiels_extraits, _ = self.extraire_materiels_depuis_jonctions(jonction_features)

        existing_materiels = features_by_type.get("RPD_Materiel_Reco", [])

        # Ensemble pour IDs existants (recherche O(1))
        existing_ids = {
            feat.get("properties", {}).get("id") for feat in existing_materiels if feat.get("properties", {}).get("id")
        }

        for materiel in materiels_extraits:
            mat_id = materiel["properties"]["id"]
            if mat_id not in existing_ids:
                existing_materiels.append(materiel)

        if existing_materiels:
            features_by_type["RPD_Materiel_Reco"] = existing_materiels

    def _transferer_proprietes_manquantes(self, feature_cible: dict, feature_source: dict):
        """Transfère les propriétés manquantes de la source vers la cible.

        Copie chaque propriété présente dans la source mais absente ou None
        dans la cible, sans écraser les valeurs existantes.
        Les identifiants (fid, ogr_pkid, id) ne sont pas transférés.
        """
        identifiants_exclus = {"fid", "ogr_pkid", "id"}
        props_cible = feature_cible["properties"]
        props_source = feature_source["properties"]

        for cle, valeur in props_source.items():
            if cle in identifiants_exclus:
                continue
            if valeur is not None and props_cible.get(cle) is None:
                props_cible[cle] = valeur

    def _selectionner_feature_prioritaire(self, features_groupe: list[dict]) -> tuple[dict, list[dict]]:
        """Sélectionne la feature prioritaire parmi un groupe de doublons.

        Règle métier : l'entité avec ChargeGeneratrice est prioritaire
        sur celle sans (AltitudeGeneratrice). Si aucune n'a ChargeGeneratrice
        ou si plusieurs en ont, la première occurrence est conservée.

        Retourne la feature conservée et la liste des features supprimées.
        """
        idx_charge = None
        for i, feature in enumerate(features_groupe):
            if feature["properties"].get("ChargeGeneratrice") is not None:
                idx_charge = i
                break

        if idx_charge is not None and idx_charge > 0:
            conservee = features_groupe[idx_charge]
            supprimees = [f for i, f in enumerate(features_groupe) if i != idx_charge]
        else:
            conservee = features_groupe[0]
            supprimees = features_groupe[1:]

        return conservee, supprimees

    def _extraire_cle_coordonnees(self, feature: dict) -> tuple | None:
        """Extrait la clé de coordonnées d'une feature pour la déduplication.

        Retourne None si la feature n'a pas de géométrie ou de coordonnées.
        """
        geometry = feature.get("geometry")
        if geometry is None:
            return None
        coords = geometry.get("coordinates")
        if coords is None:
            return None
        return tuple(coords)

    def _supprimer_doublons_geographiques_plor(self, features_by_type: dict[str, list]):
        """Supprime les doublons géographiques des RPD_PointLeveOuvrageReseau_Reco.

        Deux entités sont considérées en doublon si elles ont des coordonnées
        strictement identiques. Les entités sans géométrie sont toujours conservées.

        Règle métier : entre un doublon ChargeGeneratrice et AltitudeGeneratrice,
        l'entité ChargeGeneratrice est conservée et les propriétés pertinentes
        de l'entité supprimée sont transférées vers l'entité conservée.
        """
        type_cible = "RPD_PointLeveOuvrageReseau_Reco"
        features = features_by_type.get(type_cible)
        if not features:
            return

        groupes: dict[tuple, list[dict]] = {}
        features_sans_geom: list[dict] = []

        for feature in features:
            cle = self._extraire_cle_coordonnees(feature)
            if cle is None:
                features_sans_geom.append(feature)
            else:
                groupes.setdefault(cle, []).append(feature)

        features_uniques: list[dict] = list(features_sans_geom)
        for features_groupe in groupes.values():
            if len(features_groupe) == 1:
                features_uniques.append(features_groupe[0])
                continue

            conservee, supprimees = self._selectionner_feature_prioritaire(features_groupe)
            for feature_supprimee in supprimees:
                self._transferer_proprietes_manquantes(conservee, feature_supprimee)
            features_uniques.append(conservee)

        nb_doublons = len(features) - len(features_uniques)
        if nb_doublons > 0:
            features_by_type[type_cible] = features_uniques
            print(f"  [OK] {nb_doublons} doublon(s) geographique(s) supprime(s) pour {type_cible}")

    def _ajouter_relations(self, root, features_by_type: dict[str, list]):
        """Ajoute les relations, métadonnées et réseau à la racine GML."""
        relations = self._extraire_relations(features_by_type)

        for cable_id, noeud_id, etat in relations["cable_noeud"]:
            root.append(self._creer_relation_cable_noeud(cable_id, noeud_id, etat))

        for cable_id, cheminement_id in relations["cheminement_cable"]:
            root.append(self._creer_relation_cheminement_cable(cable_id, cheminement_id))

        for aerien_id, support_id in relations["aerien_support"]:
            root.append(self._creer_relation_aerien_support(aerien_id, support_id))

        if self.metadata:
            root.append(self._creer_membre_metadonnees())

        for ouvrage_id, materiel_id in relations["ouvrage_materiel"]:
            root.append(self._creer_relation_ouvrage_materiel(ouvrage_id, materiel_id))

        if self.metadata:
            root.extend(self._creer_membres_reseau())

    def _construire_cache_conteneurs(self, features_by_type: dict[str, list]) -> dict[str, dict]:
        """Indexe les géométries des conteneurs (Coffret, Support, BatimentTechnique) par id.

        Permet aux entités sans géométrie de résoudre leur conteneur_href
        vers une géométrie ponctuelle valide.
        """
        # Types de conteneurs possédant une géométrie propre (cohérent avec recostar_to_geojson)
        types_conteneurs = (
            "RPD_Coffret_Reco",
            "RPD_EnceinteCloturee_Reco",
            "RPD_Support_Reco",
            "RPD_BatimentTechnique_Reco",
        )
        cache: dict[str, dict] = {}

        for type_conteneur in types_conteneurs:
            for feat in features_by_type.get(type_conteneur, []):
                geom = feat.get("geometry")
                feat_id = feat.get("properties", {}).get("id")
                if feat_id and geom and geom.get("type"):
                    cache[feat_id] = geom

        return cache

    def _enrichir_geometries_depuis_conteneurs(
        self, features_by_type: dict[str, list], cache_conteneurs: dict[str, dict]
    ):
        """Attribue la géométrie du conteneur aux entités sans géométrie propre.

        Concerne les noeuds liés via conteneur_href : PointDeComptage, Jonction,
        PosteElectrique, JeuBarres, SupportModules, Terre, CoupeCircuitAFusibles,
        OuvrageCollectifBranchement.
        """
        types_noeuds = (
            "RPD_CoupeCircuitAFusibles_Reco",
            "RPD_JeuBarres_Reco",
            "RPD_Jonction_Reco",
            "RPD_OuvrageCollectifBranchement_Reco",
            "RPD_PointDeComptage_Reco",
            "RPD_PosteElectrique_Reco",
            "RPD_SupportModules_Reco",
            "RPD_Terre_Reco",
        )

        nb_enrichies = 0
        for type_noeud in types_noeuds:
            for feat in features_by_type.get(type_noeud, []):
                geom = feat.get("geometry")
                if geom and geom.get("type"):
                    continue

                conteneur_href = feat.get("properties", {}).get("conteneur_href")
                if conteneur_href and conteneur_href in cache_conteneurs:
                    feat["geometry"] = cache_conteneurs[conteneur_href]
                    nb_enrichies += 1

        if nb_enrichies > 0:
            print(f"  {nb_enrichies} entite(s) enrichie(s) avec geometrie du conteneur")

    def _ajouter_membres_entites(self, root, features_by_type: dict[str, list], type_mappers: dict):
        """Ajoute les membres entités RPD principales à la racine GML."""
        ordered_types = (
            "RPD_Aerien_Reco",
            "RPD_BatimentTechnique_Reco",
            "RPD_CableElectrique_Reco",
            "RPD_CableTelecommunication_Reco",
            "RPD_CableTerre_Reco",
            "RPD_Coffret_Reco",
            "RPD_CoupeCircuitAFusibles_Reco",
            "RPD_EnceinteCloturee_Reco",
            "RPD_GeometrieSupplementaire_Reco",
            "RPD_JeuBarres_Reco",
            "RPD_Jonction_Reco",
            "RPD_Materiel_Reco",
            "RPD_ModuleRaccordement_Reco",
            "RPD_OuvrageCollectifBranchement_Reco",
            "RPD_PleineTerre_Reco",
            "RPD_Fourreau_Reco",
            "RPD_Galerie_Reco",
            "RPD_PointDeComptage_Reco",
            "RPD_PointLeveOuvrageReseau_Reco",
            "RPD_PosteElectrique_Reco",
            "RPD_ProtectionMecanique_Reco",
            "RPD_Support_Reco",
            "RPD_SupportModules_Reco",
            "RPD_Terre_Reco",
        )

        for feature_type in ordered_types:
            features = features_by_type.get(feature_type)
            if not features:
                continue
            mapper_func = type_mappers.get(feature_type)
            if not mapper_func:
                continue

            for feature in features:
                props = feature.get("properties", {})
                feature_id = props.get("id") or f"id{props.get('fid', '')}"
                member = ET.SubElement(root, f"{{{NAMESPACE_GML}}}featureMember")
                member.append(mapper_func(feature, feature_id))

    def _ecrire_fichier_gml(self, root, output_path: Path):
        """Écrit l'arbre XML dans le fichier GML avec en-tête et commentaire."""
        tree = ET.ElementTree(root)
        ET.indent(tree, space="  ")

        with open(output_path, "wb") as f:
            f.write(b'<?xml version="1.0" encoding="utf-8" ?>\n')

            tree_bytes = ET.tostring(root, encoding="utf-8")
            tree_str = tree_bytes.decode("utf-8")
            lines = tree_str.split("\n")

            if lines:
                f.write(lines[0].encode("utf-8") + b"\n")
                f.write(b"<!-- GML au format RecoStarElec v1.10. -->\n")
                for line in lines[1:]:
                    f.write(line.encode("utf-8") + b"\n")

        print(f"Fichier GML généré: {output_path}")

    def generer_gml(
        self,
        features_by_type: dict[str, list],
        output_path: Path,
        remplacer_ids: bool = False,
    ):
        """Génère le fichier GML à partir des entités.

        Si remplacer_ids est True, tous les gml:id et les xlink:href associés
        sont reformatés en UUIDs préfixés 'id' avant l'écriture.
        """

        for prefix, uri in NS_MAP.items():
            ET.register_namespace(prefix, uri)

        root = ET.Element(f"{{{NAMESPACE_GML}}}FeatureCollection")
        root.set(
            f"{{{NAMESPACE_XSI}}}schemaLocation",
            # Tag canonique de la version 1.1 : RecoStar-v1.1 (et non v1.10).
            f"{NAMESPACE_RECOSTAR} https://gitlab.com/StaR-Elec/StaR-Elec/-/raw/RecoStar-v1.1/RecoStaR/SchemaStarElecRecoStar.xsd",
        )

        # Dispatch O(1) des types vers les fonctions de mappage
        type_mappers = {
            "RPD_Aerien_Reco": self.mapper.mapper_aerien,
            "RPD_CableElectrique_Reco": self.mapper.mapper_cable_electrique,
            "RPD_CableTelecommunication_Reco": self.mapper.mapper_cable_telecommunication,
            "RPD_CableTerre_Reco": self.mapper.mapper_cable_terre,
            "RPD_Coffret_Reco": self.mapper.mapper_coffret,
            "RPD_CoupeCircuitAFusibles_Reco": self.mapper.mapper_coupe_circuit_a_fusibles,
            "RPD_EnceinteCloturee_Reco": self.mapper.mapper_enceinte_cloturee,
            "RPD_Fourreau_Reco": self.mapper.mapper_fourreau,
            "RPD_Galerie_Reco": self.mapper.mapper_galerie,
            "RPD_GeometrieSupplementaire_Reco": self.mapper.mapper_geometrie_supplementaire,
            "RPD_JeuBarres_Reco": self.mapper.mapper_jeu_barres,
            "RPD_Jonction_Reco": self.mapper.mapper_jonction,
            "RPD_ModuleRaccordement_Reco": self.mapper.mapper_module_raccordement,
            "RPD_OuvrageCollectifBranchement_Reco": self.mapper.mapper_ouvrage_collectif_branchement,
            "RPD_PointDeComptage_Reco": self.mapper.mapper_point_comptage,
            "RPD_PointLeveOuvrageReseau_Reco": self.mapper.mapper_point_leve,
            "RPD_Materiel_Reco": self.mapper.mapper_materiel,
            "RPD_PleineTerre_Reco": self.mapper.mapper_pleine_terre,
            "RPD_Support_Reco": self.mapper.mapper_support,
            "RPD_SupportModules_Reco": self.mapper.mapper_support_modules,
            "RPD_BatimentTechnique_Reco": self.mapper.mapper_batiment_technique,
            "RPD_PosteElectrique_Reco": self.mapper.mapper_poste_electrique,
            "RPD_ProtectionMecanique_Reco": self.mapper.mapper_protection_mecanique,
            "RPD_Terre_Reco": self.mapper.mapper_terre,
        }

        self._fusionner_materiels(features_by_type)

        # Résolution des géométries héritées avant écriture
        cache_conteneurs = self._construire_cache_conteneurs(features_by_type)
        self._enrichir_geometries_depuis_conteneurs(features_by_type, cache_conteneurs)

        print("Suppression des doublons géographiques (PointLeveOuvrageReseau)...")
        self._supprimer_doublons_geographiques_plor(features_by_type)

        self._ajouter_relations(root, features_by_type)
        self._ajouter_membres_entites(root, features_by_type, type_mappers)

        if remplacer_ids:
            RemappeurIds().remanier(root)

        self._ecrire_fichier_gml(root, output_path)


def main():
    """Point d'entrée principal du script"""
    parser = argparse.ArgumentParser(
        description="Convertit des fichiers GeoJSON RPD_* en GML RecoStaR conforme au XSD",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )

    parser.add_argument("input_dir", type=Path, help="Répertoire contenant les fichiers GeoJSON RPD_*")

    parser.add_argument("output_file", type=Path, help="Fichier GML de sortie")

    parser.add_argument("--logiciel", default=None, help="Logiciel utilisé pour la génération")

    parser.add_argument("--producteur", default=None, help="Producteur du récolement")

    parser.add_argument("--responsable", default=None, help="Responsable du récolement")

    parser.add_argument("--nom", default=None, help="Nom du réseau")

    parser.add_argument(
        "--srs",
        default=None,
        help="Forcer le CRS (ex: EPSG:2154). Si absent, le CRS est détecté depuis les GeoJSON.",
    )

    parser.add_argument(
        "--id",
        dest="reformater_ids",
        action="store_true",
        default=False,
        help="Reformate tous les gml:id en UUIDs prefixes par 'id' (ex: id7515b3fc-...).",
    )

    parser.add_argument(
        "--commentaire",
        dest="commentaire_vide",
        action="store_true",
        default=False,
        help=(
            "Ajoute une balise Commentaire vide aux entites qui n'en possedent pas "
            "(evolution V1.1 du standard). Les commentaires deja renseignes sont conserves."
        ),
    )

    args = parser.parse_args()

    # Validation du répertoire d'entrée
    if not args.input_dir.exists():
        print(f"Erreur: Le répertoire {args.input_dir} n'existe pas", file=sys.stderr)
        sys.exit(1)

    if not args.input_dir.is_dir():
        print(f"Erreur: {args.input_dir} n'est pas un répertoire", file=sys.stderr)
        sys.exit(1)

    # Chargement des fichiers GeoJSON (avec détection CRS)
    generator = GenerateurGML(commentaire_vide=args.commentaire_vide)
    print(f"Chargement des fichiers GeoJSON depuis {args.input_dir}...")
    features, crs_detecte = generator.charger_fichiers_geojson(args.input_dir)

    if not features:
        print("Aucun fichier GeoJSON RPD_* trouvé", file=sys.stderr)
        sys.exit(1)

    # Résolution du CRS : override CLI > détection GeoJSON > défaut
    if args.srs:
        srs_final = args.srs
        print(f"CRS forcé par l'utilisateur : {srs_final}")
    elif crs_detecte:
        srs_final = crs_detecte
        print(f"CRS détecté depuis les fichiers GeoJSON : {srs_final}")
    else:
        srs_final = DEFAULT_SRS
        print(f"Aucun CRS détecté, utilisation du défaut : {srs_final}")

    # Le CRS résolu est appliqué au générateur existant : le recréer perdrait
    # les métadonnées chargées depuis _metadata.json.
    generator.definir_metadonnees(
        logiciel=args.logiciel,
        producteur=args.producteur,
        responsable=args.responsable,
        nom=args.nom,
        srs=srs_final,
    )

    print(f"Fichiers chargés: {', '.join(features.keys())}")

    # Génération du GML
    print("Génération du fichier GML...")
    generator.generer_gml(features, args.output_file, remplacer_ids=args.reformater_ids)

    print("Conversion terminée avec succès!")


if __name__ == "__main__":
    main()
