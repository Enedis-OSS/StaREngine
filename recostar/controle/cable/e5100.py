"""
Controle E-5100 : densite de sommets des cables electriques.
...
Perimetre :
  ...
  - Les cables references par un cheminement aerien (RPD_Aerien_Reco.cables_href)
    sont exclus, conformement au comportement d'E-5201 et du moteur des
    sommets de cables.
...
non conforme et genere une anomalie E-5100.

Usage CLI :
    python -m recostar.controle.cable.e5100 --repertoire <chemin> [--sortie <chemin>]

Sortie : ecarts_e5100_densite_sommets_cable.geojson
"""
...
FICHIER_SORTIE: str = "ecarts_e5100_densite_sommets_cable.geojson"

# Identite du controle, utilisee pour normaliser les proprietes des ecarts.
CODE_CONTROLE: str = "E-5100"

DESCRIPTIONS_ANOMALIES: dict[str, str] = {
    "densite_sommets_insuffisante": ("La distance entre deux sommets consécutifs du câble dépasse le seuil autorisé."),
}
...
# ---------------------------------------------------------------------------
# Exclusion des cables aeriens (mecanisme commun a l'altimetrie)
# ---------------------------------------------------------------------------
...
def _analyser_partie(sommets: list[list[float]], seuil: float) -> tuple[float, int]:
    ...
    La distance est calculee en 3D selon la convention du calcul de longueur du
    projet, altitudes nulles corrigees comprises : un sommet dont Z vaut 0.0
    n'est pas au niveau de la mer, son altitude est simplement absente. Sans
    cette correction, un tel sommet produit un denivele fictif egal a l'altitude
    du terrain — plusieurs centaines de metres attribuees a un segment de
    quelques centimetres, donc une non-conformite E-5100 sur un cable dont la
    densite de sommets est en realite conforme.
    """
    distance_max = 0.0
    nb_depassements = 0
