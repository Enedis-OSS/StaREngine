...
|------|------|------------------------------|--------------------|-----------|-----------|-----------|
| E010 | E110 | `controle_e010.py` / `controle_e110.py` | Ordre des éléments des objets RPD | `sequenceur_xsd` (XSD de la version) | ERREUR | forte |
| E011 | E111 | `controle_e011.py` / `controle_e111.py` | Règles métier conditionnelles | `regles_metier` (PDF de structuration) | ERREUR | forte |
| E012 | E112 | `controle_e012.py` / `controle_e112.py` | Validation XSD native complète | XSD officiel via `lxml` | ERREUR | moyenne (code `E-1104`) |
| E013 | E113 | `controle_e013.py` / `controle_e113.py` | En-tête, namespaces, métadonnées, unicité `gml:id` | `regles_entete` | ERREUR | forte, **sauf `schemaLocation` sur la branche `main` → moyenne** |
| E014 | E114 | `controle_e014.py` / `controle_e114.py` | Valeurs des champs (énumérations, CodeLists, formats) | `regles_valeurs` (PDF §9/§10) | ERREUR | forte, **sauf `ReseauUtilite/Theme` → basse** |
...
un champ `priorite` — et **seules les entrées `forte` invalident la
...
sévérité `ERREUR`, mais toutes ne déclassent pas.

L'échelle est celle du **vérificateur RecoStaR** : `forte`, `moyenne`, `basse`.
Le niveau `bloquante` du vérificateur, qui signifie l'interruption du traitement,
est ramené à `forte` par `NIVEAUX_VERIFICATEUR` — star-engine poursuit toujours
son analyse.

| Priorité | Déclasse ? | Règles concernées |
|----------|-----------|-------------------|
| `forte` | **oui** | toutes, sauf celles ci-dessous |
| `moyenne` | non | E112/E012 en entier (code `E-1104`) ; E113/E013 — `xsi:schemaLocation` pointant la branche `main` du XSD |
| `basse` | non | E114/E014 — règle `E_THEME_RPD` (`ReseauUtilite/Theme` hors énumération) |

La priorité d'une règle **n'est plus choisie ici** : elle est le niveau du code
d'erreur du vérificateur que la règle déclenche, lu dans `NIVEAU_PAR_CODE` par
`priorite_par_regle()`. Une règle sans code retombe sur `PRIORITE_PAR_DEFAUT`
(`forte`) : une anomalie de structuration non répertoriée reste déclassante.
...
Le champ `priorite` de `RegleValeur` a pour défaut `forte` : **oublier de la
...
- un fichier dont les seules anomalies sont moyennes ou basses est
...
- **`priorites_structuration.py`** : échelle de priorité des anomalies
  (`forte` / `moyenne` / `basse`), ventilation (`ventiler_par_priorite`) et
...
**`moyenne`** (`regles_entete.PRIORITE_SCHEMA_LOCATION_BRANCHE_MAIN`) : le
...
**Priorités :** une seule règle du catalogue déroge au niveau `forte` —
`E_THEME_RPD` (`ReseauUtilite/Theme` hors de l'énumération `{ELECTRD}`, §9) est
de priorité **`basse`** : l'étiquette de réseau est corrigeable sans reprise du
...
- `nb_erreurs_bloquantes` : sous-total des seules erreurs déclassantes (`forte`) ;
...
- `conformite_globale` : `CONFORME` seulement si aucune erreur **bloquante**
  **et** aucun contrôle en échec. Un fichier ne portant que des anomalies
  moyennes ou basses est donc `CONFORME`, ces anomalies restant comptées dans
