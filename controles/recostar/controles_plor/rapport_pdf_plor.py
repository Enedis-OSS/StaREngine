"""
Generation du rapport PDF de synthese des controles PLOR.

Produit un document PDF recapitulatif des resultats de l'ensemble des
controles PLOR (cable, doublons, cheminement superpose). Le rapport
presente une synthese globale, un tableau recapitulatif par controle,
puis le detail des entites en ecart avec leurs identifiants.

Usage CLI :
    python rapport_pdf_plor.py --repertoire <chemin> [--sortie <chemin>]

Sortie : rapport_controles_plor.pdf
"""
...
FICHIER_RAPPORT: str = "rapport_controles_plor.pdf"
...
_COLONNES_CABLE: tuple[tuple[str, str, float], ...] = (
    ("id_entite", _LABEL_ID_ENTITE, 6.0),
    ("type_anomalie", "Type anomalie", 4.5),
    ("priorite", "Priorite", 2.5),
)

_COLONNES_DOUBLONS: tuple[tuple[str, str, float], ...] = (
    ("id_entite", _LABEL_ID_ENTITE, 5.5),
    ("nb_doublons", "Nb doublons", 2.5),
    ("type_anomalie", "Type anomalie", 4.0),
    ("priorite", "Priorite", 2.5),
)

_COLONNES_CHEMINEMENT: tuple[tuple[str, str, float], ...] = (
    ("id_entite", _LABEL_ID_ENTITE, 4.0),
    ("fichier_source", "Fichier source", 5.0),
    ("nb_superpositions", "Nb superpositions", 2.5),
    ("priorite", "Priorite", 2.5),
)
...
CONTROLES: tuple[tuple[str, str, str, tuple[tuple[str, str, float], ...]], ...] = (
    (
        ...
        "Superposition PLOR / cables",
        "Points leves non superposes aux sommets des cables",
        _COLONNES_CABLE,
    ),
    (
        "plor_doublons.geojson",
        "Doublons de points",
        "Points leves en doublon geometrique (X, Y, Z)",
        _COLONNES_DOUBLONS,
    ),
    (
        "cheminement_superpose.geojson",
        "Superposition de cheminements",
        "Cheminements partageant des segments geometriques",
        _COLONNES_CHEMINEMENT,
    ),
)
...
def generer_rapport_pdf(
    ...
    """Genere le rapport PDF de synthese des controles PLOR.
    ...
    # Titre
    elements.append(Paragraph("Rapport des controles PLOR", styles["titre"]))
    elements.append(Spacer(1, 6 * mm))
...
def main() -> None:
    ...
    parseur = argparse.ArgumentParser(
        description="Generation du rapport PDF des controles PLOR"
    )
