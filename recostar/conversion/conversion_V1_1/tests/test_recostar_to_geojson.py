...
from recostar.conversion.conversion_V1_1.recostar_to_geojson import (
    CHAMP_NATURE_GEOMETRIE,
    GEOMETRY_LIGNE_3D,
    GEOMETRY_SURFACE_3D,
    NAMESPACE_GML,
    NAMESPACE_RECOSTAR,
...
class TestExtracteurEntitesExtraction:
    ...
    def test_extraire_cable_telecommunication_optionnels_absents(self, entity_extractor):
    ...
    def test_extraire_module_raccordement_etat_avant_raccordement(self, entity_extractor):
        ...
        entity_extractor.relations["cable_noeud_etat"]["module_001_cable_aaa"] = "RACCORDE"
        elem = self._creer_element_module_raccordement()
        feature = entity_extractor.extract_module_raccordement(elem)
...
class TestConvertisseurGMLRelations:
    ...
    def _creer_arbre_relations(self):
        ...
        # Relations Aerien_Support : deux aériens portés par le même support
        for aerien_id in ("aerien_001", "aerien_002"):
            member = ET.SubElement(root, f"{{{NAMESPACE_GML}}}featureMember")
            rel = ET.SubElement(member, f"{{{NAMESPACE_RECOSTAR}}}Aerien_Support")
            aerien_ref = ET.SubElement(rel, f"{{{NAMESPACE_RECOSTAR}}}aerien")
            aerien_ref.set(f"{{{NAMESPACE_XLINK}}}href", aerien_id)
            support_ref = ET.SubElement(rel, f"{{{NAMESPACE_RECOSTAR}}}support")
            support_ref.set(f"{{{NAMESPACE_XLINK}}}href", "support_001")
        return root
    ...
    def test_extraire_relations_aerien_support(self, gml_converter):
        """Un support porteur de plusieurs aériens conserve la liste complète."""
        root = self._creer_arbre_relations()
        gml_converter._extract_relations(root)
        rels = gml_converter.extractor.relations
        assert rels["support_aerien"]["support_001"] == ["aerien_001", "aerien_002"]

    def test_extraire_relations_aerien_support_incomplete(self, gml_converter):
        """Une relation amputée d'une de ses deux références est ignorée."""
        root = ET.Element(f"{{{NAMESPACE_GML}}}FeatureCollection")
        member = ET.SubElement(root, f"{{{NAMESPACE_GML}}}featureMember")
        rel = ET.SubElement(member, f"{{{NAMESPACE_RECOSTAR}}}Aerien_Support")
        aerien_ref = ET.SubElement(rel, f"{{{NAMESPACE_RECOSTAR}}}aerien")
        aerien_ref.set(f"{{{NAMESPACE_XLINK}}}href", "aerien_001")
        gml_converter._extract_relations(root)
        assert gml_converter.extractor.relations["support_aerien"] == {}


class TestExtractionSupportAeriens:
    """Tests du report de la relation Aerien_Support sur RPD_Support_Reco."""

    def _creer_support(self, gml_id: str = "support_001") -> ET.Element:
        """Crée un élément RPD_Support_Reco minimal."""
        elem = ET.Element(f"{{{NAMESPACE_RECOSTAR}}}RPD_Support_Reco")
        elem.set(f"{{{NAMESPACE_GML}}}id", gml_id)
        return elem

    def test_aeriens_href_liste_multiple(self, entity_extractor):
        """Les aériens portés sont concaténés dans l'ordre de lecture du GML."""
        entity_extractor.relations["support_aerien"]["support_001"] = [
            "aerien_001",
            "aerien_002",
        ]
        feature = entity_extractor.extract_support(self._creer_support())
        assert feature["properties"]["aeriens_href"] == "aerien_001,aerien_002"

    def test_aeriens_href_unique(self, entity_extractor):
        """Un seul aérien porté produit une valeur sans virgule."""
        entity_extractor.relations["support_aerien"]["support_001"] = ["aerien_001"]
        feature = entity_extractor.extract_support(self._creer_support())
        assert feature["properties"]["aeriens_href"] == "aerien_001"

    def test_aeriens_href_absent_sans_relation(self, entity_extractor):
        """Un support sans relation ne reçoit pas la propriété."""
        feature = entity_extractor.extract_support(self._creer_support())
        assert "aeriens_href" not in feature["properties"]

    def test_references_liees_sans_identifiant(self, entity_extractor):
        """Un identifiant absent ne déclenche aucune résolution de relation."""
        assert entity_extractor._references_liees("support_aerien", None) == []


class TestRattachementReseau:
    """Tests du report de la référence `reseau` sur les entités extraites."""

    def _creer_entite(self, *reseaux: str) -> ET.Element:
        """Crée une entité portant les références réseau demandées."""
        elem = ET.Element(f"{{{NAMESPACE_RECOSTAR}}}RPD_Aerien_Reco")
        elem.set(f"{{{NAMESPACE_GML}}}id", "aerien_001")
        for reseau_id in reseaux:
            ref = ET.SubElement(elem, f"{{{NAMESPACE_RECOSTAR}}}reseau")
            ref.set(f"{{{NAMESPACE_XLINK}}}href", reseau_id)
        return elem

    def test_rattachement_unique(self, entity_extractor):
        """Une seule référence produit une valeur sans virgule."""
        properties = {}
        entity_extractor.peupler_reseau_href(properties, self._creer_entite("ReseauA"))
        assert properties["reseau_href"] == "ReseauA"

    def test_rattachement_multiple(self, entity_extractor):
        """Le XSD autorise plusieurs réseaux : ils sont concaténés dans l'ordre."""
        properties = {}
        entity_extractor.peupler_reseau_href(properties, self._creer_entite("ReseauA", "ReseauB"))
        assert properties["reseau_href"] == "ReseauA,ReseauB"

    def test_rattachement_absent(self, entity_extractor):
        """Une entité hors ElementReseauType ne reçoit pas la propriété."""
        properties = {}
        entity_extractor.peupler_reseau_href(properties, self._creer_entite())
        assert "reseau_href" not in properties

    def test_rattachement_reference_vide(self, entity_extractor):
        """Une balise `reseau` sans href est ignorée."""
        elem = self._creer_entite()
        ET.SubElement(elem, f"{{{NAMESPACE_RECOSTAR}}}reseau")
        properties = {}
        entity_extractor.peupler_reseau_href(properties, elem)
        assert "reseau_href" not in properties
...
class TestDetectionVersionSchema:
    ...
    def test_detecter_version_v1_10(self):
        """Vérifie la détection de la version V1.10 (tag legacy) via l'URL complète."""
        converter = GMLConverter()
        root = ET.Element(f"{{{NAMESPACE_GML}}}FeatureCollection")
        root.set(
            f"{{{NAMESPACE_XSI}}}schemaLocation",
    ...
    def test_detecter_version_v1_1_canonique(self):
        """Le tag canonique RecoStar-v1.1 est reconnu comme la version 1.1."""
        converter = GMLConverter()
        root = ET.Element(f"{{{NAMESPACE_GML}}}FeatureCollection")
        root.set(
            f"{{{NAMESPACE_XSI}}}schemaLocation",
            f"{NAMESPACE_RECOSTAR} {SCHEMA_URL_V1_1}",
        )
        assert converter._detecter_version_schema(root) == VERSION_V1_10

    def test_detecter_version_fallback_v1_1(self):
        """Fallback par recherche partielle : 'RecoStar-v1.1' → version 1.1."""
        converter = GMLConverter()
        root = ET.Element(f"{{{NAMESPACE_GML}}}FeatureCollection")
        root.set(
            f"{{{NAMESPACE_XSI}}}schemaLocation",
            "http://example.com RecoStar-v1.1/schema.xsd",
        )
        assert converter._detecter_version_schema(root) == VERSION_V1_10
...
<gml:FeatureCollection
    ...
    def test_extraction_reseau_utilite(self, tmp_path):
        ...
        # Vérification ReseauUtilite : une liste, le fichier pouvant porter
        # plusieurs tranches de travaux
        assert "ReseauUtilite" in metadata
        assert len(metadata["ReseauUtilite"]) == 1
        reseau = metadata["ReseauUtilite"][0]
    ...
    def test_extraction_plusieurs_reseaux(self, tmp_path):
        """Les tranches de travaux multiples sont toutes conservées, dans l'ordre."""
        gml_file = self._creer_gml_avec_metadata(tmp_path)
        contenu = gml_file.read_text(encoding="utf-8")
        # Duplique le ReseauUtilite en une seconde tranche
        bloc = contenu[
            contenu.index("<RecoStaR:ReseauUtilite") : contenu.index("</RecoStaR:ReseauUtilite>")
            + len("</RecoStaR:ReseauUtilite>")
        ]
        second = bloc.replace("Reseau_Test", "Reseau_Test_B").replace("Tranche ABC", "Tranche DEF")
        contenu = contenu.replace(
            bloc,
            f"{bloc}</gml:featureMember><gml:featureMember>{second}",
        )
        gml_file.write_text(contenu, encoding="utf-8")

        output_dir = tmp_path / "sortie_multi"
        GMLConverter().convert_gml_to_geojson(gml_file, output_dir)
        metadata = json.loads((output_dir / "_metadata.json").read_text(encoding="utf-8"))

        assert [reseau["id"] for reseau in metadata["ReseauUtilite"]] == [
            "Reseau_Test",
            "Reseau_Test_B",
        ]
...
class TestSupportGeometrieSupplementaire:
    ...
    def test_extraction_geom_supp_href_support(self, extracteur):
        ...
        ET.SubElement(support, f"{ns_r}NatureSupport").set(f"{ns_xl}href", "http://example.com/nature")
...
class TestPropagationCablesParTension:
    ...
    def _creer_jonction(self, id_jonction: str, tension: str, conteneur: str, cables_href: str = "") -> dict:
    ...
    def test_propagation_inchangee_pour_autres_noeuds(self):
        ...
        assert features_by_type["RPD_PosteElectrique_Reco"][0]["properties"]["cables_href"] == attendu
        assert features_by_type["RPD_SupportModules_Reco"][0]["properties"]["cables_href"] == attendu
...
class TestExtractGeometrieSupplementaire:
    ...
    def test_ligne3d_stocke_en_ligne3d(self, entity_extractor):
        ...
        self._ajouter_ligne(elem, "Ligne3D", "100000.0 200000.0 10.0 100010.0 200010.0 20.0")
        result = entity_extractor.extract_geometrie_supplementaire(elem)
        # Sans surface pour l'occuper, la ligne devient la geometrie de la feature.
        assert result["geometry"] == {
            "type": "LineString",
            "coordinates": [[100000.0, 200000.0, 10.0], [100010.0, 200010.0, 20.0]],
        }
        assert result["properties"][CHAMP_NATURE_GEOMETRIE] == GEOMETRY_LIGNE_3D

    def test_ligne3d_seule_sans_propriete_texte(self, entity_extractor):
        """La ligne portee par la geometrie n'est pas dupliquee en propriete.

        La dupliquer ferait emettre deux Ligne3D a la reecriture.
        """
        elem = self._creer_elem_geom_supp()
        self._ajouter_ligne(elem, "Ligne3D", "100000.0 200000.0 10.0 100010.0 200010.0 20.0")
        result = entity_extractor.extract_geometrie_supplementaire(elem)
    ...
    def test_ligne2_5d_fallback_stocke_en_ligne3d(self, entity_extractor):
        """Vérifie que Ligne2.5D (legacy/EP) produit aussi la geometry de la feature."""
        elem = self._creer_elem_geom_supp()
        self._ajouter_ligne(elem, "Ligne2.5D", "100000.0 200000.0 10.0 100010.0 200010.0 20.0")
        result = entity_extractor.extract_geometrie_supplementaire(elem)
        assert result["geometry"]["type"] == "LineString"
        assert result["properties"][CHAMP_NATURE_GEOMETRIE] == GEOMETRY_LIGNE_3D

    def test_ligne_et_surface_coexistent(self, entity_extractor):
        """La surface occupe la geometrie ; la ligne garde le canal texte."""
        elem = self._creer_elem_geom_supp()
        self._ajouter_ligne(elem, "Ligne3D", "100000.0 200000.0 10.0 100010.0 200010.0 20.0")
        self._ajouter_surface(elem, "Surface3D")
        result = entity_extractor.extract_geometrie_supplementaire(elem)
        assert result["geometry"]["type"] == "MultiPolygon"
        assert result["properties"][CHAMP_NATURE_GEOMETRIE] == GEOMETRY_SURFACE_3D
        assert result["properties"][GEOMETRY_LIGNE_3D] == "100000.0 200000.0 10.0 100010.0 200010.0 20.0"

    def test_sans_geometrie_aucune_nature(self, entity_extractor):
        """Une entite sans ligne ni surface ne declare aucune nature."""
        result = entity_extractor.extract_geometrie_supplementaire(self._creer_elem_geom_supp())
        assert result["geometry"] is None
        assert CHAMP_NATURE_GEOMETRIE not in result["properties"]
...
class TestExtractGalerie:
    ...
    def entity_extractor(self):
        """Instance d'EntityExtractor avec namespace helper."""
        from recostar.conversion.conversion_V1_1.recostar_to_geojson import EntityExtractor, GMLNamespaceHelper
